...
ssize_t BT_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
	...
	if (count > 0) {
		...
		case HCI_COMMAND_TYPE:
			 ...
			 * Header length = 1 + 2 + 1
			 */
			pkt_len = o_buf[3] + MTK_HCI_CMD_HEADER_LEN;
			break;
		case HCI_ACLDATA_TYPE:
			/* ACL data : Type(8b) handle+flag(16b) length(16b)
			 * Header length = 1 + 2 + 2
			 */
			pkt_len = (o_buf[3] | (o_buf[4] << 8)) + MTK_HCI_ACL_HEADER_LEN;
			break;
		case HCI_SCODATA_TYPE:
			/* SCO data : Type(8b) handle+flag(16b) length(8b)
			 * Header length = 1 + 2 + 1
			 */
			pkt_len = o_buf[3] + MTK_HCI_SCO_HEADER_LEN;
			break;
		default:
			BT_LOG_PRT_ERR("type is 0x%x", pkt_type);
			retval = -EINVAL;
			goto OUT;
		}

		BT_LOG_PRT_DBG("pkt_len count %d\n", pkt_len);
		BT_LOG_PRT_DBG_RAW(o_buf, count, "%s: len[%d], TX: ", __func__, count);
		/* check frame length is valid */
		if (pkt_len != count) {
			BT_LOG_PRT_ERR("input len(%d) error (expect %d)", count, pkt_type);
			retval = -EFAULT;
			goto OUT;
		}
