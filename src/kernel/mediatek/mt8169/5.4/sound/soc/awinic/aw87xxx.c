...
struct gpio_desc *spk_kpa_5v_en_gpio;
...
EXPORT_SYMBOL_GPL(aw87xxx_show_current_profile);
...
EXPORT_SYMBOL_GPL(aw87xxx_set_profile);
				...
				const struct i2c_device_id *id)
{
	...
	if (spk_kpa_5v_en_gpio == NULL) {
		spk_kpa_5v_en_gpio = devm_gpiod_get(dev, "spk-kpa-5v-en",
							GPIOD_OUT_HIGH);

		if (IS_ERR(spk_kpa_5v_en_gpio)) {
			ret = PTR_ERR(spk_kpa_5v_en_gpio);
			spk_kpa_5v_en_gpio = NULL;
			dev_info(dev, "failed to get spk_kpa_5v_en_gpio: %d\n", ret);
		}
	} else {
		dev_info(dev, "spk_kpa_5v_en_gpio has been got");
	}

	usleep_range(5000, 5000 + 1);
	/*hw power on PA*/
