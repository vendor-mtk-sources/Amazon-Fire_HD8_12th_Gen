...
/*
 * The number of firmware bin is 2 for now which is related to the number of speaker
 * and doesn`t exceed 8 for AW83790 PA in the future.
 **/
...
int aw_check_dsp_reg_num_v1(struct aw_bin *bin, int bin_num)
{
	...
	if (bin->header_info[bin_num].reg_data_byte_len != 0 &&
			bin->header_info[bin_num].bin_data_len > 12) {
		check_dsp_reg_num =
		    (bin->header_info[bin_num].bin_data_len -
			12) / bin->header_info[bin_num].reg_data_byte_len;
	} else {
		DBG_ERR("aw_bin_parse date length of header info is invalid\n");
		return -3;
	}
	DBG
