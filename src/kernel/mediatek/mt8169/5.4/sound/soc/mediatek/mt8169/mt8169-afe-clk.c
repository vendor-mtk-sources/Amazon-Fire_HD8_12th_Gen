...
//
// mt8169-afe-clk.c  --  Mediatek 8169 afe clock ctrl
//
...
static const char *aud_clks[CLK_NUM] = {
	...
	[CLK_I2S1_BCLK] = "aud_i2s1_b_clk",
	[CLK_I2S2_BCLK] = "aud_i2s2_b_clk",
	[CLK_I2S3_BCLK] = "aud_i2s3_b_clk",
	[CLK_I2S4_BCLK] = "aud_i2s4_b_clk",
	...
	[CLK_ETDM_IN1_BCLK] = "aud_etdm_in1_b_clk",
	[CLK_ETDM_OUT1_BCLK] = "aud_etdm_out1_b_clk",
...
int mt8169_set_audio_int_bus_parent(struct mtk_base_afe *afe,
					   int clk_id)
{
	struct mt8169_afe_private *afe_priv = afe->platform_priv;
	int ret;

	if (clk_id < CLK_AFE || clk_id >= CLK_NUM) {
...
int mt8169_afe_enable_clock(struct mtk_base_afe *afe)
{
	...
	ret = mt8169_set_audio_int_bus_parent(afe, CLK_CLK26M);
	if (ret)
...
int mt8169_afe_suspend_clock(struct mtk_base_afe *afe)
{
	struct mt8169_afe_private *afe_priv = afe->platform_priv;
	int ret;
...
int mt8169_afe_resume_clock(struct mtk_base_afe *afe)
{
	struct mt8169_afe_private *afe_priv = afe->platform_priv;
	int ret;
	...
	ret = mt8169_set_audio_int_bus_parent(afe,
...
int mt8169_afe_dram_request(struct device *dev)
{
	...
	if (afe_priv->dram_resource_counter == 0)
		pm_qos_add_request(&(afe_priv->qos_request),
...
int mt8169_afe_dram_release(struct device *dev)
{
	...
	if (afe_priv->dram_resource_counter == 0)
		pm_qos_remove_request(&(afe_priv->qos_request));
...
{
	return (apll == MT8169_APLL1) ? 180633600 : 196608000;
}

int mt8169_get_apll_by_rate(struct mtk_base_afe *afe, int rate)
{
	return ((rate % 8000) == 0) ? MT8169_APLL2 : MT8169_APLL1;
}
...
static const struct mt8169_mck_div mck_div[MT8169_MCK_NUM] = {
	[MT8169_I2S0_MCK] = {
...
{
	struct mt8169_afe_private *afe_priv = afe->platform_priv;
	int apll = mt8169_get_apll_by_rate(afe, rate);
	int apll_clk_id = apll == MT8169_APLL1 ?
			  CLK_TOP_MUX_AUD_1 : CLK_TOP_MUX_AUD_2;
	int m_sel_id;
	int div_clk_id;
	int ret;

	if (mck_id < MT8169_I2S0_MCK || mck_id >= MT8169_MCK_NUM) {
	...
	/* select apll */
	if (m_sel_id >= CLK_AFE && m_sel_id < CLK_NUM) {
		...
		dev_info(afe->dev, "%s invalid m_sel_id:%d\n", __func__, m_sel_id);
		ret = -EINVAL;
		goto error;
	}

	/* enable div, set rate */
	if (div_clk_id >= CLK_AFE && div_clk_id < CLK_NUM) {
