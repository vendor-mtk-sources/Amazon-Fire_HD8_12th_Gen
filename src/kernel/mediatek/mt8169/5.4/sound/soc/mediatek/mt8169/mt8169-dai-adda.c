			     ...
			     int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		mt8169_afe_gpio_request(afe->dev, true, MT8169_DAI_ADDA, 1);
		...
		if (mtkaif_dmic) {
			...
			regmap_update_bits(afe->regmap, AFE_ADDA_MTKAIF_RX_CFG0,
					   ...
					   0x0);
			mtk_adda_ul_src_dmic(afe, MT8169_DAI_ADDA);
		}
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* should delayed 1/fs(smallest is 8k) = 125us before afe off */
		udelay(125);
		mt8169_afe_gpio_request(afe->dev, false, MT8169_DAI_ADDA, 1);
				     ...
				     int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		regmap_update_bits(afe->regmap,
				   ...
				   MTKAIF_IF_LOOPBACK1_MASK_SFT,
				   afe_priv->mtkaif_looback1 <<
				   MTKAIF_IF_LOOPBACK1_SFT);
#endif
		break;
	default:
		break;
			     ...
			     int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		mt8169_afe_gpio_request(afe->dev, true, MT8169_DAI_ADDA, 0);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* should delayed 1/fs(smallest is 8k) = 125us before afe off */
		udelay(125);
		mt8169_afe_gpio_request(afe->dev, false, MT8169_DAI_ADDA, 0);
		break;
	default:
		break;
	}
...
/* mtkaif dmic */
static const char * const mt8169_adda_off_on_str[] = {
	"Off", "On"
};

static const struct soc_enum mt8169_adda_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt8169_adda_off_on_str),
			    mt8169_adda_off_on_str),
};

static int mt8169_adda_dmic_get(struct snd_kcontrol *kcontrol,
...
static int mt8169_adda_dmic_set(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
#if IS_ENABLED(CONFIG_SND_SOC_FPGA)
static int mt8169_adda_loopback0_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] = afe_priv->mtkaif_looback0;
	return 0;
}

static int mt8169_adda_loopback0_set(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	loopback_on = ucontrol->value.integer.value[0];

	dev_info(afe->dev, "%s(), kcontrol name %s, dmic_on %d\n",
		 __func__, kcontrol->id.name, loopback_on);

	afe_priv->mtkaif_looback0 = loopback_on;
	return 0;
}

static int mt8169_adda_loopback1_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] = afe_priv->mtkaif_looback1;
	return 0;
}

static int mt8169_adda_loopback1_set(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	afe_priv->mtkaif_looback1 = loopback_on;
	return 0;
}
#endif
...
static const struct snd_kcontrol_new mtk_adda_controls[] = {
	...
	SOC_ENUM_EXT("MTKAIF_DMIC", mt8169_adda_enum[0],
		     mt8169_adda_dmic_get, mt8169_adda_dmic_set),
#if IS_ENABLED(CONFIG_SND_SOC_FPGA)
	SOC_ENUM_EXT("MTKAIF_LOOPBACK0", mt8169_adda_enum[0],
		     mt8169_adda_loopback0_get, mt8169_adda_loopback0_set),
	SOC_ENUM_EXT("MTKAIF_LOOPBACK1", mt8169_adda_enum[0],
		     mt8169_adda_loopback1_get, mt8169_adda_loopback1_set),
#endif
};
...
static int mtk_afe_adda_apll_connect(struct snd_soc_dapm_widget *source,
				     struct snd_soc_dapm_widget *sink)
{
	struct snd_soc_dapm_widget *w = source;
	struct snd_soc_component *cmpnt = snd_soc_dapm_to_component(w->dapm);
...
static const struct snd_soc_dapm_route mtk_dai_adda_routes[] = {
	...
	{"ADDA_DL_CH1", "DL3_CH1", "DSP_DL3_VIRT"},
	{"ADDA_DL_CH2", "DL3_CH2", "DSP_DL3_VIRT"},
#endif
	...
	{"top_mux_audio_h", NULL, APLL2_W_NAME, mtk_afe_adda_apll_connect},

	{"aud_dac_hires_clk", NULL, "top_mux_audio_h"},
	{"aud_adc_hires_clk", NULL, "top_mux_audio_h"},

#if IS_ENABLED(CONFIG_SND_SOC_FPGA)
	/* allow adda on without codec on */
...
#if IS_ENABLED(CONFIG_SND_SOC_FPGA)
		/* loopback0 */
		ul_src_con0 |= afe_priv->mtkaif_looback0 << 2 & 1 << 2;
#endif
		switch (id) {
		case MT8169_DAI_ADDA:
		case MT8169_DAI_AP_DMIC:
...
int mt8169_dai_adda_register(struct mtk_base_afe *afe)
{
	...
	/* set dai priv */
	ret = mt8169_dai_set_priv(afe, MT8169_DAI_ADDA,
				  sizeof(struct mtk_afe_adda_priv), NULL);
	if (ret)
		return ret;

	/* ap dmic priv share with adda */
	afe_priv->dai_priv[MT8169_DAI_AP_DMIC] =
		afe_priv->dai_priv[MT8169_DAI_ADDA];
