			    ...
			    int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		mt8169_afe_gpio_request(afe->dev, true, MT8169_DAI_PCM, 0);
		break;
	case SND_SOC_DAPM_POST_PMD:
		mt8169_afe_gpio_request(afe->dev, false, MT8169_DAI_PCM, 0);
		break;
	default:
		break;
	}
				 ...
				 struct snd_soc_dai *dai)
{
	...
	unsigned int rate_reg = mt8169_rate_transform(afe->dev, rate, dai->id);
	...
	if (pcm_id >= MT8169_DAI_NUM || pcm_id < MT8169_MEMIF_DL1) {
		dev_info(afe->dev, "%s(), invalid pcm_id:%d\n", __func__, pcm_id);
		return -EINVAL;
	}

	pcm_priv = afe_priv->dai_priv[pcm_id];
	if (!pcm_priv) {
		dev_info(afe->dev, "%s(), pcm_priv == NULL", __func__);
		return -EINVAL;
	}
...
int mt8169_dai_pcm_register(struct mtk_base_afe *afe)
{
	...
	afe_priv->dai_priv[MT8169_DAI_PCM] = pcm_priv;
