...
static struct gpio_desc *hp_switch_gpio;
...
static inline void clear_accdet_eint(u32 eintid)
{
	...
	pr_info("%s() eint-%s IRQ-STS:[0x%x]=0x%x\n", __func__,
		...
		ACCDET_IRQ_ADDR, accdet_read(ACCDET_IRQ_ADDR));
}
...
static inline void clear_accdet_eint_check(u32 eintid)
{
	...
	pr_info("%s() eint-%s IRQ-STS:[0x%x]=0x%x TOP_INT_STS:[0x%x]:0x%x\n",
		...
		(eintid == PMIC_EINT0)?"0":((eintid == PMIC_EINT1)?"1":"BI"),
		ACCDET_IRQ_ADDR, accdet_read(ACCDET_IRQ_ADDR),
		RG_INT_STATUS_ACCDET_ADDR, accdet_read(RG_INT_STATUS_ACCDET_ADDR));
}
...
static inline void enable_accdet(void)
{
#ifdef CONFIG_ACCDET_OMTP_CTIA_SWITCH_MANUAL
	/* Disable ACCDET comparator to avoid ACCDET IRQ before CTIA/OMTP detected */
	accdet_clear_bit(ACCDET_CMP_PWM_EN_ADDR, ACCDET_CMP_PWM_EN_SFT);
#endif

	/* enable ACCDET unit */
	accdet_update_bit(ACCDET_EN_ADDR, ACCDET_EN_SFT);
	pr_info("%s done IRQ-STS[0x%x]=0x%x,STATE_SWCTRL[0x%x]=0x%x\n",
		__func__, ACCDET_IRQ_ADDR, accdet_read(ACCDET_IRQ_ADDR),
		ACCDET_CMP_PWM_EN_ADDR, accdet_read(ACCDET_CMP_PWM_EN_ADDR));
}
...
/* reverted... In order to adapt the HW, we revert the hp_switch_gpio */
/*control for SD80, too. Currently hp_switch_gpio = 1 means */
/*MICP = RING2 = 4th ring (GND_MIC2_1), CTIA mode. hp_switch_gpio = 0 */
/*means MICP = SLEEVE = 3rd ring (GND_MIC1), OMTP mode. */
	if (plug_in) {
		if (type == TYPE_CTIA) {
			pr_info("[accdet] CTIA headset!!!\n");
			/* Default: CTIA, so not setting again  */
		} else if (type == TYPE_OMTP) {
			pr_info("[accdet] OMTP headset!!!\n");
			if (!IS_ERR(hp_switch_gpio))
				gpiod_set_value_cansleep(hp_switch_gpio, 0);
		} else {
			pr_notice("Error:%s unknown headset type\n", __func__);
		}
	} else {
		/* Default: CTIA:LRGM */
		if (!IS_ERR(hp_switch_gpio))
			gpiod_set_value_cansleep(hp_switch_gpio, 1);
	}
...
static void eint_work_callback(struct work_struct *work)
{
	if (accdet->cur_eint_state == EINT_PLUG_IN) {
		...
		/* after CTIA/OMTP detected,enable ACCDET comparator */
		accdet_update_bit(ACCDET_CMP_PWM_EN_ADDR, ACCDET_CMP_PWM_EN_SFT);
#endif
	} else {
...
static inline void check_cable_type(void)
{
	...
	pr_info("accdet cur cable type:[%s], status switch:[%s]\n",
		accdet_report_str[accdet->cable_type], accdet_status_str[accdet->accdet_status]);
}
...
static int accdet_get_dts_data(struct platform_device *platform_device)
{
	...
	if (platform_device == NULL) {
		pr_notice("%s() *platform_device is NULL\n", __func__);
		return -EINVAL;
	}
	node = of_find_matching_node(node, accdet_of_match);
	if (!node) {
	...
	if (ret < 0) {
		...
		pr_notice("%s: mic voltage delay is not enabled\n", __func__);
	}
	ret = of_property_read_u32(node, "switch-mode-vol",
			 &switch_mode_voltage);
	if (ret < 0) {
		switch_mode_voltage = 521;
		pr_notice("%s: switch mode voltage is not enabled\n", __func__);
	}
	hp_switch_gpio = devm_gpiod_get(&platform_device->dev,
						      "hp-switchs",
						      GPIOD_OUT_HIGH);
	if (IS_ERR(hp_switch_gpio)) {
		ret = PTR_ERR(hp_switch_gpio);
		dev_info(&platform_device->dev, "failed to get hp_switch_gpio: %d\n", ret);
	}
#endif
...
int mt6357_accdet_set_drvdata(struct snd_soc_card *card)
{
	snd_soc_card_set_drvdata(card, accdet);

	return 0;
}
EXPORT_SYMBOL_GPL(mt6357_accdet_set_drvdata);
