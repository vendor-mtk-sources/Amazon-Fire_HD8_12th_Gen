...
//
// mt6357.c  --  mt6357 ALSA SoC audio codec driver
//
...
struct mt6357_priv {
	...
	struct mutex ana_buf_ctrl_mutex;
	int ana_buf_ctrl_ref;
	struct mutex ana_clk_mutex;
	int ana_clk_ref;
	struct mutex mic_bias_mutex;
	int mic_bias_ref;
...
static const struct soc_enum mt8169_audio_ul_enum[] = {
...
EXPORT_SYMBOL_GPL(mt6357_set_mtkaif_protocol);

static void playback_gpio_set(struct mt6357_priv *priv)
{
	/* set gpio mosi mode */
	regmap_update_bits(priv->regmap, MT6357_GPIO_MODE2_CLR,
			   0x01f8, 0x01f8);
	regmap_update_bits(priv->regmap, MT6357_GPIO_MODE2_SET,
			   0xffff, 0x0249);
	regmap_update_bits(priv->regmap, MT6357_GPIO_MODE2,
			   0xffff, 0x0249);
}

static void playback_gpio_reset(struct mt6357_priv *priv)
{
...
static void capture_gpio_set(struct mt6357_priv *priv)
{
	/* set gpio miso mode */
	regmap_update_bits(priv->regmap, MT6357_GPIO_MODE3_CLR,
			   0xffff, 0xffff);
	regmap_update_bits(priv->regmap, MT6357_GPIO_MODE3_SET,
			   0xffff, 0x0249);
	regmap_update_bits(priv->regmap, MT6357_GPIO_MODE3,
			   0xffff, 0x0249);
}

static void capture_gpio_reset(struct mt6357_priv *priv)
{
...
static int mt6357_set_dcxo(struct mt6357_priv *priv, bool enable)
{
	mutex_lock(&priv->ana_buf_ctrl_mutex);
	pr_info("%s, ref:%d, enable:%d\n", __func__, priv->ana_buf_ctrl_ref, enable);
	if (enable) {
		if (priv->ana_buf_ctrl_ref++ == 0)
		...
		if (priv->ana_buf_ctrl_ref < 0) {
			pr_info("audclk_buf_ref < 0\n");
			priv->ana_buf_ctrl_ref = 0;
		}
	}
	mutex_unlock(&priv->ana_buf_ctrl_mutex);

	return 0;
}

/* use only when not govern by DAPM */
static int mt6357_set_clksq(struct mt6357_priv *priv, bool enable)
{
	/* audio clk source from internal dcxo */
	regmap_update_bits(priv->regmap, MT6357_AUDENC_ANA_CON6,
...
/* use only when not govern by DAPM */
static int mt6357_set_aud_global_bias(struct mt6357_priv *priv, bool enable)
{

	mutex_lock(&priv->ana_clk_mutex);
	pr_info("%s, ref:%d, enable:%d\n", __func__, priv->ana_clk_ref, enable);
	if (enable) {
		if (priv->ana_clk_ref++ == 0)
		...
		if (priv->ana_clk_ref < 0) {
			pr_info("ana_clk_ref < 0\n");
			priv->ana_clk_ref = 0;
		}
	}
	mutex_unlock(&priv->ana_clk_mutex);

	return 0;
}

static int mt6357_set_micbias0(struct mt6357_priv *priv, bool enable)
{
	mutex_lock(&priv->mic_bias_mutex);
	pr_info("%s, ref:%d, enable:%d\n", __func__, priv->mic_bias_ref, enable);
	if (enable) {
		if (priv->mic_bias_ref++ == 0)
		...
		if (priv->mic_bias_ref < 0) {
			pr_info("mic_bias_ref < 0\n");
			priv->mic_bias_ref = 0;
		}
	}
	mutex_unlock(&priv->mic_bias_mutex);

	return 0;
}

/* use only when not govern by DAPM */
static int mt6357_set_topck(struct mt6357_priv *priv, bool enable)
{
	regmap_update_bits(priv->regmap, MT6357_AUD_TOP_CKPDN_CON0,
			   0x0066, enable ? 0x0 : 0x66);
	return 0;
}

static int mt6357_mtkaif_tx_enable(struct mt6357_priv *priv)
{
	switch (priv->mtkaif_protocol) {
	case MT6357_MTKAIF_PROTOCOL_2_CLK_P2:
...
static int mt6357_mtkaif_tx_disable(struct mt6357_priv *priv)
{
...
int mt6357_mtkaif_calibration_enable(struct snd_soc_component *cmpnt)
{
	...
	mt6357_mtkaif_tx_enable(priv);

	mt6357_set_dcxo(priv, true);
	mt6357_set_aud_global_bias(priv, true);
	mt6357_set_clksq(priv, true);
	mt6357_set_topck(priv, true);

	/* set dat_miso_loopback on */
	regmap_update_bits(priv->regmap, MT6357_AUDIO_DIG_CFG,
...
EXPORT_SYMBOL_GPL(mt6357_mtkaif_calibration_enable);
...
int mt6357_mtkaif_calibration_disable(struct snd_soc_component *cmpnt)
{
	...
	mt6357_set_topck(priv, false);
	mt6357_set_clksq(priv, false);
	mt6357_set_aud_global_bias(priv, false);
	mt6357_set_dcxo(priv, false);

	mt6357_mtkaif_tx_disable(priv);
	playback_gpio_reset(priv);
	capture_gpio_reset(priv);
	return 0;
}
EXPORT_SYMBOL_GPL(mt6357_mtkaif_calibration_disable);
...
EXPORT_SYMBOL_GPL(mt6357_set_mtkaif_calibration_phase);
...
#define DL_GAIN_REG_MASK (RG_AUDHPLGAIN_MASK_SFT | RG_AUDHPLGAIN_MASK_SFT)

static void Zcd_Enable(struct mt6357_priv *priv, bool _enable, int device)
{
	if (_enable) {
		switch (device) {
		case AUDIO_ANALOG_DEVICE_OUT_EARPIECEL:
		case AUDIO_ANALOG_DEVICE_OUT_EARPIECER:
			regmap_update_bits(priv->regmap, MT6357_AUDDEC_ANA_CON8,
					   0x7, 0x2);
			break;
		...
		regmap_update_bits(priv->regmap, MT6357_ZCD_CON0,
				   0x7 << 8, 0x1 << 8);
		regmap_update_bits(priv->regmap, MT6357_ZCD_CON0,
				   0x1 << 7, 0x0 << 7);
...
static void hp_pull_down(struct mt6357_priv *priv, bool enable)
{
	if (enable)
		regmap_update_bits(priv->regmap, MT6357_AUDDEC_ANA_CON2,
				   ...
				   1 << RG_HPPSHORT2VCM_VAUDP15_SFT);
	else
		regmap_update_bits(priv->regmap, MT6357_AUDDEC_ANA_CON2,
				   RG_HPPSHORT2VCM_VAUDP15_MASK_SFT,
				   0 << RG_HPPSHORT2VCM_VAUDP15_SFT);
}
...
static void headset_volume_ramp(struct mt6357_priv *priv, int from_l, int from_r, int to_l, int to_r)
{
	int offset_l = 0, offset_r = 0, count_l = 0, count_r = 0, reg_idx_l, reg_idx_r;

	if (!is_valid_hp_pga_idx(from_l) ||
	    !is_valid_hp_pga_idx(from_r) ||
	    !is_valid_hp_pga_idx(to_l) ||
	    !is_valid_hp_pga_idx(to_r)) {
		dev_info(priv->dev,
			"%s(), volume index is not valid, from_l %d, from_r %d to_l %d, to_r:%d\n",
			 __func__, from_l, from_r, to_l, to_r);
		return;
	}

	dev_info(priv->dev, "%s(), from_l %d, from_r %d, to_l %d, to_r:%d\n",
		 __func__, from_l, from_r, to_l, to_r);
	...
	while (offset_l >= 0 || offset_r >= 0) {
		...
		if (is_valid_hp_pga_idx(reg_idx_l) && offset_l >= 0) {
			regmap_update_bits(priv->regmap,
					   MT6357_ZCD_CON2,
		...
		if (is_valid_hp_pga_idx(reg_idx_r) && offset_r >= 0) {
			regmap_update_bits(priv->regmap,
					   MT6357_ZCD_CON2,
...
static void set_speaker_gain(struct mt6357_priv *priv, int spk_gain)
{
...
static int mt6357_put_volsw(struct snd_kcontrol *kcontrol,
...
static int mt6357_put_volsw_spk(struct snd_kcontrol *kcontrol,
			    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
			snd_soc_kcontrol_component(kcontrol);
	struct soc_mixer_control *mc =
			(struct soc_mixer_control *)kcontrol->private_value;
	struct mt8169_raspite_priv *card_priv =
			snd_soc_card_get_drvdata(component->card);
	int ret;

	if (mc->reg == MT6357_ZCD_CON2) {
		if (component->card->instantiated) {
			/* check spk l/r codec gain is vaild and the left and right are the same */
			if ((card_priv->spk_codec_gain_left != -1) && (card_priv->spk_codec_gain_right != -1) &&
				(card_priv->spk_codec_gain_left == card_priv->spk_codec_gain_right)) {
				ucontrol->value.integer.value[0] = card_priv->spk_codec_gain_left;
				ucontrol->value.integer.value[1] = card_priv->spk_codec_gain_right;
				pr_debug("%s apply spk codec gain from dts\n", __func__);
			} else {
				pr_debug("%s apply spk codec gain from xml\n", __func__);
			}
		}
#if IS_ENABLED(CONFIG_IDME)
	...
	ret = mt6357_put_volsw(kcontrol,ucontrol);
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0]) {
		if (audio_micbias0_on) {
			...
			mt6357_set_dcxo(priv, true);
			mt6357_set_aud_global_bias(priv, true);
			mt6357_set_micbias0(priv, true);
		}
	} else {
		if (audio_micbias0_on) {
			mt6357_set_micbias0(priv, false);
			mt6357_set_aud_global_bias(priv, false);
			mt6357_set_dcxo(priv, false);
		}
	}
...
static const struct snd_kcontrol_new mt6357_snd_controls[] = {
	...
	SOC_DOUBLE_EXT_TLV("Lineout Volume",
			   MT6357_ZCD_CON1, 0, 7, 0x1f, 0,
			   snd_soc_get_volsw, mt6357_put_volsw, playback_tlv),
	SOC_SINGLE_EXT_TLV("Handset Volume",
			   MT6357_ZCD_CON3, 0, 0x1f, 0,
			   snd_soc_get_volsw, mt6357_put_volsw, playback_tlv),
	SOC_DOUBLE_EXT_TLV("Headphone_Volume_Tuning",
			   MT6357_ZCD_CON2, 0, 7, 0x1f, 0,
			   snd_soc_get_volsw, mt6357_put_volsw, playback_tlv),
	SOC_DOUBLE_EXT_TLV("Headphone_Volume_Cust",
			   MT6357_ZCD_CON2, 0, 7, 0x1f, 0,
			   snd_soc_get_volsw, mt6357_put_volsw, playback_tlv),
	SOC_DOUBLE_EXT_TLV("Speaker_Volume_Cust",
			   MT6357_ZCD_CON2, 0, 7, 0x1f, 0,
			   snd_soc_get_volsw, mt6357_put_volsw_spk, playback_tlv),
	/* ul pga gain */
	SOC_DOUBLE_R_EXT_TLV("PGA Volume",
			     MT6357_AUDENC_ANA_CON0, MT6357_AUDENC_ANA_CON1,
			     8, 4, 0,
			     snd_soc_get_volsw, mt6357_put_volsw, pga_tlv),
	SOC_ENUM_EXT("Audio_MICBIAS0_Switch", mt8169_audio_ul_enum[0],
			    Audio_MICBIAS0_Get,
			    Audio_MICBIAS0_Set),
};
...
static int mt_clkbuf_event(struct snd_soc_dapm_widget *w,
...
static int mt_audglb_event(struct snd_soc_dapm_widget *w,
			  struct snd_kcontrol *kcontrol,
			  int event)
{
	struct snd_soc_component *cmpnt = snd_soc_dapm_to_component(w->dapm);
			 ...
			 int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		regmap_write(priv->regmap, MT6357_AFUNC_AUD_CON2, 0x0006);
		/* scrambler clock on enable */
		regmap_write(priv->regmap, MT6357_AFUNC_AUD_CON0, 0xCBA1);
		/* sdm power on */
		regmap_write(priv->regmap, MT6357_AFUNC_AUD_CON2, 0x0003);
		/* sdm fifo enable */
		regmap_write(priv->regmap, MT6357_AFUNC_AUD_CON2, 0x000B);

		regmap_update_bits(priv->regmap, MT6357_AFE_SGEN_CFG0,
				   0xff3f,
				   0x0000);
		regmap_update_bits(priv->regmap, MT6357_AFE_SGEN_CFG1,
				   0xffff,
				   0x0001);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* DL scrambler disabling sequence */
		regmap_write(priv->regmap, MT6357_AFUNC_AUD_CON2, 0x0000);
		regmap_write(priv->regmap, MT6357_AFUNC_AUD_CON0, 0xcba0);
		break;
	default:
		break;
	}
...
static int mtk_hp_enable(struct mt6357_priv *priv)
{
	/* Enable HP main CMFB Switch */
	regmap_update_bits(priv->regmap, MT6357_AUDDEC_ANA_CON6,
			   0x0fff, 0x0280);
	/* Pull-down HPL/R, HS, LO to AVSS28_AUD */
	regmap_update_bits(priv->regmap, MT6357_AUDDEC_ANA_CON7,
			   0xffff, 0x00a8);
	...
	regmap_write(priv->regmap, MT6357_AUDNCP_CLKDIV_CON1, 0x0001);
	/* Set NCP clock as 604kHz // 26MHz/43 = 604KHz */
	regmap_write(priv->regmap, MT6357_AUDNCP_CLKDIV_CON2, 0x002c);
	/* Toggle RG_DIVCKS_CHG */
	regmap_write(priv->regmap, MT6357_AUDNCP_CLKDIV_CON0, 0x0001);
	/* Set NCP soft start mode as default mode: 100us */
	regmap_write(priv->regmap, MT6357_AUDNCP_CLKDIV_CON4, 0x0002);
	/* Enable NCP */
	regmap_write(priv->regmap, MT6357_AUDNCP_CLKDIV_CON3, 0x0000);
	...
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON13, 0x0001);
	...
	/* Enable AUD_ZCD */
	Zcd_Enable(priv, true, AUDIO_ANALOG_DEVICE_OUT_HEADSETL);

	/* Enable IBIST */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON10, 0x0055);

	/* Set HP DR bias current optimization, 010: 6uA */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON9, 0x0092);
	...
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON1, 0x000c);
	/* Enable HP aux feedback loop */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON1, 0x003c);
	/* Enable HP aux CMFB loop */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON6, 0x0c80);
	/* Enable HP driver bias circuits */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON0, 0x30c0);
	/* Enable HP driver core circuits */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON0, 0x30f0);
	/* Short HP main output to HP aux output stage */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON1, 0x00fc);

	/* Enable HP main CMFB loop */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON6, 0x0e80);
	/* Disable HP aux CMFB loop */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON6, 0x0280);

	/* Enable HP main output stage */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON1, 0x00ff);
	...
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON1, 0x77cf);
	...
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON1, 0x77c3);
	/* Unshort HP main output to HP aux output stage */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON1, 0x7703);
	usleep_range(100, 120);
	...
	regmap_update_bits(priv->regmap, MT6357_AUDDEC_ANA_CON11,
			   0x1, 0x1);
	/* Enable Audio DAC  */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON0, 0x30ff);
	/* Enable low-noise mode of DAC */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON6, 0xf281);
	usleep_range(100, 120);

	/* Switch HPL MUX to audio DAC */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON0, 0x32ff);
	/* Switch HPR MUX to audio DAC */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON0, 0x3aff);
...
static int mtk_hp_disable(struct mt6357_priv *priv)
{
	/* Pull-down HPL/R to AVSS28_AUD */
	hp_pull_down(priv, true);
	...
	/* Enable HP aux feedback loop */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON1, 0x77ff);
	...
	regmap_update_bits(priv->regmap, MT6357_AUDDEC_ANA_CON1, 0x3, 0x0);

	/* Enable HP aux CMFB loop */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON6, 0x0e00);

	/* Disable HP main CMFB loop */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON6, 0x0c00);
	...
	/* Disable AUD_ZCD */
	Zcd_Enable(priv, false, AUDIO_ANALOG_DEVICE_OUT_HEADSETL);
...
static int mtk_hp_spk_enable(struct mt6357_priv *priv)
{
	/* Enable HP main CMFB Switch */
	regmap_update_bits(priv->regmap, MT6357_AUDDEC_ANA_CON6,
			   0x0fff, 0x0280);
	...
	/* Enable AUD_ZCD */
	Zcd_Enable(priv, true, AUDIO_ANALOG_DEVICE_OUT_SPEAKER_HEADSET_L);
	...
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON4, 0x0110);
	/* Enable LO driver bias circuits */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON4, 0x0112);
	/* Enable LO driver core circuits */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON4, 0x0113);
	...
	/* Enable AUD_CLK */
	regmap_update_bits(priv->regmap, MT6357_AUDDEC_ANA_CON11, 0x1, 0x1);
	/* Enable Audio DAC  */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON0, 0x30f9);
	/* Enable low-noise mode of DAC */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON6, 0x0281);
	/* Switch LOL MUX to audio DAC */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON4, 0x011b);
	/* Switch HPL MUX to Line-out */
	/* Switch HPR MUX to DAC */
	regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON0, 0x39ff);
...
static int mtk_hp_spk_disable(struct mt6357_priv *priv)
{
	...
	/* Disable AUD_ZCD */
	Zcd_Enable(priv, false, AUDIO_ANALOG_DEVICE_OUT_SPEAKER_HEADSET_L);
			...
			int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		/* Enable AUD_ZCD */
		Zcd_Enable(priv, true, AUDIO_ANALOG_DEVICE_OUT_SPEAKERL);
		...
		/* Enable Audio DAC  */
		regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON0, 0x3009);

		/* Enable low-noise mode of DAC */
		regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON6, 0x0201);
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		regmap_write(priv->regmap, MT6357_ZCD_CON1,
			     DL_GAIN_N_40DB_REG);
		...
		/* Disable AUD_ZCD */
		Zcd_Enable(priv, false, AUDIO_ANALOG_DEVICE_OUT_SPEAKERL);
			...
			int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		regmap_write(priv->regmap, MT6357_AUDNCP_CLKDIV_CON0, 0x0001);
		/* Set NCP soft start mode as default mode: 100us */
		regmap_write(priv->regmap, MT6357_AUDNCP_CLKDIV_CON4, 0x0003);
		/* Enable NCP */
		regmap_write(priv->regmap, MT6357_AUDNCP_CLKDIV_CON3, 0x0000);
		usleep_range(250, 270);
		...
		/* Enable AUD_ZCD */
		Zcd_Enable(priv, true, AUDIO_ANALOG_DEVICE_OUT_EARPIECEL);
		...
		regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON3, 0x0090);

		/* Enable HS driver bias circuits */
		regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON3, 0x0092);

		/* Enable HS driver core circuits */
		regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON3, 0x0093);

		/* Set HS gain to normal gain step by step */
		regmap_write(priv->regmap, MT6357_ZCD_CON3,
			     priv->ana_gain[AUDIO_ANALOG_VOLUME_HSOUTL]);
		...
		/* Enable Audio DAC  */
		regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON0, 0x0009);
		/* Enable low-noise mode of DAC */
		regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON6, 0x0001);
		/* Switch HS MUX to audio DAC */
		regmap_write(priv->regmap, MT6357_AUDDEC_ANA_CON3, 0x009b);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		/* HS mux to open */
		...
		/* Disable AUD_ZCD */
		Zcd_Enable(priv, false, AUDIO_ANALOG_DEVICE_OUT_EARPIECEL);
			       ...
			       int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		regmap_write(priv->regmap, MT6357_AUDENC_ANA_CON3,
			     0x0000);
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		/* ADC CLK from CLKGEN (13MHz) */
		regmap_write(priv->regmap, MT6357_AUDENC_ANA_CON3, 0x0000);
...
static int mt6357_amic_enable(struct mt6357_priv *priv)
{
	...
	if (IS_DCC_BASE(mic_type)) {
		...
		regmap_write(priv->regmap, MT6357_AFE_DCCLK_CFG0, 0x2062);
		regmap_write(priv->regmap, MT6357_AFE_DCCLK_CFG0, 0x2062);
		regmap_write(priv->regmap, MT6357_AFE_DCCLK_CFG0, 0x2060);
		regmap_write(priv->regmap, MT6357_AFE_DCCLK_CFG0, 0x2061);
		regmap_write(priv->regmap, MT6357_AFE_DCCLK_CFG1, 0x0100);
	}
	...
	if (mux_pga_l == PGA_MUX_AIN1 || mux_pga_r == PGA_MUX_AIN1) {
		...
		if (mic_type == MIC_TYPE_MUX_DCC_ECM_SINGLE)
			regmap_write(priv->regmap,
				     MT6357_AUDENC_ANA_CON9, 0x0161);
		else
			regmap_write(priv->regmap,
				     MT6357_AUDENC_ANA_CON9, 0x0061);
	}
	...
	if (IS_DCC_BASE(mic_type)) {
		...
		regmap_update_bits(priv->regmap, MT6357_AUDENC_ANA_CON3,
				   0x1 << 12, 0x0);
	}
	...
	/* UL dmic setting off */
	regmap_write(priv->regmap, MT6357_AFE_UL_SRC_CON0_H, 0x0000);

	/* UL turn on */
	regmap_write(priv->regmap, MT6357_AFE_UL_SRC_CON0_L, 0x0001);

	/* Prevent pop noise from amic hw */
	msleep(100);

	return 0;
}

static void mt6357_amic_disable(struct mt6357_priv *priv)
{
	unsigned int mic_type = priv->mux_select[MUX_MIC_TYPE];
	unsigned int mux_pga_l = priv->mux_select[MUX_PGA_L];
	unsigned int mux_pga_r = priv->mux_select[MUX_PGA_R];
	...
	regmap_update_bits(priv->regmap, MT6357_AUDENC_ANA_CON8, 0xff00, 0x0000);
	mt6357_set_micbias0(priv, false);
...
static int mt6357_dmic_enable(struct mt6357_priv *priv)
{
	dev_info(priv->dev, "%s()\n", __func__);
	...
	/* DMIC enable */
	regmap_write(priv->regmap, MT6357_AUDENC_ANA_CON7, 0x0005);

	/* here to set digital part */
	mt6357_mtkaif_tx_enable(priv);

	/* UL dmic setting */
	regmap_write(priv->regmap, MT6357_AFE_UL_SRC_CON0_H, 0x0080);

	/* UL turn on */
	regmap_write(priv->regmap, MT6357_AFE_UL_SRC_CON0_L, 0x0003);
...
static void mt6357_dmic_disable(struct mt6357_priv *priv)
{
	dev_info(priv->dev, "%s()\n", __func__);
	...
	/* DMIC disable */
	regmap_write(priv->regmap, MT6357_AUDENC_ANA_CON7, 0x0000);

	/* mic bias */
	/* MISBIAS0 = 1P7V */
	regmap_write(priv->regmap, MT6357_AUDENC_ANA_CON8, 0x0001);
...
static void mt6357_restore_pga(struct mt6357_priv *priv)
{
			     ...
			     int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		case MIC_TYPE_MUX_DMIC:
			mt6357_dmic_enable(priv);
			break;
		default:
			mt6357_amic_enable(priv);
			break;
		}
		mt6357_restore_pga(priv);

		break;
	case SND_SOC_DAPM_POST_PMD:
		switch (priv->mux_select[MUX_MIC_TYPE]) {
		case MIC_TYPE_MUX_DMIC:
			mt6357_dmic_disable(priv);
			break;
		default:
			mt6357_amic_disable(priv);
			break;
		}
...
static const struct snd_soc_dapm_widget mt6357_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SUPPLY("LO Stability Enh", MT6357_AUDDEC_ANA_CON4,
			    RG_LOOUTPUTSTBENH_VAUDP15_SFT, 0, NULL, 0),

	SND_SOC_DAPM_OUT_DRV("LOL Buffer", MT6357_AUDDEC_ANA_CON4,
	...
	/* SGEN */
	SND_SOC_DAPM_SUPPLY("SGEN DL Enable", MT6357_AFE_SGEN_CFG0,
			    SGEN_DAC_EN_CTL_SFT, 0, NULL, 0),
	SND_SOC_DAPM_SUPPLY("SGEN MUTE", MT6357_AFE_SGEN_CFG0,
			    SGEN_MUTE_SW_CTL_SFT, 1,
			    mt_sgen_event,
			    SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_SUPPLY("SGEN DL SRC", MT6357_AFE_DL_SRC2_CON0_L,
...
static int mt6357_codec_dai_hw_params(struct snd_pcm_substream *substream,
...
static const struct snd_soc_dai_ops mt6357_codec_dai_ops = {
	.hw_params = mt6357_codec_dai_hw_params,
};

#define MT6357_FORMATS (SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S16_BE |\
...
static struct snd_soc_dai_driver mt6357_dai_driver[] = {
	{
		...
		.ops = &mt6357_codec_dai_ops,
	},
};

static void mt6357_codec_init_reg(struct mt6357_priv *priv)
{
	...
	regmap_update_bits(priv->regmap, MT6357_AUDENC_ANA_CON10,
			   RG_EINTCOMPVTH_MASK_SFT,
			   0x1 << RG_EINTCOMPVTH_SFT);

	/* gpio miso driving set to 4mA */
	regmap_write(priv->regmap, MT6357_DRV_CON3, 0x8888);
...
static int mt6357_codec_probe(struct snd_soc_component *cmpnt)
{
	struct mt6357_priv *priv = snd_soc_component_get_drvdata(cmpnt);
	int ret;
	...
	mt6357_codec_init_reg(priv);

	priv->ana_gain[AUDIO_ANALOG_VOLUME_HPOUTL] = DL_GAIN_0DB;
	priv->ana_gain[AUDIO_ANALOG_VOLUME_HPOUTR] = DL_GAIN_0DB;
	priv->ana_gain[AUDIO_ANALOG_VOLUME_LINEOUTL] = DL_GAIN_0DB;
	priv->ana_gain[AUDIO_ANALOG_VOLUME_LINEOUTR] = DL_GAIN_0DB;
	priv->ana_gain[AUDIO_ANALOG_VOLUME_MICAMP1] = UL_GAIN_18DB;
	priv->ana_gain[AUDIO_ANALOG_VOLUME_MICAMP2] = UL_GAIN_18DB;

	priv->avdd_reg = devm_regulator_get(priv->dev, "Avdd");
	if (IS_ERR(priv->avdd_reg)) {
		dev_info(priv->dev, "%s() have no Avdd supply", __func__);
		return PTR_ERR(priv->avdd_reg);
	}

	dev_info(priv->dev, "%s() Add supply successful!", __func__);
	...
	mutex_init(&priv->ana_buf_ctrl_mutex);
	mutex_init(&priv->ana_clk_mutex);
	mutex_init(&priv->mic_bias_mutex);
	return 0;
}

static void mt6357_codec_remove(struct snd_soc_component *cmpnt)
{
	struct mt6357_priv *priv = snd_soc_component_get_drvdata(cmpnt);

	mutex_destroy(&priv->ana_buf_ctrl_mutex);
	mutex_destroy(&priv->ana_clk_mutex);
	mutex_destroy(&priv->mic_bias_mutex);
}
...
static const struct snd_soc_component_driver mt6357_soc_component_driver = {
	...
	.controls = mt6357_snd_controls,
	.num_controls = ARRAY_SIZE(mt6357_snd_controls),
...
static int mt6357_platform_driver_probe(struct platform_device *pdev)
{
	...
	dev_info(priv->dev, "%s(), dev name %s version1\n",
		 __func__, dev_name(&pdev->dev));

	return devm_snd_soc_register_component(&pdev->dev,
				      &mt6357_soc_component_driver,
				      mt6357_dai_driver,
				      ARRAY_SIZE(mt6357_dai_driver));
}
