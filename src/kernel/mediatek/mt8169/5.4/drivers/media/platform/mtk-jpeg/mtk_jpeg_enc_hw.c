...
static void mtk_jpeg_enc_set_quality(void __iomem *base, u32 quality)
{
	...
	if (quality > mtk_jpeg_enc_quality[ARRAY_SIZE(mtk_jpeg_enc_quality)-1].hardware_value)
		enc_quality =
		mtk_jpeg_enc_quality[ARRAY_SIZE(mtk_jpeg_enc_quality)-1].hardware_value;
...
void mtk_jpeg_enc_reg_dump(void __iomem *base)
{
	int i = JPEG_ENC_RSTB;
	unsigned long value;

	while (i <= JPEG_ENC_IMG_STRIDE) {
		value = readl(base + i);
		pr_info("base[%d],reg:0x%x = 0x%x",
		    base, (base + i), value);
		i= i + 0x4;
	}
}
