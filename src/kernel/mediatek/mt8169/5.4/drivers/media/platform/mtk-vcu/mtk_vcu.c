...
#define VCU_SHMEM_SIZE 0x80000L
...
inline unsigned int ipi_id_to_inst_id(int vcuid, int id)
{
	/* Assume VENC uses instance 1 and others use 0. */
	if ((id < IPI_VENC_COMMON && id >= IPI_VCU_INIT) || id == IPI_MDP)
		return VCU_VDEC;
	else
		return VCU_VENC;
}
		     ...
		     const char *name, void *priv)
{
	...
	if (vcu == NULL) {
		dev_info(&pdev->dev, "vcu device in not ready %p\n", vcu);
		return -EPROBE_DEFER;
	}
		 ...
		 unsigned int len, void *priv)
{
	...
	if (vcu_running(vcu) == false) {
		dev_info(&pdev->dev, "[VCU] %s: VCU is not running,signaled:%d, vcu:%p\n", __func__, (bool)vcu->run.signaled, vcu);
		return -EPERM;
	}
	...
	if (vcu->abort) {
		if (vcu->open_cnt > 0) {
			...
			ret = down_interruptible(&vcu->vpud_killed);
		}
	 ...
	 */
	while (atomic_read(&vcu->ipi_got[i]) == 1)
		cond_resched();
end:
	return ret;
}
...
static int vcu_ipi_get(struct mtk_vcu *vcu, unsigned long arg)
{
	...
	mutex_unlock(&vcu->vcu_buf_mutex);
...
static int vcu_gce_get_inst_id(struct mtk_vcu *vcu, u64 gce_handle)
{
	int i, temp;
	...
	for (i = 0; i < VCODEC_INST_MAX; i++) {
		if (vcu->gce_info[i].user_hdl == gce_handle) {
			temp = atomic_read(&vcu->gce_info[i].flush_done);
			mutex_unlock(&vcu->vcu_share);
...
static void vcu_gce_clear_inst_id(struct mtk_vcu *vcu, void *ctx)
{
	...
	for (i = 0; i < VCODEC_INST_MAX; i++) {
		if (vcu->gce_info[i].v4l2_ctx == ctx) {
			...
			temp2 = atomic_read(&vcu->gce_info[i].flush_done);
...
static void vcu_gce_flush_callback(struct cmdq_cb_data data)
{
	...
	vcu = buff->vcu_ptr;
	j = vcu_gce_get_inst_id(vcu, buff->cmdq_buff.gce_handle);
	...
	if (atomic_dec_and_test(&vcu->gce_job_cnt[i][core_id]) &&
		...
		if (i == VCU_VENC && vcu->cbf.enc_unlock != NULL)
	...
	struct mtk_vcu_queue *q, unsigned long arg)
{
	...
	user_data_addr = (unsigned char *)
				   compat_ptr((uint32_t)buff.cmdq_buff.cmds_user_ptr);
#else
	user_data_addr = (unsigned char *)
				   (uint32_t)buff.cmdq_buff.cmds_user_ptr;
#endif
	...
	if (j < 0)
		j = vcu_gce_set_inst_id(vcu, vcu->curr_ctx[i],
	...
	if (buff.cmdq_buff.codec_type == VCU_VENC &&
		...
		do {
			if (vcu->cbf.enc_lock != NULL) {
				vcu->cbf.enc_lock(vcu->gce_info[j].v4l2_ctx, core_id, &lock);
				if (lock != 0) {
					mutex_unlock(&vcu->vcu_gce_mutex[i]);
	...
	pkt_ptr->err_cb.data = (void *)&vcu->gce_info[j].buff[i];
	...
	cmdq_pkt_flush_threaded(pkt_ptr,
		vcu_gce_flush_callback, (void *)&vcu->gce_info[j].buff[i]);

	atomic_inc(&vcu->gce_info[j].flush_pending);
	time_check_end(100, strlen(vcodec_param_string));
...
static int vcu_wait_gce_callback(struct mtk_vcu *vcu, unsigned long arg)
{
	 ...
	 */
	j = vcu_gce_get_inst_id(vcu, obj.gce_handle);
...
int vcu_get_sig_lock(unsigned long *flags, u32 vcuid)
{
	struct mtk_vcu *vcu;

	if (vcuid >= VCU_ID_MAX) {
		pr_info("[VCU] invalid vcu id\n",
			__func__, vcuid);
		return -1;
	}
	...
	return spin_trylock_irqsave(&vcu->vpud_sig_lock, *flags);
}
EXPORT_SYMBOL_GPL(vcu_get_sig_lock);

void vcu_put_sig_lock(unsigned long flags, u32 vcuid)
{
	struct mtk_vcu *vcu;
...
int vcu_check_vpud_alive(u32 vcuid)
{
	struct mtk_vcu *vcu;
	...
	return (vcu->vpud_is_going_down > 0) ? 0:1;
}
...
static int vcu_init_ipi_handler(void *data, unsigned int len, void *priv)
{
	...
	if (vcu->run.signaled == 1u && run->signaled == 0u) {
		 ...
		 */
		pr_info("run signaled is 1, signaled is 0, vcu:%p\n", vcu);
		vcu->abort = true;
		for (i = 0; i < IPI_MAX; i++)
		...
		dev_info(vcu->dev, "[VCU] vpud killing, init\n");

		return 0;
	}

	dev_info(vcu->dev, "[VCU] run.signaled %d, signaled:%d, vcu:%p\n", (bool)vcu->run.signaled, run->signaled, vcu);
	...
	dev_info(vcu->dev, "[VCU] fw ver: %s\n", vcu->run.fw_ver);
	dev_info(vcu->dev, "[VCU] dec cap: %x\n", vcu->run.dec_capability);
	dev_info(vcu->dev, "[VCU] enc cap: %x\n", vcu->run.enc_capability);
...
static int mtk_vcu_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	uint64_t pa_start = vma->vm_pgoff << PAGE_SHIFT;
	uint64_t pa_start_base = pa_start;
	uint64_t pa_end = pa_start + length;
#if IS_ENABLED(CONFIG_MTK_IOMMU)
	...
	switch (vcu_queue->map_type) {
	case VAL_MEM_TYPE_FOR_HW_CACHEABLE:
		pa_start += MAP_SHMEM_MM_CACHEABLE_BASE;
		pa_start_base += MAP_SHMEM_MM_CACHEABLE_BASE;
		vcu_dbg_log("[VCU] line:%d pa_start 0x%llx, vm_pgoff 0x%lx\n",
			 __LINE__, pa_start, vma->vm_pgoff);
		break;
	case VAL_MEM_TYPE_FOR_HW_NONCACHEABLE:
		pa_start += MAP_SHMEM_MM_BASE;
		pa_start_base += MAP_SHMEM_MM_BASE;
		vcu_dbg_log("[VCU] line:%d pa_start 0x%llx, vm_pgoff 0x%lx\n",
			 __LINE__, pa_start, vma->vm_pgoff);
		break;
	default:
		//pa_start = vma->vm_pgoff;
		vcu_dbg_log("[VCU] line:%d pa_start 0x%llx, vm_pgoff 0x%lx\n",
			 __LINE__, pa_start, vma->vm_pgoff);
		break;
	}
	...
	if (vcu_queue->map_buf != 1) {
		if (pa_start >= MAP_SHMEM_ALLOC_BASE && pa_end <= MAP_SHMEM_ALLOC_END) {
			vma->vm_pgoff =
				(uint64_t)(VCU_DMEM0_PHY(vcu_dev) >> PAGE_SHIFT);
		goto valid_map;
		}
	...
	if (pa_start_base >= MAP_SHMEM_MM_BASE &&
		...
		mem_buff_data.iova = (vcu_dev->iommu_padding) ?
				   ...
				   unsigned long arg)
{
	...
	case VCU_SET_MMAP_TYPE:
		...
		pr_debug("vcu set map_type:%d, map_buf:%d\n",
				vcu_queue->map_type, mem_map_obj.map_buf);
		break;
	case VCU_GET_OBJECT:
...
static int mtk_vcu_probe(struct platform_device *pdev)
{
	...
	dev_info(dev, "[VCU] initialization completed, %p\n",vcu);
	return 0;
...
static const struct of_device_id mtk_vcu_match[] = {
	...
	{.compatible = "mediatek,mt8169-vcu",},
	{},
};
