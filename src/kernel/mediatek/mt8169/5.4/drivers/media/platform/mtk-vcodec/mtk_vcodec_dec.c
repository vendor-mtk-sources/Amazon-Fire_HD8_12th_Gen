...
void mtk_vdec_do_gettimeofday(struct timeval *tv)
{
...
#if IS_ENABLED(CONFIG_VB2_MEDIATEK_DMA_SG)
...
static struct dma_buf *vdec_dc_get_dmabuf(void *buf_priv, unsigned long flags)
{
	return vb2_dma_contig_memops.get_dmabuf(buf_priv, flags);
}

static int vidioc_vdec_reqbufs(struct file *file, void *priv,
							struct v4l2_requestbuffers *rb)
{
	enum vb2_memory memory = rb->memory;
	struct mtk_vcodec_ctx *ctx = fh_to_ctx(priv);
	...
	dst_vq = v4l2_m2m_get_vq(ctx->m2m_ctx,
		V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE);
	mtk_v4l2_debug(4, "%s, memory type: %d", __func__, memory);
	if (memory == VB2_MEMORY_MMAP) {
		...
		vdec_ion_dma_contig_memops.get_dmabuf = vdec_dc_get_dmabuf;
...
#elif IS_ENABLED(CONFIG_VB2_MEDIATEK_DMA_CONTIG)
	vdec_ion_dma_contig_memops = mtk_dma_contig_memops;
	vdec_ion_dma_contig_memops.get_dmabuf = vdec_dc_get_dmabuf;
	dst_vq->mem_ops         = &vdec_ion_dma_contig_memops;
