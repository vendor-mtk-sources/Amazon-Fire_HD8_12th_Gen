...
void mtk_venc_unlock(struct mtk_vcodec_ctx *ctx, unsigned int hw_id)
{
...
unsigned int mtk_venc_lock(struct mtk_vcodec_ctx *ctx, unsigned int hw_id)
{
	unsigned int suspend_block_cnt = 0;
	unsigned int ret = 1;
	enum venc_lock_type lock = VENC_LOCK_NONE;
