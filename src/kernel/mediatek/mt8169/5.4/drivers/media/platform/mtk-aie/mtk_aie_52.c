...
{
	mutex_lock(&fd->u_lock);
	memcpy(user_init, &fd->u_init, sizeof(struct user_init));
	mutex_unlock(&fd->u_lock);
}
...
static int mtk_aie_set_ctrl(struct v4l2_ctrl *ctrl)
{
	struct mtk_aie_ctx *ctx = container_of(ctrl->handler, struct mtk_aie_ctx, hdl);
	struct device *dev = ctx->dev;
	int ret = 0;

	mutex_lock(&ctx->fd_dev->u_lock);
	switch (ctrl->id) {
	case V4L2_CID_MTK_AIE_INIT:
		memcpy(&ctx->fd_dev->u_init, ctrl->p_new.p_u32, sizeof(struct user_init));
		dev_dbg(dev, "init param : max w:%d, max h:%d",
			ctx->fd_dev->u_init.max_img_width,
			ctx->fd_dev->u_init.max_img_height);
		dev_dbg(dev, "init param : p_w:%d, p_h:%d, f thread:%d",
			ctx->fd_dev->u_init.pyramid_width,
			ctx->fd_dev->u_init.pyramid_height,
			ctx->fd_dev->u_init.feature_thread);
		break;
	default:
	...
	mutex_unlock(&ctx->fd_dev->u_lock);
