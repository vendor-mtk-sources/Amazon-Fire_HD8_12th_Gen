...
static bool dumpReg = false;
module_param(dumpReg, bool, 0644);
...
static void mtk_jpeg_update_bw_request(struct mtk_jpeg_ctx *ctx)
{
	...
	pr_info("bw update +");
	picSize = (ctx->out_q.w * ctx->out_q.h) / 1000;
	emi_bw = picSize * target_fps;
	emi_bw = emi_bw * 4/3;
	pr_info("Width %d Height %d emi_bw %d\n",
		 ctx->out_q.w, ctx->out_q.h, emi_bw);
...
static void mtk_jpeg_job_timeout_work(struct work_struct *work)
{
	...
	pr_info("irq_timeout_work, failed\n");
	mtk_jpeg_enc_reg_dump(jpeg->reg_base);
