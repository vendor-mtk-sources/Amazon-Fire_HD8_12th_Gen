...
unsigned long div_64(unsigned long a, unsigned long b)
{
#if IS_ENABLED(CONFIG_64BIT)
	return (a/b);
#else
	...
	dividend = (a >= 0) ? a : (-a);
	divisor = (b >= 0) ? b : (-b);
	rem = do_div_dvfs(dividend, divisor);
	a = ((a < 0) ^ (b < 0)) ?
		(0LL - (unsigned long)dividend) :
...
/**
 * free_next_hist - free next codec_history and shift the next but one forward
 *                  (not used)
 *
 ...
 *                   is really for the next but one job.
 *
 * Return time in us. 0 means no history available, treat it as coming now
 */
unsigned long est_next_submit(struct codec_history *hist)
{
	...
	/* Add 2x estimated gap for the next but one job */
	if (hist->cur_cnt == 1)
 ...
 *        -1 for error
 */
int est_next_job(unsigned long now_us, unsigned long *t_us, int *kcy, int *min_mhz,
		struct codec_job *job, struct codec_history *head)
{
	struct codec_history *hist;
	unsigned long deadline;
	unsigned long exec_dur;
...
int update_hist_item(struct codec_job *job, struct codec_history *hist)
{
	...
	if ((hist->cur_cnt > 1 &&
		...
		memset(hist->submit, 0, sizeof(unsigned long)*MAX_HISTORY);
		memset(hist->start, 0, sizeof(unsigned long)*MAX_HISTORY);
		memset(hist->end, 0, sizeof(unsigned long)*MAX_HISTORY);
		memset(hist->sw_time, 0, sizeof(unsigned long)*MAX_HISTORY);
