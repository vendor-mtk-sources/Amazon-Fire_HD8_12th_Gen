...
static int scpsys_bus_protect_enable(struct scpsys_domain *pd, enum regmap_type type)
{
	int i, ret = 0;
	const struct scpsys_bus_prot_data *bpd = pd->data->bp_cfg;
	struct regmap *map;
	u32 val = 0, mask, ack_mask;

	for (i = 0; i < SPM_MAX_BUS_PROT_DATA; i++) {
		mask = bpd[i].bus_prot_mask;

		if (bpd[i].type == INFRA_TYPE)
			map = pd->infracfg;
		else if (bpd[i].type == SUB_TYPE)
			map = pd->subcfg;
		else if (bpd[i].type == SMI_TYPE)
			map = pd->smi;
		else
			map = NULL;

		if (!mask || !map || (type != INVALID_TYPE && type != bpd[i].type))
			continue;

		if (bpd[i].setclr_inv) {
			if (bpd[i].bus_prot_reg_update)
				regmap_clear_bits(map, bpd[i].bus_prot_clr, mask);
			else
				regmap_write(map, bpd[i].bus_prot_clr, mask);

			ack_mask = 0;
		} else {
			if (bpd[i].bus_prot_reg_update)
				regmap_set_bits(map, bpd[i].bus_prot_set, mask);
			else
				regmap_write(map, bpd[i].bus_prot_set, mask);

			ack_mask = mask;
		}

		ret = regmap_read_poll_timeout(map, bpd[i].bus_prot_sta,
					val, (val & mask) == ack_mask,
...
static int scpsys_bus_protect_disable(struct scpsys_domain *pd, enum regmap_type type)
{
	int i, ret = 0;
	const struct scpsys_bus_prot_data *bpd = pd->data->bp_cfg;
	struct regmap *map;
...
static int scpsys_clamp_protection(struct scpsys_domain *pd)
{
	...
	ret = scpsys_bus_protect_enable(pd, SMI_TYPE);
	if (ret)
		return ret;
...
static int scpsys_smi_larb_reset(struct scpsys_domain *pd)
{
	...
	for (i = 0; i < SPM_MAX_SMI_RESET_DATA; i++) {
		ret = _scpsys_smi_larb_reset(pd->data->reset_smi[i], pd->larb);
		if (ret)
			return ret;
	}
...
static int scpsys_power_on(struct generic_pm_domain *genpd)
{
	...
	if (ret) {
		pr_err("[%s] regulator enable fail\n", __func__);
		return ret;
	}

	ret = clk_bulk_prepare_enable(pd->num_clks, pd->clks);
	if (ret) {
		pr_err("[%s] clk prepare enable fail\n", __func__);
		goto err_reg;
	}
	...
	if (ret) {
		pr_err("[%s] sub clk prepare enable fail\n", __func__);
		goto err_pwr_ack;
	}

	ret = scpsys_sram_enable(pd);
	if (ret < 0) {
		pr_err("[%s] sram enable fail\n", __func__);
	...
	ret = scpsys_bus_protect_disable(pd, INVALID_TYPE);
	if (ret < 0) {
		pr_err("[%s] bus protect disable fail\n", __func__);
...
static int scpsys_power_off(struct generic_pm_domain *genpd)
{
	...
	ret = scpsys_bus_protect_enable(pd, INVALID_TYPE);
	if (ret < 0) {
		pr_err("[%s] bus protect enable fail\n", __func__);
		return ret;
	}

	ret = scpsys_sram_disable(pd);
	if (ret < 0) {
		pr_err("[%s] sram disable fail\n", __func__);
		return ret;
	}
	...
	ret = readx_poll_timeout(scpsys_domain_is_on, pd, tmp, !tmp, MTK_POLL_DELAY_US,
				 MTK_POLL_TIMEOUT);
	if (ret < 0) {
		pr_err("[%s] power off ack timeout\n", __func__);
		return ret;
	}
...
generic_pm_domain *scpsys_add_one_domain(struct scpsys *scpsys, struct device_node *node)
{
	...
	pd->subcfg = syscon_regmap_lookup_by_phandle_optional(node, "mediatek,subcfg");
	if (IS_ERR(pd->subcfg))
		return ERR_CAST(pd->subcfg);

	pd->smi = syscon_regmap_lookup_by_phandle_optional(node, "mediatek,smi");
	if (IS_ERR(pd->smi))
		return ERR_CAST(pd->smi);

	pd->larb = syscon_regmap_lookup_by_phandle_optional(node, "mediatek,larb");
	if (IS_ERR(pd->larb))
		return ERR_CAST(pd->larb);
...
static const struct of_device_id scpsys_of_match[] = {
	{
		.compatible = "mediatek,mt8169-power-controller",
		.data = &mt8169_scpsys_data,
	},
	{ }
};
