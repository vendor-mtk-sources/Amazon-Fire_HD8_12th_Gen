...
static void svs_get_vops_v2(struct svs_platform *svsp)
{
	...
	/* mt8169 only */
	if (svsb->sw_id == SVSB_GPU && svsb->opp_freqs[0] > svsb->freq_base) {
		if (svsb->volt_bin == 0x3)
			svsb->volts[0] = 0x58; /* 0.95V */
		else if (svsb->volt_bin == 0x4)
			svsb->volts[0] = 0x52; /* 0.9125V */
		else if (svsb->volt_bin == 0x5)
			svsb->volts[0] = 0x4c; /* 0.875V */
		else
		...
		for (i = 1; i < svsb->turn_pt; i++)
			svsb->volts[i] =
				interpolate(svsb->freqs_pct[0],
					    svsb->freqs_pct[svsb->turn_pt],
					    svsb->volts[0],
					    svsb->volts[svsb->turn_pt],
					    svsb->freqs_pct[i]);
	}
}
...
static bool svs_mt8169_efuse_parsing(struct svs_platform *svsp)
{
	struct svs_bank *svsb;
	...
	if (svsp->fake_efuse) {
		...
		svsp->efuse[0] = 0x00030002;
		svsp->efuse[1] = 0x67122c38;
		svsp->efuse[2] = 0x80e92438;
		svsp->efuse[3] = 0x47142c0d;
		svsp->efuse[4] = 0x62122c3b;
		svsp->efuse[5] = 0x67132c3b;
		svsp->efuse[6] = 0x3f1a2452;
		svsp->efuse[13] = 0x1b031b03;
		svsp->efuse[14] = 0x1b031b03;
		svsp->efuse[15] = 0x1b031b03;
		svsp->efuse[19] = 0x80000005;
	}
	...
	for (idx = 0; idx < svsp->bank_num; idx++) {
		...
		case SVSB_GPU:
			...
			svsb->volt_bin = svsp->efuse[19] & GENMASK(2, 0);
			break;
		default:
			break;
		}
...
static struct svs_bank svs_mt8169_banks[] = {
	{
		.sw_id			= SVSB_CPU_BIG,
		...
		.tzone_high_temp_offset	= 8,
		.tzone_low_temp		= 25000,
		.tzone_low_temp_offset	= 8,
		.core_sel		= 0x0fff0100,
		.int_st			= BIT(0),
		.ctl0			= 0x00540003,
		.type			= SVSB_LOW,
...
static int svs_get_svs_mt8169_platform_data(struct svs_platform *svsp)
{
	struct device *dev;
	struct svs_bank *svsb;
	bool is_cpu_big_supported, is_mt6357_supported = true;
	u32 idx, cpu, gpu;

	svsp->name = "mt8169-svs";
	svsp->banks = svs_mt8169_banks;
	svsp->efuse_parsing = svs_mt8169_efuse_parsing;
	svsp->regs = svs_regs_v2;
	svsp->irqflags = IRQF_TRIGGER_HIGH;
	svsp->bank_num = ARRAY_SIZE(svs_mt8169_banks);
	svsp->efuse_check = 0;
	...
	for (idx = 0; idx < svsp->bank_num; idx++) {
		...
		case SVSB_CPU_LITTLE:
			...
			break;
		case SVSB_CPU_BIG:
			/* help check if CPU BIG exists */
			for_each_possible_cpu(cpu) {
				if (svsb->cpu_id == cpu) {
					is_cpu_big_supported = true;
					is_mt6357_supported = false;
					break;
				}
			}

			if (!is_cpu_big_supported)
				continue;

			if (svsb->type == SVSB_LOW)
				svsb->mode_support = SVSB_MODE_INIT02;
	...
	/* For MT6357 support. */
	if (is_mt6357_supported)
		svsp->banks[gpu].vmin = 0x28;
...
static const struct of_device_id mtk_svs_of_match[] = {
	{
		.compatible = "mediatek,mt8169-svs",
		.data = &svs_get_svs_mt8169_platform_data,
	}, {
		/* Sentinel */
	},
};
