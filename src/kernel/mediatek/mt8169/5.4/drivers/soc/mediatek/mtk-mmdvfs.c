...
static void set_all_muxes(struct mmdvfs_drv_data *drv_data, u32 opp_level)
{
	...
	for (i = 0; i < num_muxes; i++) {
		...
		mmdvfs_debug("prepare mux(%s):%d opp_level:%d\n",
			drv_data->muxes[i].mux_name, opp_level);
		...
		mmdvfs_debug("set parent(%s):%d opp_level:%d\n",
				drv_data->muxes[i].mux_name, opp_level);
	}
}
...
static void set_all_hoppings(struct mmdvfs_drv_data *drv_data, u32 opp_level)
{
	...
	for (i = 0; i < num_hoppings; i++) {
		...
		mmdvfs_debug("prepare hopping(%s)opp_level:%d\n",
			drv_data->hoppings[i].hopping_name, opp_level);
...
static int mmdvfs_probe(struct platform_device *pdev)
{
	...
	of_property_for_each_string(dev->of_node, "mediatek,support_mux",
		...
		mmdvfs_notice("%s: support_mux:%s, num_mux:%d\n", __func__, mux_name, num_mux);
	}
	...
	of_property_for_each_string(dev->of_node, "mediatek,support_hopping",
		...
		num_hopping++;
		mmdvfs_notice("%s: support_hopping:%s, num_hopping:%d\n",
			__func__, hopping_name, num_hopping);
	}
	...
	freq = 0;
	index = 0;
	mmdvfs_notice("%s: dev_pm_opp_find_freq_ceil ++\n", __func__);
	while (!IS_ERR(opp = dev_pm_opp_find_freq_ceil(dev, &freq))) {
		drv_data->voltages[index] = dev_pm_opp_get_voltage(opp);
		freq++;
		index++;
		dev_pm_opp_put(opp);
		mmdvfs_notice("%s: opp-index:%d, freq:%ld\n", __func__, index, freq);
	}
	reg = devm_regulator_get(dev, "dvfsrc-vcore");
	if (IS_ERR(reg))
		return PTR_ERR(reg);
	mmdvfs_notice("%s: devm_regulator_get done\n", __func__);
...
static int __init mtk_mmdvfs_init(void)
{
	...
	mmdvfs_notice("%s: platform_driver_register  done\n", __func__);
	return 0;
}
