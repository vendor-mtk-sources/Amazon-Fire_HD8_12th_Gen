	...
	struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (!pmic_op || !pmic_op->pmic_clk_buf_set_cap_id
	...
	if (buf != NULL && count != 0) {
		if (!strncmp(buf, "cmd1#", 5)) {
			...
			pmic_op->pmic_clk_buf_get_cap_id(&capid_old);
			pr_info("original cap code: 0x%x\n", capid_old);

			pmic_op->pmic_clk_buf_set_cap_id(capid);
...
static ssize_t clk_buf_capid_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	if (!pmic_op || !pmic_op->pmic_clk_buf_get_cap_id)
		return CLK_BUF_NOT_READY;

	pmic_op->pmic_clk_buf_get_cap_id(&capid);
...
static int get_capid_idme(u32 *cap)
{
	...
	if (ap) {
		...
		if (len > 0) {
			...
			if (ret || *cap > PMIC_XO_CDAC_FPM_MASK) {
...
int clk_buf_dts_init(struct platform_device *pdev)
{
	...
	u32 cap = 0, capid_old = 0, capid_new = 0;
	...
	if (!ret) {
		...
		pmic_op->pmic_clk_buf_set_cap_id_pre();
		pmic_op->pmic_clk_buf_set_cap_id(cap);
