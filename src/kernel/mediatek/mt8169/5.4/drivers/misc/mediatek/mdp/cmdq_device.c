...
#if IS_ENABLED(CONFIG_FPGA_8169)
...
#if IS_ENABLED(CONFIG_xxxMACH_MT6885) || IS_ENABLED(CONFIG_xxxMACH_MT6893)
	struct device *pdev2;
	unsigned long va2;
	phys_addr_t pa2;
	u32 irqId2;
...
struct device *cmdq_dev_get_cmdq(void)
{
	return gCmdqDev.pDev_cmdq;
}

struct device *cmdq_dev_get_mdp(void)
{
	return gCmdqDev.pDev;
}
...
void cmdq_dev_init(struct platform_device *cmdq_pDevice,
	struct platform_device *mdp_pDevice)
{
	struct device_node *node = mdp_pDevice->dev.of_node;
	u32 dma_mask_bit = 0;
	s32 ret;
#if IS_ENABLED(CONFIG_xxxMACH_MT6885) || IS_ENABLED(CONFIG_xxxMACH_MT6893)
	...
	pm_runtime_enable(&mdp_pDevice->dev);
	/* init cmdq device dependent data */
	do {
		memset(&gCmdqDev, 0x0, sizeof(struct CmdqDeviceStruct));

		gCmdqDev.pDev_cmdq = &cmdq_pDevice->dev;
		gCmdqDev.pDev = &mdp_pDevice->dev;
#if !IS_ENABLED(CONFIG_xxxMACH_MT6885) && !IS_ENABLED(CONFIG_xxxMACH_MT6893)
		...
		gCmdqDev.clk_gce = devm_clk_get(&mdp_pDevice->dev, "GCE");
		gCmdqDev.clk_gce_timer = devm_clk_get(&mdp_pDevice->dev,
			"GCE_TIMER");
		gCmdqDev.clk_mmsys_mtcmos = devm_clk_get(&mdp_pDevice->dev,
			"MMSYS_MTCMOS");
	...
	ret = of_property_read_u32(gCmdqDev.pDev->of_node,
		"mediatek,dma_mask_bit",
		&dma_mask_bit);//mediatek,dma_mask_bit = <34>;
	gCmdqDev.dma_mask_result = dma_set_mask_and_coherent(
		&mdp_pDevice->dev, DMA_BIT_MASK(34));//34
	/* map MMSYS VA */
