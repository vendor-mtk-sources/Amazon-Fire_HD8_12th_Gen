...
#include "cmdq-sec.h"
struct cmdq_sec_helper_fp *cmdq_util_sec_helper;
#endif
...
#if IS_ENABLED(CONFIG_MTK_DEVAPC)
__weak void register_devapc_vio_callback(struct devapc_vio_callbacks *viocb)
{
	cmdq_log("Devapc vio callback function is not ready!");
}
#endif
...
static void cmdq_util_handle_devapc_vio(void)
{
	...
	for (i = 0; i < util.mbox_sec_cnt; i++)
		cmdq_util_sec_helper->sec_dump_thread_all(util.cmdq_sec_mbox[i]);
#endif
}
