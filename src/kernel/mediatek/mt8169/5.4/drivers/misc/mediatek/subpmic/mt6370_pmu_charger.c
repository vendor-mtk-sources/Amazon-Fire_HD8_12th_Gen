...
struct mt6370_iaicr_calibration
{
...
struct mt6370_pmu_charger_data {
	...
	struct pinctrl *sgm2541_pinctrl;
	struct pinctrl_state *sgm2541_output0;
	struct pinctrl_state *sgm2541_output1;
	struct pinctrl_state *usbc_otg_en0;
	struct pinctrl_state *usbc_otg_en1;

	struct mt6370_iaicr_calibration iaicr_data;
};
...
static void mt6370_calibrate_aicl_result(struct mt6370_pmu_charger_data *chg_data,
	u32 *uA);
...
static const char *mt6370_chg_get_cmd(void)
{
	struct file *fd;
	mm_segment_t fs;
	loff_t pos = 0;
...
static bool mt6370_get_wpc_support(void)
{
	char keyword[] = "androidboot.wpc.support=1";

	if (strstr(mt6370_chg_get_cmd(), keyword))
		return true;
...
static void diff(int index, ktime_t start, ktime_t end)
{
	static ktime_t ktime_diff_array[9] = {0};

	if (index < 1 || index > 8) {
		pr_info("%s: index input error\n", __func__);
		return;
	}

	if (index == 1)
		memset(ktime_diff_array, 0, sizeof(ktime_diff_array));

	ktime_diff_array[index] = ktime_sub(end, start);
	if (index == 8)
		pr_info("%s: duration:[1-8] %lld,%lld,%lld,%lld,%lld,%lld,%lld,%lld\n",
			__func__, ktime_diff_array[1], ktime_diff_array[2], ktime_diff_array[3],
			ktime_diff_array[4], ktime_diff_array[5], ktime_diff_array[6],
			ktime_diff_array[7], ktime_diff_array[8]);
}
	...
	enum mt6370_adc_sel adc_sel, int *adc_val)
{
	...
	ptime[2] = ktime_get_boottime();
	diff(2, ptime[1], ptime[2]);
	...
	int state)
{
	struct phy *phy;
	int ret;
	int mode = (state == MT6370_USBSW_CHG) ? PHY_MODE_BC11_SET :
						 PHY_MODE_BC11_CLR;
...
static int __mt6370_chgdet_handler(struct mt6370_pmu_charger_data *chg_data)
{
	...
	dev_info(chg_data->dev, "%s chg_type:%d, pwr_rdy:%d, usb_status = %d\n",
				__func__, chg_data->psy_usb_type,
				chg_data->pwr_rdy, usb_status);
...
{
	int ret = 0;
	u8 reg_ieoc = 0, ieoc_mod = 0;

	/* IEOC workaround */
	/* 500 <= ICHG < 900mA, IEOC *= 0.57 */
	if (chg_data->ieoc_wkard)
		ieoc = ieoc * 100 / 57;

	/* roundup for safety check */
	ieoc_mod = ieoc % MT6370_IEOC_STEP;
	if (ieoc_mod > 0)
		ieoc += MT6370_IEOC_STEP;
...
static int __mt6370_run_aicl(struct charger_device *chg_dev)
{
	int ret = 0;
	...
	if (!chg_dev->is_adapter_detect)
		aicl_vth = mivr + chg_data->chg_desc->aicl_vth_offset;
	else
		aicl_vth = mivr + DETECTION_VTH_OFFSET_UV;
...
skip_post_aicl:
	/* if aicr > 1A, compensation data is applied */
	if (aicr > 1000000) {
		if (atomic_read(&chg_data->iaicr_data.enable_iaicr_cal) &&
			chg_data->chg_desc->en_iaicr_cal) {
			mt6370_calibrate_aicl_result(chg_data, &aicr);
		} else {
...
static int __mt6370_set_ichg(struct mt6370_pmu_charger_data *chg_data, u32 uA)
{
	...
	if (uA < 900000 && !chg_data->ieoc_wkard) {
		chg_data->ieoc_wkard = true;
		ret = __mt6370_set_ieoc(chg_data, chg_data->ieoc);
...
static int mt6370_is_enabled_charging(struct charger_device *chg_dev, bool *en)
{
	struct mt6370_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);

	return mt6370_is_charging_enable(chg_data, en);
}
...
static int idme_get_iaicr_cal(struct mt6370_pmu_charger_data *chg_data,
	int *data, u8 size)
{
...
static void correct_iaicr_cal_data(int *iaicr_cal, int target_iaicr)
{
	...
	/* Max. variation of mt6370 is ~15%. Calculate the corresponding
	...
	struct mt6370_pmu_charger_data *chg_data)
{
	...
	memset(chg_data->iaicr_data.iaicr_cal_data, 0, sizeof(chg_data->iaicr_data.iaicr_cal_data));
	atomic_set(&chg_data->iaicr_data.enable_iaicr_cal, 0);

#if IS_ENABLED(CONFIG_IDME)
	ret = idme_get_iaicr_cal(chg_data, idmedata, IAICR_ARRAY_NUM);
	if (ret < 0) {
		/* There is no IAICR calibration data. */
		dev_err(chg_data->dev, "%s: Not support IAICR calibration.\n", __func__);
		return -1;
	}
#else
	dev_info(chg_data->dev, "%s: Not support IAICR calibration.\n", __func__);
	return 0;
#endif

	for (i = 0; i < IAICR_ARRAY_NUM; i++) {
		chg_data->iaicr_data.iaicr_cal_data[i] = idmedata[i];
		correct_iaicr_cal_data(&chg_data->iaicr_data.iaicr_cal_data[i],
			chg_data->iaicr_data.iaicr_itarget[i]);
		dev_info(chg_data->dev, "%s: %d\n", __func__, chg_data->iaicr_data.iaicr_cal_data[i]);
	}

	atomic_set(&chg_data->iaicr_data.enable_iaicr_cal, 1);
	...
	u32 *uA)
{
	...
	if (!atomic_read(&chg_data->iaicr_data.enable_iaicr_cal) ||
		(*uA < chg_data->iaicr_data.iaicr_threshold[0] +
		 chg_data->iaicr_data.iaicr_cal_data[0] * 1000))
		return;

	for (i = 1; i < IAICR_ARRAY_NUM; i++) {
		if ((*uA) < chg_data->iaicr_data.iaicr_threshold[i] +
			chg_data->iaicr_data.iaicr_cal_data[i] * 1000)
			break;
	}

	itarget = chg_data->iaicr_data.iaicr_itarget[i - 1];
	iaicr_cal = chg_data->iaicr_data.iaicr_cal_data[i - 1];

	ma = *uA / 1000;

	calibrated_ma = mult_frac(ma,
		itarget, (itarget + iaicr_cal));
	calibrated_ma = max(calibrated_ma, MT6370_AICR_MIN / 1000);

	*uA = (unsigned int)calibrated_ma * 1000;
	dev_err(chg_data->dev, "%s: ma:%d %d uA:%u\n", __func__, ma, calibrated_ma, *uA);
}
	...
	u32 *uA)
{
	...
	calibrated_ma = min(calibrated_ma, MT6370_AICR_MAX/1000);
...
static int mt6370_get_indpm_state(struct charger_device *chg_dev,
				bool *vdpm, bool *idpm)
{
	int ret = 0;
	struct mt6370_pmu_charger_data *chg_data =
	...
	*vdpm = !!(ret & MT6370_MASK_MIVR_STAT);
	*idpm = !!(ret & MT6370_MASK_AICR_STAT);
...
static int mt6370_enable_otg(struct charger_device *chg_dev, bool en)
{
	...
	if (chg_data->chg_desc->power_switch_support && chg_data->sgm2541_pinctrl) {
		if (en)	{
			pinctrl_select_state(chg_data->sgm2541_pinctrl, chg_data->sgm2541_output1);
			mdelay(3);
			pinctrl_select_state(chg_data->sgm2541_pinctrl, chg_data->usbc_otg_en1);
		} else {
			pinctrl_select_state(chg_data->sgm2541_pinctrl, chg_data->usbc_otg_en0);
			/* Set 50ms in order to ensure that usb_otg_en level is low. */
			msleep(50);
			pinctrl_select_state(chg_data->sgm2541_pinctrl, chg_data->sgm2541_output0);
		}
	...
	/* Disable watchdog to avoid voltage-drop during suspend */
	mt6370_enable_wdt(chg_data, false);
...
static int mt6370_enable_dcap(struct charger_device *chg_dev, bool en)
{
	struct mt6370_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);

	chg_data->dcap_en = en;
	return chg_data->dcap_en;
}
...
static irqreturn_t mt6370_pmu_ovpctrl_uvp_d_evt_irq_handler(int irq, void *data)
{
	...
	if (IS_ENABLED(CONFIG_TCPC_CLASS)) {
		dev_err(chg_data->dev, "%s: use tcpc\n", __func__);
		goto out;
	}
...
int sgm2541_get_gpio_info(struct device *dev, struct mt6370_pmu_charger_data *chg_data)
{
	int ret = 0;

	chg_data->sgm2541_pinctrl = devm_pinctrl_get(dev);

	if (IS_ERR(chg_data->sgm2541_pinctrl)) {
		ret = PTR_ERR(chg_data->sgm2541_pinctrl);
		dev_err(chg_data->dev, "%s: Cannot find pinctrl\n", __func__);
		goto out;
	}
	chg_data->sgm2541_output0 = pinctrl_lookup_state(chg_data->sgm2541_pinctrl, "sgm2541_en_output0");
	if (IS_ERR(chg_data->sgm2541_output0)) {
		ret = PTR_ERR(chg_data->sgm2541_output0);
		dev_err(chg_data->dev, "%s: Cannot find pinctrl sgm2541_output0!\n", __func__);
		goto out;
	}
	chg_data->sgm2541_output1 = pinctrl_lookup_state(chg_data->sgm2541_pinctrl, "sgm2541_en_output1");
	if (IS_ERR(chg_data->sgm2541_output1)) {
		ret = PTR_ERR(chg_data->sgm2541_output1);
		dev_err(chg_data->dev, "%s: Cannot find pinctrl sgm2541_output1!\n", __func__);
		goto out;
	}
	chg_data->usbc_otg_en0 = pinctrl_lookup_state(chg_data->sgm2541_pinctrl, "usbc_otg_en0");
	if (IS_ERR(chg_data->usbc_otg_en0)) {
		ret = PTR_ERR(chg_data->usbc_otg_en0);
		dev_err(chg_data->dev, "%s: Cannot find pinctrl usbc_otg_en0!\n", __func__);
		goto out;
	}
	chg_data->usbc_otg_en1 = pinctrl_lookup_state(chg_data->sgm2541_pinctrl, "usbc_otg_en1");
	...
	struct mt6370_pmu_charger_data *chg_data)
{
	...
	if (node) {
		if (of_property_read_u32(node, "bc12_sel", &chg_desc->bc12_sel) < 0)
			dev_notice(chg_data->dev, "%s: can't parse bc12_sel\n", __func__);
	} else
		dev_notice(chg_data->dev, "%s: can't parse phandle bc12_ref\n", __func__);

	if (of_property_read_u32(np, "aicl_vth_offset",
			&chg_desc->aicl_vth_offset) < 0) {
		dev_err(chg_data->dev, "%s: no aicl_vth_offset, use default value\n", __func__);
		chg_desc->aicl_vth_offset = 200000;
	}

	if (of_property_read_u32(np, "aicl_err_h",
			&chg_desc->aicl_err_h) < 0) {
		dev_err(chg_data->dev, "%s: no aicl_err_h, use default value\n", __func__);
		chg_desc->aicl_err_h = 200000;
	}

	if (of_property_read_u32(np, "aicl_err_l",
			&chg_desc->aicl_err_l) < 0) {
		dev_err(chg_data->dev, "%s: no aicl_err_l, use default value\n", __func__);
	...
	chg_desc->en_iaicr_cal = of_property_read_bool(np, "enable_iaicr_cal");
	chg_desc->power_switch_support = mt6370_get_wpc_support();

	if (of_property_read_u32_array(np, "iaicr_itarget",
		chg_data->iaicr_data.iaicr_itarget, IAICR_ARRAY_NUM) < 0)
		dev_err(chg_data->dev, "%s: 'iaicr_itarget' cannot be read!\n", __func__);

	if (of_property_read_u32_array(np, "iaicr_threshold",
		chg_data->iaicr_data.iaicr_threshold, IAICR_ARRAY_NUM) < 0)
		dev_err(chg_data->dev, "%s: 'iaicr_threshold' cannot be read!\n", __func__);

	if (chg_desc->power_switch_support)
		sgm2541_get_gpio_info(dev, chg_data);
...
static int mt6370_chg_init_setting(struct mt6370_pmu_charger_data *chg_data)
{
	...
	ret = mt6370_pmu_reg_update_bits(chg_data->chip, MT6370_PMU_REG_CHGCTRL3,
		MT6370_MASK_AICR_EN, 1 << MT6370_SHIFT_AICR_EN);
	if (ret < 0)
		dev_err(chg_data->dev, "%s: Enable AICR failed\n", __func__);
	...
	if (ret < 0)
		dev_err(chg_data->dev, "%s: get ieoc failed\n", __func__);
...
static struct charger_ops mt6370_chg_ops = {
	...
	.is_enabled = mt6370_is_enabled_charging,
	...
	.get_indpm_state = mt6370_get_indpm_state,
	...
	.enable_dcap = mt6370_enable_dcap,
	...
	struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "enable_iaicr_cal %d\n",
		atomic_read(&chg_data->iaicr_data.enable_iaicr_cal));
}
        ...
        struct device *dev, struct device_attribute *attr, char *buf)
{
        ...
        ret[0] = mt6370_pmu_reg_read(chg_data->chip, MT6370_PMU_REG_CHGCTRL3);
        if (ret[0] < 0)
                return ret[0];

        ret[1] = mt6370_pmu_reg_read(chg_data->chip, MT6370_PMU_REG_CHGCTRL4);
        if (ret[1] < 0)
                return ret[1];

        ret[2] = mt6370_pmu_reg_read(chg_data->chip, MT6370_PMU_REG_CHGCTRL6);
        if (ret[2] < 0)
                return ret[2];

        ret[3] = mt6370_pmu_reg_read(chg_data->chip, MT6370_PMU_REG_CHGCTRL7);
        if (ret[3] < 0)
                return ret[3];

        ret[4] = mt6370_pmu_reg_read(chg_data->chip, MT6370_PMU_REG_CHGSTAT);
        if (ret[4] < 0)
                return ret[4];

        ret[5] = mt6370_pmu_reg_read(chg_data->chip, MT6370_PMU_REG_CHGSTAT1);
        if (ret[5] < 0)
                return ret[5];

        return sprintf(buf, "13:%02X 14:%02X 16:%02X 17:%02X 4A:%02X D0:%02X\n",
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TYPE:
		...
		bat_metrics_chrdet(chg_data->psy_desc.type);
		break;
...
static int mt6370_pmu_charger_probe(struct platform_device *pdev)
{
	...
	ret = mt6370_update_iaicr_cal_value(chg_data);
	if (ret < 0) {
		dev_err(chg_data->dev, "%s: update_iaicr_cal_value failed\n",
			__func__);
	}
	...
	ret = device_create_file(chg_data->dev,
		&dev_attr_enable_iaicr_calibration);
	if (ret < 0) {
		dev_notice(&pdev->dev, "create enable_iaicr_calibration fail\n");
		goto err_devfs;
	}
...
static int mt6370_pmu_charger_remove(struct platform_device *pdev)
{
	...
	if (chg_data) {
		device_remove_file(chg_data->dev, &dev_attr_shipping_mode);
		device_remove_file(chg_data->dev,
			&dev_attr_enable_iaicr_calibration);
		device_remove_file(chg_data->dev, &dev_attr_CHG_REGS);
		charger_device_unregister(chg_data->ls_dev);
		charger_device_unregister(chg_data->chg_dev);
		mutex_destroy(&chg_data->ichg_access_lock);
		mutex_destroy(&chg_data->adc_access_lock);
