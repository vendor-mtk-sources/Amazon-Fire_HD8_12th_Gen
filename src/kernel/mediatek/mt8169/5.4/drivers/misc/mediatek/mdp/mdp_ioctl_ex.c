...
//	struct ion_handle *handles[MAX_HANDLE_NUM];
	struct dma_buf *dma_buffer[MAX_HANDLE_NUM];
	struct dma_buf_attachment *dma_attach[MAX_HANDLE_NUM];
	struct sg_table *table[MAX_HANDLE_NUM];
	int fds[MAX_HANDLE_NUM];
	u64 mvas[MAX_HANDLE_NUM];
	u32 handle_count;
	void *node;
};
				...
				struct mdp_job_mapping *mapping_job)
{
	...
	dma_addr_t dma_addr;
	unsigned long mva = 0;
	struct iommu_domain *domin;
	u32 i;
	u32 port = cmdq_mdp_get_hw_port(meta->engine);

	if (!port)
		return 0;
	domin = iommu_get_domain_for_dev(cmdq_dev_get_mdp());
	...
	if (i == mapping_job->handle_count) {
		...
		dma_buffer = dma_buf_get(meta->fd);
		if (IS_ERR(dma_buffer)) {
			cmdq_err("ion_fd[%d] pass to mdp is invalid, get dma_buffer failed[%d]",
				meta->fd, PTR_ERR(dma_buffer));
			return -EINVAL;
		}
		dma_attach = dma_buf_attach(dma_buffer, cmdq_dev_get());
...
//			mapping_job->handles[i] = ion_h;
			mapping_job->dma_buffer[i] = dma_buffer;
			mapping_job->dma_attach[i] = dma_attach;
			mapping_job->table[i] = table;
			mapping_job->mvas[i] = mva;
			mapping_job->handle_count++;
		} else {
			mdp_ion_free_handle(dma_buffer, dma_attach, table);
			return 0;
		}
	} else {
				...
				struct cmdq_command_buffer *cmd_buf)
{
	...
	handle->reg_values = cmdq_core_alloc_hw_buffer(cmdq_dev_get_cmdq(),
...
void mdp_check_pending_task(struct mdp_job_mapping *mapping_job)
{
	...
	for (i = 0; i < mapping_job->handle_count; i++)
		mdp_ion_free_handle(mapping_job->dma_buffer[i],
			mapping_job->dma_attach[i],
			mapping_job->table[i]);
	kfree(mapping_job);
}
#endif
...
s32 mdp_ioctl_async_wait(unsigned long param)
{
	...
	if (handle->pkt->sec_data) {
		for (i = 0; i < data->addrMetadataCount; i++)
			mdp_ion_free_handle(data->dma_buffer[i],
				data->dma_attach[i],
				data->table[i]);
	}
#endif
