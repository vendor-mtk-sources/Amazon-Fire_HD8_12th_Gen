...
void __iomem *CAMX_REG_TG_INTER_ST(int reg_module)
{
	...
	if (reg_module < 0 || reg_module >= ISP_DEV_NODE_NUM) {
		LOG_NOTICE("Wrong reg module: %d\n", reg_module);
		return NULL;
	}
	switch (reg_module) {
	case ISP_CAM_A_IDX:
...
static void ISP_RecordCQAddr(enum ISP_DEV_NODE_ENUM regModule)
{
	...
	for (i = 0; i < reg_module_count; i++) {
		...
		if (index > (ISP_CAM_C_INNER_IDX - ISP_CAM_A_INNER_IDX) || index < 0) {
			LOG_NOTICE(
...
static unsigned int NodeName_to_DevIdx(const char *name)
{
	 ...
	 * defined in dts.
	 */
	if (strncmp(name, "camisp", strlen("camisp")) == 0)
...
void ISP_get_larb(struct platform_device *pDev, unsigned int dev_idx, int larb_num)
{
	...
	for (i = 0; i < larb_num; i++) {
		...
		if (!node) {
			LOG_DBG("%s: no mediatek,larb found\n",
...
#endif
	if (atomic_read(&G_u4DevNodeCt) == 0)
...
/* Get platform_device parameters */
	...
	*(isp_devs[dev_idx].dev->dma_mask) = (u64)DMA_BIT_MASK(34);
	...
	if ((strncmp(pDev->dev.of_node->name, "cam1", strlen("cam1")) == 0) ||
		(strncmp(pDev->dev.of_node->name, "cam2", strlen("cam2")) == 0) ||
		(strncmp(pDev->dev.of_node->name, "cam3", strlen("cam3")) == 0) ||
		(strncmp(pDev->dev.of_node->name, "camisp", strlen("camisp")) == 0))
	...
	if (isp_devs[dev_idx].irq > 0) {
		...
		LOG_DBG("No IRQ!!: G_u4DevNodeCt=%d, devnode(%s), irq=%d\n",
			atomic_read(&G_u4DevNodeCt),
			pDev->dev.of_node->name,
			isp_devs[dev_idx].irq);
	}
	...
	else if (strncmp(pDev->dev.of_node->name, "camisp",
		strlen("camisp")) == 0)
		ISP_get_larb(pDev, dev_idx, 2);
#endif

	/* Only register char driver in the camisp */
	if (strncmp(pDev->dev.of_node->name, "camisp", strlen("camisp")) == 0) {
		...
		LOG_DBG("Init %s", pDev->dev.of_node->name);
...
/* Otherwise, probe() of Seninf driver cannot be called. */
	...
	for (k = ISP_IRQ_TYPE_INT_CAM_A_ST; k <= ISP_IRQ_TYPE_INT_CAM_B_ST; k++)
#else
			  ...
			  enum ISP_DEV_NODE_ENUM cam_idx, const char *str)
{
	...
	if (IrqStatus & SV_SW_PASS1_DON_ST) {
		...
		//TODO: in MT6873 is IspInfo.DebugMask & ISP_DBG_CAMSV_LOG, HW change?
		if (IspInfo.DebugMask & ISP_DBG_INT) {
			IRQ_LOG_KEEPER(
				module, m_CurrentPPB, _LOG_INF,
