...
#ifdef MT8169_CMDQ_SEC //IS_ENABLED(CONFIG_xxxMACH_MT6885) || IS_ENABLED(CONFIG_xxxMACH_MT6893)
	if (id == 0) {
...
static int cmdq_bwmon_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < ARRAY_SIZE(bw_mon.smi); i++)
		if (cmdq_bwmon_parse_larb(pdev, i))
			break;
	bw_mon.smi_cnt = i;
	ret = of_property_read_u8(pdev->dev.of_node, "bw_mon_gpr",
		&bw_mon.timer_gpr);
	if (ret) {
		cmdq_msg("%s bw_mon_gpr not find", __func__);
	}
