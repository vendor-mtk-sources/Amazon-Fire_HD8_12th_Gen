...
IMP_ENABLE_MDP_HW_CLOCK(IMG0_IMG_DL_ASYNC2, IMG0_IMG_DL_ASYNC2);
...
IMP_ENABLE_MDP_HW_CLOCK(IMG0_IMG_DL_RELAY2_ASYNC2, IMG0_IMG_DL_RELAY2_ASYNC2);
...
IMP_MDP_HW_CLOCK_IS_ENABLE(IMG0_IMG_DL_ASYNC2, IMG0_IMG_DL_ASYNC2);
...
IMP_MDP_HW_CLOCK_IS_ENABLE(IMG0_IMG_DL_RELAY2_ASYNC2,
	IMG0_IMG_DL_RELAY2_ASYNC2);
...
int32_t cmdq_mdp_reset_with_mmsys(const uint64_t engineToResetAgain)
{
	...
	int engineResetBit[48] = {
		...
		-1,	/* bit 14 : MDP_FAKE_ENG */
		CMDQ_ENG_MDP_AAL0,	/* bit 15 : mdp_AAL0*/
		-1,	/* bit 16 : MDP_DISP_WDMA */
		CMDQ_ENG_MDP_COLOR0,	/* bit 17 : MDP_COLOR0 */
		CMDQ_ENG_MDP_WPEIN,	/* bit 18 : IMG_DL_ASYNC2  */
		-1,	/* bit 19 : MDP_COLOR1  */
		-1,			/* bit 20 : empty20 */
		-1,			/* bit 21 : empty21 */
		-1,			/* bit 22 : empty22 */
...
bool cmdq_mdp_clock_is_on(enum CMDQ_ENG_ENUM engine)
{
	...
	case CMDQ_ENG_MDP_WPEIN:
		return cmdq_mdp_clock_is_enable_IMG_DL_ASYNC2() &&
			cmdq_mdp_clock_is_enable_IMG0_IMG_DL_ASYNC2() &&
			cmdq_mdp_clock_is_enable_IMG0_IMG_DL_RELAY2_ASYNC2();
...
void cmdq_mdp_enable_clock(bool enable, enum CMDQ_ENG_ENUM engine)
{
	...
	case CMDQ_ENG_MDP_WPEIN:
		...
		cmdq_mdp_enable_clock_IMG0_IMG_DL_ASYNC2(enable);
		cmdq_mdp_enable_clock_IMG0_IMG_DL_RELAY2_ASYNC2(enable);
		break;
	case CMDQ_ENG_MDP_RDMA0:
...
void cmdq_mdp_init_module_clk(void)
{
	...
	cmdq_dev_get_module_clock_by_name("mmsys_config",
		"MDP_IMG_DL_RELAY2_ASYNC2",
		&gCmdqMdpModuleClock.clk_IMG0_IMG_DL_RELAY2_ASYNC2);
...
int32_t cmdqMdpClockOn(uint64_t engineFlag)
{
	...
	cmdq_mdp_enable(engineFlag, CMDQ_ENG_MDP_WPEIN);
...
int32_t cmdqMdpResetEng(uint64_t engineFlag)
{
	...
	if (engineFlag & (1LL << CMDQ_ENG_MDP_WPEIN)) {
		/* MDP_CAMIN2 can only reset by mmsys, */
		/* so this is not a "error" */
		cmdq_mdp_reset_with_mmsys((1LL << CMDQ_ENG_MDP_WPEIN));
	}
...
int32_t cmdqMdpClockOff(uint64_t engineFlag)
{
	...
	if (engineFlag & (1LL << CMDQ_ENG_MDP_WPEIN)) {
		if (cmdq_mdp_get_func()->mdpClockIsOn(CMDQ_ENG_MDP_WPEIN)) {
			cmdq_mdp_reset_with_mmsys((1LL << CMDQ_ENG_MDP_WPEIN));
...
{
#ifdef CMDQ_PWR_AWARE
#if IS_ENABLED(CONFIG_MTK_SMI_EXT0)
	if (enable) {
		/* Use SMI clock API */
...
#define MDP_ICC_GET(port) do { \
	...
	if (IS_ERR(path_##port[thread_id]))					\
		CMDQ_ERR("%s port:%s icc fail %ld\n",      \
			__func__, #port, PTR_ERR(path_##port[thread_id]));	\
} while (0)
...
static void *mdp_qos_get_path(u32 thread_id, u32 port)
{
	...
	CMDQ_ERR("%s pmqos invalid port %d thread_id %d\n", __func__, port, thread_id);
	return NULL;
}
