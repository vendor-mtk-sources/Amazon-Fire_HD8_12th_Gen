				...
				loff_t *data)
{
	...
	if (sscanf(
		...
		PK_DBG("write addr = 0x%08x, data = 0x%08x\n",
			sensorReg.RegAddr,
			sensorReg.RegData);
		if(snprintf(mtk_i2c_dump, sizeof(mtk_i2c_dump),
			"addr = 0x%08x, data = 0x%08x\n",
			 sensorReg.RegAddr, sensorReg.RegData) < 0)
			PK_PR_ERR("mtk_i2c_dump snprintf fail\n");
...
static int imgsensor_proc_status_info_read(struct seq_file *m, void *v)
{
	...
	if(snprintf(status_info, sizeof(status_info), "ERR_L0, %x\n",
			*((uint32_t *)(&gimgsensor.status))) < 0)
		PK_PR_ERR("status_info snprintf fail\n");
	seq_printf(m, "%s\n", status_info);
	return 0;
};
