...
#ifdef CMDQ_SECURE_PATH_SUPPORT
        if (handle->pkt->sec_data && handle->secData.is_secure) {
               mdp_ctx.thread[thread].secure = true;
        } else {
               mdp_ctx.thread[thread].secure = false;
        }
#endif
...
static s32 cmdq_mdp_check_engine_waiting_unlock(struct cmdqRecStruct *handle)
{
	...
	for (i = MDP_THREAD_START; i < max_thd; i++) {
		...
		if (mdp_ctx.thread[i].task_count &&
			...
			CMDQ_LOG(
				"sec engine busy %u count:%u engine:%#llx & %#llx is_secure %d, thread_secure:%d\n",
			...
			struct cmdqRecStruct *handle)
{
	...
	CMDQ_MSG("handle:%p mtee:%d dapc:%#llx(%#llx) port:%#llx(%#llx) sec_id:%d\n",
		handle,
...
static void cmdq_mdp_pool_create(void)
{
	...
	mdp_pool.pool = dma_pool_create("mdp", cmdq_dev_get_cmdq(),
...
#ifdef MTK_MDP_MMDVFS_SUPPORT
static void mdp_parse_opp(struct platform_device *pdev, int opp_num,
	u64 **mdp_freq_out, int **mdp_volt_out, u64 **isp_freq_out, int **isp_volt_out)
{
	...
	u64 freq;
	u32 index = 0;
	int opp_table_nums = 2;
	int flag = 0;
	int volt, *volts, num_available, ret;

	num_available = opp_num;

	of_for_each_phandle(&it, ret, pdev->dev.of_node, "my-opp-tables", NULL, 0) {
		np = of_node_get(it.node);

		if (!np) {
			pr_info("%s fail to parse opp:%s\n", __func__, ret);
			return;
		}

		speeds = kzalloc(opp_table_nums * num_available * sizeof(u64), GFP_KERNEL);
		if (!speeds) {
			kfree(speeds);
			return;
		}

		volts = kzalloc(opp_table_nums * num_available * sizeof(u64), GFP_KERNEL);
		if (!volts) {
			kfree(volts);
			return;
		}
		...
		do {
			...
			if (child_np) {
				...
				CMDQ_MSG("test %s %u: %llu %d\n", ret, index, freq, volt);
			}
		} while (child_np);

		if (flag < opp_table_nums && flag == 0) {
			*mdp_freq_out = speeds;
			*mdp_volt_out = volts;
		} else if (flag < opp_table_nums && flag == 1) {
			*isp_freq_out = speeds;
			*isp_volt_out = volts;
		}
		of_node_put(np);
...
static void cmdq_mdp_init_pmqos(struct platform_device *pdev)
{
	...
	if (ret < 0)
		CMDQ_ERR("Failed to get opp table\n");

	/* Get regulator instance by name. */
	mdp_mmdvfs_reg = devm_regulator_get(&pdev->dev, "dvfsrc-vcore");
	if (IS_ERR(mdp_mmdvfs_reg)) {
		CMDQ_ERR("%s devm_regulator_get mdp_mmdvfs_reg: %p return fail: %ld\n",
			__func__, mdp_mmdvfs_reg, PTR_ERR(mdp_mmdvfs_reg));
	}

	mdp_pmqos_opp_num = dev_pm_opp_get_opp_count(&pdev->dev);
	dev_info(&pdev->dev, "%s get opp count:%d\n", __func__, mdp_pmqos_opp_num);

	mdp_volts = kcalloc(mdp_pmqos_opp_num, sizeof(int), GFP_KERNEL);
	isp_volts = kcalloc(mdp_pmqos_opp_num, sizeof(int), GFP_KERNEL);
	mdp_parse_opp(pdev, mdp_pmqos_opp_num, &mdp_pmqos_freq, &mdp_volts,
		&isp_pmqos_freq, &isp_volts);
	for (i = 0; i < mdp_pmqos_opp_num; i++) {
		CMDQ_LOG("mdp_pmqos_freq %llu, mdp_volts %d isp_pmqos_freq %llu, isp_volts %d\n",
			mdp_pmqos_freq[i], mdp_volts[i], isp_pmqos_freq[i], isp_volts[i]);
	}
#endif
}
...
static void mdp_request_voltage(unsigned long frequency, bool is_mdp)
{
	...
	for (index = 0; index < mdp_pmqos_opp_num; index++) {
		if (frequency <= (freqs[index] / 1000000))
			break;
	}
	if (index == mdp_pmqos_opp_num)
		index--;
	low_volt = volts[index];
	low_freq = freqs[index];
	CMDQ_LOG_PMQOS("%s %s frequency %u set_voltage %u freq %u\n",
		__func__, is_mdp ? "mdp" : "isp", frequency, low_volt, low_freq);
...
#define mdp_t(_act_throughput)	\
	min((_act_throughput), (mdp_pmqos_freq[mdp_pmqos_opp_num - 1] / 1000000))

#define isp_t(_act_throughput)	\
	min((_act_throughput), (isp_pmqos_freq[mdp_pmqos_opp_num - 1] / 1000000))
	...
	struct cmdqRecStruct **handle_list, u32 size)
{
	...
	if (!mdp_pmqos_freq || !isp_pmqos_freq) {
		CMDQ_ERR("%s freq not available %p and %p\n",
	...
	CMDQ_LOG_PMQOS(
		"[MDP][%d]act_throughput %d mdp_curr_bandwidth %d, isp_curr_bandwidth %d, max_throughput %d\n",
		thread_id, act_throughput, mdp_curr_bandwidth, isp_curr_bandwidth,
		max_throughput);
	...
	/* update isp bandwidth and clock */
		for (i = 0; i < PMQOS_ISP_PORT_NUM &&
			...
			DP_BANDWIDTH(target_pmqos->qos2_isp_bandwidth[i],
				...
				isp_throughput,
				isp_curr_bandwidth);
			CMDQ_LOG_PMQOS("isp_curr_bandwidth %u", isp_curr_bandwidth);
	...
	if (target_pmqos->mdp_total_datasize) {
		...
		for (i = 0; i < PMQOS_MDP_PORT_NUM
			...
			DP_BANDWIDTH(target_pmqos->qos2_mdp_bandwidth[i],
				...
				mdp_throughput,
				mdp_curr_bandwidth);
			CMDQ_LOG_PMQOS("mdp_curr_bandwidth %u", mdp_curr_bandwidth);
			mtk_icc_set_bw(port_path,
	...
	struct cmdqRecStruct **handle_list, u32 size)
{
	...
	CMDQ_LOG_PMQOS(
		"[MDP][%d]mdp_curr_bandwidth %d isp_curr_bandwidth %d max_throughput %d act_throughput %d %s\n",
		thread_id, mdp_curr_bandwidth, isp_curr_bandwidth,
		max_throughput, act_throughput,
	...
	if (target_pmqos && target_pmqos->mdp_total_datasize) {
		...
		for (i = 0; i < PMQOS_MDP_PORT_NUM &&
			...
			CMDQ_LOG_PMQOS("set bw idx:%u port:%u\n",
				i, port);
...
void cmdq_mdp_dump_rot(const unsigned long base, const char *label)
{
	...
	value[50] = CMDQ_REG_GET32(base + 0x018);
	...
	CMDQ_ERR("VIDO_SOFT_RST: 0x%08x, VIDO_SOFT_RST_STAT: 0x%08x\n",
		value[45], value[46]);
	CMDQ_ERR(
		"VIDO_PVRIC: 0x%08x, VIDO_PENDING_ZERO: 0x%08x, VIDO_FRAME_SIZE: 0x%08x, VIDO_INT_EN: 0x%08x,\n",
