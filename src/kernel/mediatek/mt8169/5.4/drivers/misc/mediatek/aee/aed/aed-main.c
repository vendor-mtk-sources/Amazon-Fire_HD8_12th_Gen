...
static DECLARE_RWSEM(rw_ops_ke_sem);
static DECLARE_RWSEM(rw_ops_ee_sem);
						...
						size_t count, loff_t *f_pos)
{
	...
	down_read(&rw_ops_ee_sem);
	if (!aed_dev.eerec) {
		pr_info("%s fail for invalid kerec\n", __func__);
		up_read(&rw_ops_ee_sem);
		return 0;
	}
...
static unsigned int kedump_get_data_info(int index, struct ipanic_data *data)
{
	...
	header_addr = CONFIG_MTK_AEE_SAVE_DEBUGINFO_ADDR + sizeof(struct kedump_reserved_buffer)
				+ sizeof(struct ipanic_data_header) * index;
	header_size = sizeof(struct ipanic_data_header);
