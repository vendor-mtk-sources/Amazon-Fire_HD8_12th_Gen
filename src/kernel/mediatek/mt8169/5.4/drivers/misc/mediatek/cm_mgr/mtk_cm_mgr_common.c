...
#if IS_ENABLED(CONFIG_MTK_CM_MGR_MT8169)
...
#define UPSTREAM_CPUFREQ_SUPPORT
#endif
#endif
#endif
...
#ifdef UPSTREAM_CPUFREQ_SUPPORT
static struct delayed_work cm_mgr_cpufreq_work;
#endif
...
#ifdef UPSTREAM_CPUFREQ_SUPPORT
/* setting for upstream cpufreq */
#define CPU_NUM_L	(0)
#define CPU_NUM_B	(6)
#define CPU_ONLINE_MAX	(8)
#define CPUFREQ_NUM_OPP	(16)
static unsigned long cpufreq_table_l[CPUFREQ_NUM_OPP];
static unsigned long voltage_table_l[CPUFREQ_NUM_OPP];
static unsigned long cpufreq_table_b[CPUFREQ_NUM_OPP];
static unsigned long voltage_table_b[CPUFREQ_NUM_OPP];
static unsigned int cpu_is_online;
#endif
...
int cm_mgr_to_sspm_command(u32 cmd, int val)
{
	...
	case IPI_CM_MGR_CPUFREQ_TABLE_INDEX:
...
#ifdef UPSTREAM_CPUFREQ_SUPPORT
static int find_cpufreq_opp(int cluster, unsigned long freq)
{
...
static int cpufreq_get_table(void)
{
	...
	if (cpu_online(CPU_NUM_B)) {
		...
		num_opps = dev_pm_opp_get_opp_count(cpu_b_dev);

		for (i = 0, freq = ULONG_MAX; i < num_opps; i++, freq--) {
			opp = dev_pm_opp_find_freq_floor(cpu_b_dev, &freq);
			if (IS_ERR(opp))
				return PTR_ERR(opp);
			voltage_table_b[i] = dev_pm_opp_get_voltage(opp)/1000;
			cpufreq_table_b[i] = (freq)/1000000;
			dev_pm_opp_put(opp);
		}
	}
	if (cpu_online(CPU_NUM_L)) {
		cpu_l_dev = get_cpu_device(CPU_NUM_L);

		num_opps = dev_pm_opp_get_opp_count(cpu_l_dev);

		for (i = 0, freq = ULONG_MAX; i < num_opps; i++, freq--) {
			opp = dev_pm_opp_find_freq_floor(cpu_l_dev, &freq);
			if (IS_ERR(opp))
				return PTR_ERR(opp);
			voltage_table_l[i] = dev_pm_opp_get_voltage(opp)/1000;
			cpufreq_table_l[i] = (freq)/1000000;
			dev_pm_opp_put(opp);
		}
	}

	for (i = 0; i < CPUFREQ_NUM_OPP; i++) {
		qos_sram_write(CM_CPUFREQ_B_VROC_TABLE, voltage_table_b[i]);
		qos_sram_write(CM_CPUFREQ_B_FREQ_TABLE, cpufreq_table_b[i]);
		qos_sram_write(CM_CPUFREQ_L_VROC_TABLE, voltage_table_l[i]);
		qos_sram_write(CM_CPUFREQ_L_FREQ_TABLE, cpufreq_table_l[i]);
		cm_mgr_to_sspm_command(IPI_CM_MGR_CPUFREQ_TABLE_INDEX, i);
	}

	for (i = 0; i < CPUFREQ_NUM_OPP; i++) {
		pr_debug("voltage_table_l[%d] = %ld, cpufreq_table_l[%d] = %ld\n",
		     i, voltage_table_l[i], i, cpufreq_table_l[i]);
	}

	for (i = 0; i < CPUFREQ_NUM_OPP; i++) {
		pr_debug("voltage_table_b[%d] = %ld, cpufreq_table_b[%d] = %ld\n",
		     i, voltage_table_b[i], i, cpufreq_table_b[i]);
	}
	return 0;
}

void cpufreq_cm_setting(void)
{
	unsigned int cpufreq, i;

	for (i = 0; i < CPU_ONLINE_MAX; i++)
		if (cpu_online(i))
			cpu_is_online |= (0x1 << i);
		else
			cpu_is_online &= ~(0x1 << i);

	qos_sram_write(CM_CPUFREQ_ONLINE, cpu_is_online);
	...
	if (cpu_online(CPU_NUM_L)) {
		cpufreq = cpufreq_get(CPU_NUM_L)/1000;
		qos_sram_write(CM_CPUFREQ_L_CUR_FREQ, cpufreq);
		qos_sram_write(CM_CPUFREQ_L_CUR_FREQ_OPP, find_cpufreq_opp(0, cpufreq));
	}
}

void cm_mgr_cpufreq_process(struct work_struct *work)
{
	cpufreq_get_table();
	cpufreq_cm_setting();
...
#ifdef UPSTREAM_CPUFREQ_SUPPORT
	INIT_DELAYED_WORK(&cm_mgr_cpufreq_work, cm_mgr_cpufreq_process);
	schedule_delayed_work(&cm_mgr_cpufreq_work, msecs_to_jiffies(5000));
#endif
	return 0;
}
