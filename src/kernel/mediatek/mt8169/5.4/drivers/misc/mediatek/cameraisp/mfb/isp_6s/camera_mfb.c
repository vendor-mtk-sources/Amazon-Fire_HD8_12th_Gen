...
static inline void MFB_Prepare_Enable_ccf_clock(void)
{
	 ...
	 */
	pm_runtime_get_sync(MFB_devs[MSF_DEV_NODE_IDX].dev);
	ret = mtk_smi_larb_get(MFB_devs[MSF_DEV_NODE_IDX].larb);
	if (ret)
...
static signed int MFB_probe(struct platform_device *pDev)
{
	...
	_mfb_dev = krealloc(MFB_devs,
		sizeof(struct MFB_device) * MFB_NODE_NUM, GFP_KERNEL);
	if (!_mfb_dev) {
	...
	if (!strcmp(pDev->dev.of_node->name, "msf_b"))
		MFB_dev = &(MFB_devs[MSF_DEV_NODE_IDX]);
	if (!strcmp(pDev->dev.of_node->name, "mss_b"))
	...
	if (!strcmp(pDev->dev.of_node->name, "msf_b")) {/*YWtodo*/
		...
		if (!pm_runtime_enabled(MFB_dev->dev))
			goto EXIT;
		/*gki smi larb*/
		node = of_parse_phandle(MFB_dev->dev->of_node, "mediatek,larbs", 0);
		if (!node)
			return -EINVAL;
