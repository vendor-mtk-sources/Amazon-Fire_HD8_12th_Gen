...
static DEVICE_ATTR(error, MDP_ATTR_MODE, cmdq_core_print_error,
	cmdq_driver_dummy_write);
static DEVICE_ATTR(log_level, MDP_ATTR_MODE, cmdq_core_print_log_level,
	cmdq_core_write_log_level);
static DEVICE_ATTR(profile_enable, MDP_ATTR_MODE,
...
static DEVICE_ATTR(instruction_count_level, MDP_ATTR_MODE,
...
int mtk_mdp_comp_init(struct device *dev,
		...
		struct mtk_mdp_comp *comp)
{
	...
	larb_node = of_parse_phandle(node, "mediatek,mailbox-gce", 0);
	if (!larb_node)
		return -EINVAL;
	...
	comp->cmdq_dev = larb_pdev;

	return 0;
}

static int mdp_cmdq_probe(struct platform_device *pDevice)
{
	int status;
	...
	node = of_get_next_child(pDevice->dev.of_node, NULL);
	comp = devm_kzalloc(&pDevice->dev, sizeof(*comp), GFP_KERNEL);
	if (!comp) {
		CMDQ_ERR("devm_kzalloc comp failed!\n");
		of_node_put(node);
		return -ENOMEM;
	}

	ret = mtk_mdp_comp_init(&pDevice->dev, pDevice->dev.of_node, comp);

	/* init cmdq device related data */
	cmdq_dev_init(comp->cmdq_dev, pDevice);
	...
	status = alloc_chrdev_region(&gMdpDevNo, 1, 1,
