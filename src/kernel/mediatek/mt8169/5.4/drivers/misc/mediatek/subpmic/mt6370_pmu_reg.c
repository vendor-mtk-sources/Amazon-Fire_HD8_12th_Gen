...
int mt6370_pmu_read_device(void *i2c, u32 addr, int len, void *dst)
{
	return i2c_smbus_read_i2c_block_data(i2c, addr, len, dst);
}
EXPORT_SYMBOL(mt6370_pmu_read_device);

int mt6370_pmu_write_device(void *i2c, u32 addr, int len, const void *src)
{
	return i2c_smbus_write_i2c_block_data(i2c, addr, len, src);
}
EXPORT_SYMBOL(mt6370_pmu_write_device);
			      ...
			      int len, u8 *dest)
{
	...
	if (dbg_log_en) {
		for(i = 0; i < len; i++) {
			data = *(dest+i);
			mt_dbg(chip->dev, "%s: reg %02x size %d dest %02x\n",
					  __func__, addr+i, len, data);
		}
	}
	return ret;
#endif /* #ifdef CONFIG_RT_REGMAP */
			       ...
			       int len, const u8 *src)
{
	...
	if (dbg_log_en) {
		for(i = 0; i < len; i++) {
			data = *(src+i);
			mt_dbg(chip->dev, "%s: reg %02x size %d src %02x\n",
					  __func__, addr+i, len, data);
		}
	}
	mutex_lock(&chip->io_lock);
