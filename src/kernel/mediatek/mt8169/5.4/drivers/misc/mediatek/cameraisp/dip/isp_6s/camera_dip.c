...
static signed int DIP_probe(struct platform_device *pDev)
{
	...
	if (!strcmp(pDev->dev.of_node->name, "imgsys_config")) {
		pm_runtime_enable(dip_devs->dev);
		/* parse larb node*/
		node_larb9 = of_parse_phandle(pDev->dev.of_node, "mediatek,larbs", 0);
		node_larb11 = of_parse_phandle(pDev->dev.of_node, "mediatek,larbs", 1);
