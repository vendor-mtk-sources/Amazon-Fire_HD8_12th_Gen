				...
				unsigned long event, void *data)
{
	...
	if (pval.intval) {
		if ((tval.intval == POWER_SUPPLY_TYPE_WIRELESS ||
			tval.intval == POWER_SUPPLY_TYPE_WIRELESS_5W ||
			tval.intval == POWER_SUPPLY_TYPE_WIRELESS_10W)) {
			is_pre_wpc = true;
			return NOTIFY_DONE;
		}
...
static int mtk_usb_extcon_probe(struct platform_device *pdev)
{
	...
	if (!extcon->extcon_wq)
		dev_err(dev, "%s: create extcon work queue fail\n", __func__);
	else
		dev_err(dev, "%s: create extcon work queue success\n", __func__);
