				...
				unsigned long curInterval, char msg_buf[])
{
	...
	if ((local_bit & (1 << cpu)) == 0) {
		...
		/* aee_rr_rec_wdk_kick_jiffies(jiffies); */
			...
			(sched_clock() - wk_lasthpg_t[cpu] > curInterval * 1000)) {
		g_hang_detected = 1;
		dump_timeout = 1;
	}
	...
	snprintf(msg_buf, WK_MAX_MSG_SIZE,
	 ...
	 cpu, local_bit, get_check_bit(), local_bit & get_check_bit(), lasthpg_cpu, lasthpg_act,
	...
	for (tmp = 0; tmp < CPU_NR; tmp++) {
		if ((atomic_read(&plug_mask) & (1 << tmp)) || (tmp == cpu))
			cpus_kick_bit |= (1 << tmp);
	}
	...
	for (tmp = 0; tmp < 2000; tmp++) {
		...
		if(!dump_timeout) {
			...
			break;
					...
					get_check_bit() || (g_hang_detected == 0)) {
			spin_unlock(&lock);
			dump_timeout = 0;
			break;
		}
