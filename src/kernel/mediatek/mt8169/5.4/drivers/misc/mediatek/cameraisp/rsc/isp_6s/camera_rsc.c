...
#define IRQ_LOG_KEEPER(irq, ppb, logT, fmt, ...) do {\
	...
	if (avaLen > 1) {\
		ret_chk = snprintf((char *)(pDes), avaLen, fmt,\
			##__VA_ARGS__);   \
		if (ret_chk < 0) {\
			LOG_ERR("IRQ_LOG_KEEPER snprintf ERROR : %d", ret_chk);\
		} \
		...
		if (pSrc->_cnt[ppb][logT] != 0) {\
			...
			ret_chk = snprintf((char *)(pDes), avaLen, fmt, ##__VA_ARGS__);  \
			if (ret_chk < 0) {\
			  LOG_ERR("IRQ_LOG_KEEPER snprintf ERROR : %d", ret_chk);\
			} \
			while (*ptr++ != '\0') {\
...
void rsc_cmdq_cb_destroy(struct cmdq_cb_data data)
{
	if (data.data) {
		...
		for (i = 0; i < NUM_BASEADDR; i++) {
			if ((((struct tee_mmu *)(data.data))+i) != NULL)
