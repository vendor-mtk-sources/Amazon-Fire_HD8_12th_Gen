...
#define SENINF_WR32(addr, data) do {    \
...
MINT32 seninf_dump_reg(void)
{
	...
	LOG_DBG(
	"seninf_top: SENINF_TOP_MUX_CTRL_0(0x%x) SENINF_TOP_MUX_CTRL_1(0x%x)\n",
	     SENINF_RD32(gseninf.pseninf_base[0] + 0x0010),
	     SENINF_RD32(gseninf.pseninf_base[0] + 0x0014));

	LOG_DBG(
	"seninf_cam_mux: SENINF_CAM_MUX_CTRL1[addr : 0x%x](0x%x), SENINF_CAM_MUX_CTRL1(0x%x) SENINF_CAM_MUX_CTRL2(0x%x) SENINF_CAM_MUX_IRQ_EN(0x%x) SENINF_CAM_MUX_IRQ_STATUS(0x%x)\n",
	     gseninf.pseninf_base[0] + 0x0400,
...
static MINT32 seninf_open(struct inode *pInode, struct file *pFile)
{
	...
	for (i = 0; i < pseninf->pwr_devs_cnt; i++)
		pm_runtime_get_sync(pseninf->pwr_devs[i]);
#endif
...
static MINT32 seninf_release(struct inode *pInode, struct file *pFile)
{
	...
	#ifdef SENINF_USE_RPM
	for (i = 0; i < pseninf->pwr_devs_cnt; i++)
		pm_runtime_put_sync(pseninf->pwr_devs[i]);
	#endif
#endif
...
static MINT32 seninf_mmap(struct file *pFile, struct vm_area_struct *pVma)
{
	...
	/*LOG_DBG("mmap: vm_pgoff(0x%lx),pfn(0x%x),phy(0x%lx),
			...
			unsigned int cmd, unsigned long arg)
{
	...
	if (_IOC_DIR(cmd) != _IOC_NONE) {
		...
		if (pbuff == NULL) {
			LOG_NOTICE("ioctl allocate mem failed\n");
			ret = -ENOMEM;
		...
		if (_IOC_WRITE & _IOC_DIR(cmd)) {
			if (copy_from_user(pbuff,
				...
				LOG_NOTICE("ioctl copy from user failed\n");
				ret = -EFAULT;
				goto SENINF_IOCTL_EXIT;
			}
		}
	...
	if ((_IOC_READ & _IOC_DIR(cmd)) && copy_to_user((void __user *)arg,
			pbuff, _IOC_SIZE(cmd))) {
		LOG_NOTICE("[CAMERA SENSOR] ioctl copy to user failed\n");
		ret = -EFAULT;
		goto SENINF_IOCTL_EXIT;
	}
...
#ifdef SENINF_USE_RPM
	pseninf->pwr_devs_cnt = of_count_phandle_with_args(pseninf->dev->of_node,
													  "power-domains",
													  "#power-domain-cells");
	if (pseninf->pwr_devs_cnt > SENINF_MAX_CLK_NUM) {
		LOG_NOTICE("[ERROR]power domain(%d) count more than SENINF_MAX_CLK_NUM(%d)",
					pseninf->pwr_devs_cnt, SENINF_MAX_CLK_NUM);
		pseninf->pwr_devs_cnt = SENINF_MAX_CLK_NUM;
	}

	for (i = 0; i < pseninf->pwr_devs_cnt; i++)
		pseninf->pwr_devs[i] = dev_pm_domain_attach_by_id(pseninf->dev, i);
...
static inline MINT32 seninf_reg_of_dev(struct SENINF *pseninf)
{
	...
	for (i = 0; i < SENINF_MAX_NUM; i++) {
		...
		if (!node) {
			LOG_NOTICE("find mediatek,seninf%d node failed!!!\n",
			i + 1);
			return -ENODEV;
		}
		pseninf->pseninf_base[i] = of_iomap(node, 0);
		if (!pseninf->pseninf_base[i]) {
			LOG_NOTICE("unable to map SENINF%d_BASE registers!!!\n",
			i + 1);
			return -ENODEV;
		}
		LOG_DBG("SENINF%d_BASE: %px\n", i + 1, pseninf->pseninf_base[i]);
	}
