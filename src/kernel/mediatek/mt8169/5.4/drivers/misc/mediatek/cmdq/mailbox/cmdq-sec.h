...
typedef void (*sec_dump_thread_all)(void *mbox_cmdq);
struct cmdq_sec_helper_fp {
	...
	sec_dump_thread_all sec_dump_thread_all;
};
...
#define SEC_MAX_HANDLE_NUM 10

struct cmdq_sec_data {
	/* address metadata, used to translate secure buffer PA
	 * related instruction in secure world
	 */
	struct dma_buf *dma_buffer[SEC_MAX_HANDLE_NUM];
	struct dma_buf_attachment *dma_attach[SEC_MAX_HANDLE_NUM];
	struct sg_table *table[SEC_MAX_HANDLE_NUM];
