...
#if IS_ENABLED(CONFIG_INTERCONNECT_MTK_MMQOS_MT8169) && IS_ENABLED(CONFIG_MTK_MMDVFS)
	#define CONFIG_MTK_QOS_SUPPORT_ENABLE 1
#else
	#define CONFIG_MTK_QOS_SUPPORT_ENABLE 0
	#define EP_STAGE
	#define EP_PMQOS
#endif
//#define EP_STAGE
...
#define DEFAULT_ISP_CLK	312

#if IS_ENABLED(CONFIG_MTK_QOS_SUPPORT_ENABLE)
#ifndef EP_STAGE
...
#include <mt8169-larb-port.h>
#include <dt-bindings/memory/mt8169-larb-port.h>
#endif//#ifdef CAMERA_ISP_MT6873

#include <dt-bindings/interconnect/mtk,mmqos.h>

#endif //#ifndef EP_STAGE
#else
#ifndef EP_STAGE
//#include <mmdvfs_mgr.h> Not use in k5.4
#endif //#ifndef EP_STAGE
#endif
...
void mtk_pmqos_add(struct device *dev, enum ISP_IRQ_TYPE_ENUM module, u32 portID)
{
	...
	case ISP_IRQ_TYPE_INT_CAMSV_0_ST:
		...
		case _camsv_ufeo_:
			gSV_BW_REQ[module][portID] = mtk_icc_get(dev,
				MASTER_LARB_PORT(M4U_PORT_L13_CAM_CAMSV_1),
				SLAVE_COMMON(0));
			break;
		default:
	...
	case ISP_IRQ_TYPE_INT_CAMSV_1_ST:
		...
		case _camsv_imgo_:
			gSV_BW_REQ[module][portID] = mtk_icc_get(dev,
				MASTER_LARB_PORT(M4U_PORT_L13_CAM_CAMSV_2),
				SLAVE_COMMON(0));
			break;
		case _camsv_ufeo_:
			gSV_BW_REQ[module][portID] = mtk_icc_get(dev,
				MASTER_LARB_PORT(M4U_PORT_L13_CAM_CAMSV_3),
				SLAVE_COMMON(0));
			break;
		default:
			LOG_NOTICE("modlue(%d): unsupported port:%d\n", module, portID);
	...
	case ISP_IRQ_TYPE_INT_CAMSV_3_ST:
		...
		case _camsv_imgo_:
			gSV_BW_REQ[module - ISP_IRQ_TYPE_INT_CAMSV_START_ST][portID] =
				mtk_icc_get(dev, MASTER_LARB_PORT(M4U_PORT_L13_CAM_CAMSV_4),
				SLAVE_COMMON(0));
			break;
		default:
			LOG_NOTICE("modlue(%d): unsupported port:%d\n", module, portID);
	...
	case ISP_IRQ_TYPE_INT_CAMSV_5_ST:
		...
		case _camsv_imgo_:
			gSV_BW_REQ[module - ISP_IRQ_TYPE_INT_CAMSV_START_ST][portID] =
				mtk_icc_get(dev, MASTER_LARB_PORT(M4U_PORT_L13_CAM_CAMSV_5),
				SLAVE_COMMON(0));
			break;
		}
		break;
	...
	case ISP_IRQ_TYPE_INT_CAMSV_7_ST:
		...
		case _camsv_imgo_:
			gSV_BW_REQ[module - ISP_IRQ_TYPE_INT_CAMSV_START_ST][portID] =
				mtk_icc_get(dev, MASTER_LARB_PORT(M4U_PORT_L13_CAM_CAMSV_6),
				SLAVE_COMMON(0));
			break;
		}
		break;
...
#endif //#if IS_ENABLED(CONFIG_MTK_QOS_SUPPORT_ENABLE)
	...
	unsigned int *pvalue)
{
	...
	case E_CLK_UPDATE:
	{
		...
		break;
#endif //IS_ENABLED(CONFIG_MTK_QOS_SUPPORT_ENABLE)
	case E_CLK_SUPPORTED:
	{
#ifdef EP_PMQOS
		*pvalue = target_clk = DEFAULT_ISP_CLK;

		LOG_DBG("1:DFS Clk_0:%d\n", pvalue[0]);
		return 1;
#else
		int num_available, i = 0;
		u32 *speeds = NULL;
		struct dev_pm_opp *opp;
		unsigned long freq;
		#define STR_SIZE (128)
		...
		LOG_INF("num_available = %d", num_available);
		if (num_available > 0) {
