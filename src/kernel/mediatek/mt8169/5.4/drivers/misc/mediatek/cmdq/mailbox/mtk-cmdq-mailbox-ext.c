...
void cmdq_dump_core(struct mbox_chan *chan)
{
	...
	if (!atomic_read(&cmdq->usage)) {
		cmdq_util_msg("cmdq:%#x try dump core without enable chan 0x%p",
	...
	cmdq_util_msg(
		"irq:%#x loaded:%#x cycle:%#x thd timer:%#x mask:%#x en:%#x chan 0x%p",
...
static int cmdq_probe(struct platform_device *pdev)
{
	...
	dev_info(dev, "%s sw_ddr_en\n", __func__, cmdq->sw_ddr_en);

	cmdq_mmp_init();

#if IS_ENABLED(CONFIG_MTK_CMDQ_MBOX_EXT)
	cmdq->hwid = cmdq_util_controller->track_ctrl(
		cmdq, cmdq->base_pa, false);
#endif
...
static const struct of_device_id cmdq_of_ids[] = {
	...
	{.compatible = "mediatek,mt8169-gce", .data = (void *)&gce_plat_v4},
	{}
};
