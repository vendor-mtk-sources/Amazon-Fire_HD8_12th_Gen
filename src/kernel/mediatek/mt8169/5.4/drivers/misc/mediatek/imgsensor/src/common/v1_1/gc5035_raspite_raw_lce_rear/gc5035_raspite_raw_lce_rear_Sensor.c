 ...
 * Filename:
 * ---------
 *     gc5035_raspite_raw_lce_rear_Sensor.c
 *
 * Project:
 * --------
 *     ALPS
...
#include "gc5035_raspite_raw_lce_rear_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC5035_RASPITE_LCE_REAR_SENSOR_ID,
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id() + 1;
			...
			LOG_INF("[gc5035_camera_sensor]get_imgsensor_id:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id() +1;
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("[gc5035_camera_sensor]open:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
			}
			LOG_INF("[gc5035_camera_sensor]open:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
UINT32 GC5035_RASPITE_LCE_REAR_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
