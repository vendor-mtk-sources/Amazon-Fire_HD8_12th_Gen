				       ...
				       unsigned long arg)
{
	...
	if (err) {
		pr_info("access error: %08X, (%2d, %2d)\n", cmd,
	...
	case GSENSOR_IOCTL_INIT:
		...
		if (accel_factory.fops != NULL &&
			...
			if (err < 0) {
				pr_info("GSENSOR_IOCTL_INIT fail!\n");
				return -EINVAL;
			}
	...
	case GSENSOR_IOCTL_SET_CALI:
		...
		if (accel_factory.fops != NULL &&
			...
			if (err < 0) {
				pr_info("GSENSOR_IOCTL_SET_CALI FAIL!\n");
				return -EINVAL;
			}
		} else {
	...
	case GSENSOR_IOCTL_CLR_CALI:
		if (accel_factory.fops != NULL &&
			...
			if (err < 0) {
				pr_info("GSENSOR_IOCTL_CLR_CALI FAIL!\n");
				return -EINVAL;
			}
		} else {
			pr_info("GSENSOR_IOCTL_CLR_CALI NULL\n");
			return -EINVAL;
		}
		return 0;
	case GSENSOR_IOCTL_GET_CALI:
		if (accel_factory.fops != NULL &&
			...
			if (err < 0) {
				pr_info("GSENSOR_IOCTL_GET_CALI FAIL!\n");
				return -EINVAL;
			}
		} else {
			pr_info("GSENSOR_IOCTL_GET_CALI NULL\n");
			return -EINVAL;
		}
	...
	case GSENSOR_IOCTL_ENABLE_CALI:
		if (accel_factory.fops != NULL &&
			...
			if (err < 0) {
				pr_info("GSENSOR_IOCTL_ENABLE_CALI FAIL!\n");
				return -EINVAL;
			}
		} else {
			pr_info("GSENSOR_IOCTL_ENABLE_CALI NULL\n");
			return -EINVAL;
		}
		return 0;
	case GSENSOR_IOCTL_SELF_TEST:
		if (accel_factory.fops != NULL &&
			...
			if (err < 0) {
				pr_info("GSENSOR_IOCTL_SELF_TEST FAIL!\n");
				return -EINVAL;
			}
		} else {
			pr_info("GSENSOR_IOCTL_SELF_TEST NULL\n");
			return -EINVAL;
		}
		return 0;
	default:
					...
					unsigned int cmd, unsigned long arg)
{
	...
	default:
		pr_info("compat_ion_ioctl : No such command!! 0x%x\n", cmd);
		return -ENOIOCTLCMD;
	}
}
#endif
...
int accel_factory_device_register(struct accel_factory_public *dev)
{
	...
	if (err) {
		pr_info("accel_factory_device register failed\n");
		err = -1;
	}
	return err;
}
