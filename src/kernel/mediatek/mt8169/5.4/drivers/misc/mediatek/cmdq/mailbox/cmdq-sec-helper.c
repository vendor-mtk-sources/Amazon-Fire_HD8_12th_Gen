...
#ifdef CMDQ_FPGA_EARLY_PORTING
#define CMDQ_PREDUMP_TIMEOUT_MS		2000
#else
	...
	const u64 base, const u32 offset, const u32 size, const u32 port)
{
	...
	if (idx < max) {
		...
		cmdq_err("meta + idx:%d make out of bounds write max:%d", idx, max);
		return -ENOMEM;
	}
	return 0;
}
	...
	const enum cmdq_sec_meta_type meta_type)
{
	...
	ret = cmdq_sec_check_sec(pkt);
	if (ret < 0) {
		cmdq_err("cmdq_sec_check_sec failed");
		return ret;
	}
	cmdq_log(
...
void cmdq_sec_pkt_set_mtee(struct cmdq_pkt *pkt, const bool enable, const int32_t sec_id)
{
	...
	cmdq_log("%s pkt:%p mtee:%d sec_id:%d\n",
	...
	u32 count, void *meta_array)
{
	...
	size_t size;
	s32 ret;
	struct cmdq_sec_addr_meta *sec_addr_meta;
#ifndef CONFIG_VIDEO_MEDIATEK_MDP_VCU
	int i;
	...
	if (!count || (count >= CMDQ_IWC_MAX_ADDR_LIST_LENGTH)) {
		cmdq_err("Count[%d] is greater than [%d]!",
                       count, CMDQ_IWC_MAX_ADDR_LIST_LENGTH);
		return -EINVAL;
	}
	ret = cmdq_sec_check_sec(pkt);
	if (ret < 0)
	...
	sec_addr_meta = (struct cmdq_sec_addr_meta *)(pkt_meta_array);

#ifndef CONFIG_VIDEO_MEDIATEK_MDP_VCU
	for (i = 0; ((i < count) && (i < SEC_MAX_HANDLE_NUM)); i++) {
		dma_buffer = dma_buf_get(sec_addr_meta->baseHandle);
		if (IS_ERR(dma_buffer)) {
			cmdq_err("MDP secure handle[%d] get dma_buffer failed[%d]!",
				sec_addr_meta->baseHandle, PTR_ERR(dma_buffer));
			return -EINVAL;
		}
		dma_attach = dma_buf_attach(dma_buffer, pkt->dev);
		table = dma_buf_map_attachment(dma_attach, 0);
		dma_addr = sg_dma_address(table->sgl);
		sec_addr_meta->baseHandle = dma_addr;
		if (dma_addr) {
			data->dma_buffer[i] = dma_buffer;
			data->dma_attach[i] = dma_attach;
			data->table[i] = table;
		} else {
			dma_buf_unmap_attachment(dma_attach, table, 0);
			dma_buf_detach(dma_buffer, dma_attach);
			dma_buf_put(dma_buffer);
			cmdq_err("MDP secure handle translate failed!");
		}
		sec_addr_meta += 1;
	}
#endif
...
int cmdq_sec_pkt_wait_complete(struct cmdq_pkt *pkt)
{
	...
	do {
		...
		cmdq_msg("===== SW timeout Pre-dump sec %hhu thd:%d chan 0x%p =====",
			cnt,
			thread_id,
			client->chan);
		cnt++;
