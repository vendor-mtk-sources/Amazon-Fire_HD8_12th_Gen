...
static void liquid_report_event(struct liquid *liquid, int event)
{
	...
	if (pre_event != event) {
		...
		now_ts = ktime_to_timespec(ktime_now);
 ...
 * HW description:
 *     switch:
 *         SWITCH_CTRL1:(PINMUX_GPIO45__FUNC_GPIO45)
 *             0: SBU2
 *             1: SBU1
 *         SWITCH_CTRL2(PINMUX_GPIO48__FUNC_GPIO48)
 *             0:ADC2 <--> ADC_BAT_MUX
 *             1:ADC2 <--> SBU1/SBU2
 *         Lquid_Detection1(PINMUX_GPIO59__FUNC_GPIO59)
 *         Lquid_Detection2(PINMUX_GPIO58__FUNC_GPIO58)
 *
 *     adc:ADC1_BAT_ID(AUXIN1)
 *     liquid ID:GPIO26 GPIO28
 *          00:init fusb251
 *          01:init max20342
 *          10:init amzn_ld
 */
...
static int liquid_probe(struct platform_device *pdev)
{
	...
	g_liquid = liquid;
	ktime_now =  ktime_get_boottime();
	liquid->event_ts = ktime_to_timespec(ktime_now);
