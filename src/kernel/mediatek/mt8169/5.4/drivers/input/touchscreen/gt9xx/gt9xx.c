...
write_exit:
	...
	if (msg.buf && tmp_len >= sizeof(put_buf)) {
		kfree(msg.buf);
		msg.buf = NULL;
	}
	return r;
}
EXPORT_SYMBOL(gtp_i2c_write);
...
EXPORT_SYMBOL(gtp_work_control_enable);
