...
#define GOODIX_FIRMWARE_FILE_NAME       "goodix_9xx_firmware.bin"
#define GOODIX_CONFIG_FILE_NAME         "goodix_config.cfg"
/*sensor id = 0*/
#define GOODIX_CFG_TRULY "goodix_cfg_TRULY.cfg"
/*sensor id = 3*/
#define GOODIX_CFG_WALLY "goodix_cfg_WALLY.cfg"
...
static int gup_update_config(struct i2c_client *client)
{
	...
	if (ret < 0) {
		dev_err(&client->dev, "Check source failed\n");
		return -EFAULT;
	}
	dev_info(&client->dev, "Config file name %s\n", config_file_name);
	...
	u8 **file_name)
{
	...
	dev_err(&client->dev, "Sensor Id = %d\n", fw_info->sensor_id);

	if (fw_info->sensor_id == GTP_TP_TRULY) {
		dev_err(&client->dev, "source is TRULY\n");
		*file_name = GOODIX_CFG_TRULY;
	} else if (fw_info->sensor_id == GTP_TP_WALLY) {
		dev_err(&client->dev, "source is WALLY\n");
		*file_name = GOODIX_CFG_WALLY;
	} else {
		/* Invalid sensor id*/
		dev_err(&client->dev, "Invalid sensor, don't set config source\n");
		return -EINVAL;
	}
