				...
				u8 *pre_buf, u32 pre_buf_len)
{
	...
	if (touch_num > 0) {
		...
		if (point_type == POINT_TYPE_STYLUS ||
			...
			if (ret) {
				ts_err("touch data checksum error");
				ts_err("data:%*ph", BYTES_PER_POINT * 2 + 2,
			...
			if (ret) {
				ts_err("touch data checksum error");
				ts_err("data:%*ph", touch_num * BYTES_PER_POINT + 2,
						&buffer[IRQ_EVENT_HEAD_LEN]);
				return -EINVAL;
			}
		}
	...
	if (touch_num > 0 && (point_type == POINT_TYPE_STYLUS
				|| point_type == POINT_TYPE_STYLUS_HOVER)) {
		/* stylus info */
		ts_event->pen_data.pen_type = point_type;
		if (pre_finger_num) {
