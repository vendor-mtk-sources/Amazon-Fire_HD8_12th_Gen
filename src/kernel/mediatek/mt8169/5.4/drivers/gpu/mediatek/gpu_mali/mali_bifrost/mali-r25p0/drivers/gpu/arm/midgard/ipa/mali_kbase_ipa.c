...
static unsigned long mtk_static_power(struct devfreq *devfreq,
...
static unsigned long mtk_dynamic_power(struct devfreq *devfreq,
...
static int mtk_real_power(struct devfreq *df, u32 *power,
			      unsigned long freqHz, unsigned long voltage_mv)
{
	int opp_idx;
	u64 total_time;
...
#endif
	.get_static_power = &mtk_static_power,
	.get_dynamic_power = &mtk_dynamic_power,
#if defined(CONFIG_MALI_PWRSOFT_765) || \
	LINUX_VERSION_CODE >= KERNEL_VERSION(4, 10, 0)
	.get_real_power = &mtk_real_power,
#endif
};
