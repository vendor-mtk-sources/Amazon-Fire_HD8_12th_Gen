...
#if 0 //kkk TODO disable for 8169 for build fail IS_ENABLED(CONFIG_MTK_FPSGO_V3)
		...
		struct GED_BRIDGE_OUT_GPU_HINT_TO_CPU *out)
{
	...
	out->boost_flag = fpsgo_notify_gpu_block(in->tid, in->i32BridgeFD, in->hint);
#else
	out->boost_flag = -1; //not support fpsgo
#endif
	out->eError = GED_OK;
