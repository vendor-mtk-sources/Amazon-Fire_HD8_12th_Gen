...
int mtk_dprec_logger_pr(unsigned int type, char *fmt, ...)
{
	...
	if (buf) {
		...
		rem_nsec = do_div_DDP(time, 1000000000);
		n += snprintf(buf + n, len - n, "[%5lu.%06lu]",
			      (unsigned long)time, div_s64(rem_nsec, 1000));
...
void ddic_dsi_send_cmd_test(unsigned int case_num)
{
	...
	if (!cmd_msg) {
		DDPMSG("%s cmd_msg allocate fail\n", __func__);
		return;
	}
...
static void process_dbg_opt(const char *opt)
{
	...
	if (strncmp(opt, "helper", 6) == 0) {
		...
		if (strncmp(opt + 7, "on", 2) == 0)
			...
			g_mobile_log = 0;
	} else if (strncmp(opt, "ovlcloselayer", 13) == 0) {
		struct drm_crtc *crtc;
		unsigned int crcs;
		struct mtk_drm_private *priv;
		struct mtk_ddp_comp *comp_ovl;

		drm_for_each_crtc(crtc, drm_dev) {
			if (!crtc) {
				DDPDBG("find crtc fail\n");
				continue;
			}
			priv = crtc->dev->dev_private;
			comp_ovl = priv->ddp_comp[DDP_COMPONENT_OVL0];
			crcs = readl(comp_ovl->regs + 0x2c);
			crcs = crcs & 0xFFFFFFE0;
			writel(crcs, comp_ovl->regs + 0x2c);
		}
	...
	} else if (strncmp(opt, "dumpdsireg", 10) == 0) {
		struct mtk_ddp_comp *comp;
		struct drm_crtc *crtc;
		struct mtk_drm_crtc *mtk_crtc;
		...
		DDPINFO("Enter set_lp_cmd! dbg_buf=%d\n", strlen(dbg_buf));
		...
		DDPINFO("Enter get_lp_cmd! dbg_buf=%d\n", strlen(dbg_buf));
...
#define SPRINTF_DEV_ATTR(fmt, arg...) \
		...
		pr_notice("esd_check enable=%d %s\n", enable, __func__);
		mtk_disp_esd_check_switch(crtc, enable);
		...
		if (drm_dev != NULL && drm_dev->dev != NULL)
