...
#include <linux/pm_wakeup.h>
	...
	#define OVL_LAYER_NR 4L
#else
	#define OVL_LAYER_NR 7L
#endif
...
#define DISP_SLOT_OVL_CRC (DISP_SLOT_CUR_BL_IDX + 0x4)

/* TODO: figure out Display pipe which need report PMQOS BW */
#define DISP_SLOT_SIZE (DISP_SLOT_OVL_CRC + 0x4)
...
#ifdef CONFIG_ARM64
#define DIV_ROUND_UP_DDP DIV_ROUND_UP
#define do_div_DDP do_div
#else
#define	DIV_ROUND_UP_DDP(n, d)({\
			 div_s64(((n) + (d) - 1), (d)); \
		})
#define do_div_DDP(n, d) ({\
			s32 __rem; \
...
struct mtk_drm_crtc {
	...
	atomic_t cmdq_flush_cb_done;
	wait_queue_head_t cmdq_flush_cb_wq;
