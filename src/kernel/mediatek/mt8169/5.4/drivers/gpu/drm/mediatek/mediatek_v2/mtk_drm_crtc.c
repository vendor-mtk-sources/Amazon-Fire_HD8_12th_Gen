...
void mtk_drm_crtc_analysis(struct drm_crtc *crtc)
{
	...
	case MMSYS_MT6853:
		...
		break;
	case MMSYS_MT8169:
		mmsys_config_dump_analysis_mt8169(mtk_crtc->config_regs);
		mutex_dump_analysis_mt8169(mtk_crtc->mutex[0]);
		break;
	default:
		pr_info("%s mtk drm not support mmsys id %d\n",
			__func__, priv->data->mmsys_id);
...
static int mtk_crc_parse_source(const char *src_name, bool *enabled)
{
	int ret = 0;
...
int mtk_crtc_verify_crc_source(struct drm_crtc *crtc, const char *src_name,
				size_t *values_cnt)
{
	bool enabled;

	if (mtk_crc_parse_source(src_name, &enabled) < 0) {
...
int mtk_crtc_set_crc_source(struct drm_crtc *crtc, const char *src_name)
{
	bool enabled = false;
	int ret = 0;
	unsigned int val;
	...
	ret = mtk_crc_parse_source(src_name, &enabled);
	if (enabled) {
		/* when .open called */
		spin_lock_irq(&crc->lock);
		mtk_crtc->crc_enabled = true;
		val = readl(mtk_crtc->config_regs + 0x120);
		val |= (1 << 2);
		writel(val, mtk_crtc->config_regs + 0x120);
		val = readl(mtk_crtc->config_regs + 0x140);
		val |= (1 << 2);
		writel(val, mtk_crtc->config_regs + 0x140);
		spin_unlock_irq(&crc->lock);
	} else {
		/* when .release called */
		spin_lock_irq(&crc->lock);
		mtk_crtc->crc_enabled = false;
		val = readl(mtk_crtc->config_regs + 0x120);
		val &= ~(1 << 2);
		writel(val, mtk_crtc->config_regs + 0x120);
		val = readl(mtk_crtc->config_regs + 0x140);
...
void mtk_crtc_handle_ovl_crc(struct drm_crtc *crtc)
{
	u32 crcs;
	...
	spin_lock_irq(&crc->lock);
	crcs = *(u32 *)(cmdq_buf->va_base + DISP_SLOT_OVL_CRC);
	spin_unlock_irq(&crc->lock);

	crcs = crcs & 0x7FFFFFFF;
...
int mtk_drm_setbacklight_grp(struct drm_crtc *crtc, unsigned int level)
{
	...
	if (!comp)
		DDPINFO("The crtc no output comp\n");
	else {
		if (comp->funcs && comp->funcs->io_cmd)
				      ...
				      struct cmdq_pkt *cmdq_handle)
{
	...
	if ((clr_vir_lyr == true) && (drm_crtc_index(crtc) == 2))
			...
			enum CRTC_DDP_PATH ddp_path)
{
	...
	if (comp->id == DDP_COMPONENT_DSI0 || comp->id == DDP_COMPONENT_DSI1) {
		if (mtk_crtc_is_frame_trigger_mode(&mtk_crtc->base))
			...
			return mtk_crtc->gce_obj.event[EVENT_VDO_EOF];/* EVENT_VDO_EOF */
...
static void mtk_crtc_cmdq_timeout_cb(struct cmdq_cb_data data)
{
	...
	if (drm_crtc_index(crtc) == 2) {
		atomic_set(&mtk_crtc->cmdq_flush_cb_done, 1);
		wake_up_interruptible(
			&mtk_crtc->cmdq_flush_cb_wq);
	}
#endif
}
...
void mtk_crtc_start_trig_loop(struct drm_crtc *crtc)
{
...
#ifdef CONFIG_DEBUG_FS
	struct cmdq_pkt_buffer *cmdq_buf = &mtk_crtc->gce_obj.buf;
	struct mtk_ddp_comp *comp_ovl = priv->ddp_comp[DDP_COMPONENT_OVL0];
#endif
#endif
	...
	if (mtk_crtc_is_frame_trigger_mode(crtc)) {
		...
		cmdq_pkt_write(cmdq_handle, NULL,
				comp_ovl->regs_pa + 0x10, 0x100, U32_MAX);
#endif
#endif
		/* sw workaround to fix gce hw bug */
...
void mtk_drm_crtc_enable(struct drm_crtc *crtc)
{
	...
	/* need remove mark when clk test ok */
...
void mtk_drm_crtc_suspend(struct drm_crtc *crtc)
{
	...
	if ((index == 2) && (atomic_read(&mtk_crtc->cmdq_flush_cb_done) == 0))
		DDPMSG("CRTC%d %s waiting for cmdq flush done\n", index, __func__);

	if (index == 2)
		ret = wait_event_interruptible(mtk_crtc->cmdq_flush_cb_wq,
			atomic_read(&mtk_crtc->cmdq_flush_cb_done));
...
#ifdef MTK_DRM_SEC_WFD_SUPPORT
	if (drm_crtc_index(crtc) == 0)
		sec_scenario = CMDQ_SEC_DISP_PRIMARY_DISABLE_SECURE_PATH;
	...
	void *cb_data, struct cmdq_pkt *cmdq_handle)
{
	...
	if (drm_crtc_index(&mtk_crtc->base) == 2) {
		atomic_set(&mtk_crtc->cmdq_flush_cb_done, 0);
	}
...
#if IS_ENABLED(CONFIG_DRM_MEDIATEK_YOCTO)
	drm_crtc_add_bgcolor_property(&mtk_crtc->base);
#endif
...
void mtk_drm_fake_vsync_init(struct drm_crtc *crtc)
{
	...
	snprintf(name, tmp, "mtk_drm_fake_vsync:%d", drm_crtc_index(crtc));
			...
			const struct mtk_crtc_path_data *path_data)
{
	...
	if (drm_crtc_index(&mtk_crtc->base) == 2) {
		atomic_set(&mtk_crtc->cmdq_flush_cb_done, 1);
		init_waitqueue_head(&mtk_crtc->cmdq_flush_cb_wq);
	}
