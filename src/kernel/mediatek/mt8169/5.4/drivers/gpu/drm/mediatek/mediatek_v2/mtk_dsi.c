...
#define MODE_CON REG_FLD_MSB_LSB(1, 0)
#define MODE (3)
#define CMD_MODE 0
...
#define DSI_DCS_WRITE_SHORT_PACKET_1_PARAM	   0x15
#define DSI_DCS_WRITE_LONG_PACKET		   0x39
...
static struct mtk_phy_timing *mtk_dsi_phy_timing_calc(struct mtk_dsi *dsi)
{
	u32 ui, cycle_time;
	struct mtk_phy_timing *timing = &dsi->phy_timing;

	ui = 1000000000U / dsi->data_rate + 1U;
	cycle_time = (1000000000U / dsi->data_rate) * 8 + 1U;
...
static unsigned int mtk_dsi_default_rate(struct mtk_dsi *dsi)
{
	...
	if (dsi->ext && dsi->ext->params->data_rate) {
		...
		data_rate = dsi->vm.pixelclock * bit_per_pixel / dsi->lanes;

		timing = mtk_dsi_phy_timing_calc(dsi);
		...
		data_rate = DIV_ROUND_UP_ULL(dsi->vm.pixelclock * total_bits,
			htotal * dsi->lanes);
	}
...
static int mtk_dsi_poweron(struct mtk_dsi *dsi)
{
	...
	writel(FORCE_COMMIT | DSI_BYPASS_SHADOW, dsi->regs + DSI_SHADOW_DEBUG);

	DRM_DEBUG_DRIVER("dsi shadow 0x%x\n", readl(dsi->regs + DSI_SHADOW_DEBUG));
				     ...
				     unsigned int timeout)
{
	...
	if (ret == 0) {
		DRM_WARN("Wait DSI IRQ(0x%08x) irq_data 0x%08x Timeout\n",
			 irq_flag,
			 dsi->irq_data);
...
{
	int ret;
	pr_notice("mtk_dsi_create_connector_1\n");
...
static int mtk_dsi_create_conn_enc(struct drm_device *drm, struct mtk_dsi *dsi)
{
	...
	pr_notice("mtk_dsi_create_conn_enc_1\n");
	...
	if (ret) {
		DRM_ERROR("mtk_dsi_create_conn_enc_4\n");
		/* Otherwise create our own connector and attach to a panel */
		ret = mtk_dsi_create_connector(drm, dsi);
		if (ret)
			goto err_encoder_cleanup;
	}
	pr_notice("mtk_dsi_create_conn_enc_end\n");
	return 0;
...
int mtk_dsi_bist_pattern_test(struct mtk_ddp_comp *comp, unsigned int color)
{
	...
	if ((readl(dsi->regs + DSI_MODE_CTRL) & MODE_CON) == CMD_MODE) {
		writel(0x002c3909, dsi->regs + DSI_CMDQ0);
		...
		struct device_attribute *attr, char *buf)
{
	...
	int len = 0;
	int temp_len = 0;
	void *reg_base = ioremap(0x14013000, 0x4);
	int k;
	...
	if (strcmp(pattr->name, "register") == 0) {
		...
		for (k = 0; k < 32; k += 16) {
			SPRINTF_DEV_ATTR("0x%04x: 0x%08x 0x%08x 0x%08x 0x%08x\n", k + 0xD00,
				readl(reg_base + 0xD00 + k),
				readl(reg_base + 0xD00 + k + 0x4),
				readl(reg_base + 0xD00 + k + 0x8),
				readl(reg_base + 0xD00 + k + 0xc));
		}
		reg_base = ioremap(0x11cc0000, 0x4);
...
static DEVICE_ATTR(register, 0444, mtk_dsi_show_info,
		NULL);
...
static int mtk_dsi_core_create_attr(struct device *dev)
{
	...
	for (idx = 0; idx < 1; idx++) {
		err = device_create_file(dev, dsi_attr_list[idx]);
		if (err)
			break;
	}
	return err;
...
int mtk_dsi_dump(struct mtk_ddp_comp *comp)
{
	...
	if (DISP_REG_GET_FIELD(MODE_FLD_REG_MODE_CON,
				   baddr + DSI_MODE_CTRL)) {
		/* VDO mode */
		reg_val = (readl(dsi->regs + 0x164)) & 0xfff;
	...
	for (k = 0; k < 32; k += 16) {
		DDPDUMP("0x%04x: 0x%08x 0x%08x 0x%08x 0x%08x\n", k + 0xD00,
			readl(dsi->regs + 0xD00 + k),
			readl(dsi->regs + 0xD00 + k + 0x4),
			readl(dsi->regs + 0xD00 + k + 0x8),
			readl(dsi->regs + 0xD00 + k + 0xc));
	}
	...
	struct mtk_drm_crtc *mtk_crtc, struct drm_crtc_state *old_state)
{
	...
	if (!(dsi->mipi_hopping_sta && adjust_panel_params &&
		...
		if (panel_ext && panel_ext->params && adjust_panel_params &&
...
static int mtk_dsi_clk_enable(struct mtk_dsi *dsi)
{
	...
	ret = clk_prepare_enable(dsi->engine_clk);
	if (ret < 0) {
		pr_err("Failed to enable engine clock: %d\n", ret);
		return ret;
	}
	ret = clk_prepare_enable(dsi->digital_clk);
	if (ret < 0) {
		pr_err("Failed to enable digital clock: %d\n", ret);
		return ret;
	}
	ret = clk_prepare_enable(dsi->hs_clk);
	if (ret < 0) {
		pr_err("Failed to enable hs_clk clock: %d\n", ret);
		return ret;
	}
	return ret;
}
			...
			struct mtk_ddic_dsi_msg *cmd_msg)
{
	...
	/* Check cmd_msg param */
	if (sizeof(cmd_msg->type)/sizeof(cmd_msg->type[0]) == 0 ||
...
static const struct mtk_dsi_driver_data mt8169_dsi_driver_data = {
	.reg_cmdq_ofs = 0xD00,
	.poll_for_idle = mtk_dsi_poll_for_idle,
	.irq_handler = mtk_dsi_irq_status,
	.need_bypass_shadow = false,
...
static const struct of_device_id mtk_dsi_of_match[] = {
	...
	{.compatible = "mediatek,mt8169-dsi", .data = &mt8169_dsi_driver_data},
...
static int mtk_dsi_bind(struct device *dev, struct device *master, void *data)
{
	...
	for (reg = 0; reg < Reg_Max_Num; reg++) {
		endpoint = of_graph_get_endpoint_by_regs(dev->of_node, reg, 0);
		dev_info(dev, "mtk_dsi_probe, of_graph_get_next_endpoint end\n");
		if (endpoint) {
			...
			if (of_device_is_available(remote_node))
				...
				pr_info("remote_node %p is state = NG\n", remote_node);

			dsi->bridge = of_drm_find_bridge(remote_node);
			dsi->panel = of_drm_find_panel(remote_node);
			if (dsi->panel == ERR_PTR(-EPROBE_DEFER) || dsi->panel == ERR_PTR(-ENODEV))
				dsi->panel = NULL;
	...
	dev_info(dev, "before mtk_dsi_create_conn_enc\n");
...
static int mtk_dsi_probe(struct platform_device *pdev)
{
	...
	dev_info(dev, "platform_get_resource, ioremap memory\n");
	...
	if (IS_ERR(dsi->vdosys_regs)) {
		...
		dev_err(dev, "Failed to iomap vdosys regs: %d\n", ret);
		//goto err_unregister_host;
	}
#endif

	dev_info(dev, "get MIPI-DPHY\n");
...
#ifdef CONFIG_FPGA_EARLY_PORTING
	dev_info(dev, "get system configuration registers\n");
	...
	if (ret) {
		ret = PTR_ERR(regmap);
		dev_err(dev, "Failed to get system configuration",
			" registers: %d\n", ret);
		return ret;
	}
	...
	dev_info(dev, "mtk_ddp_comp_get_id start\n");
		...
		unsigned char force_update)
{
	...
	if(msg.tx_len > 2)
		msg.type = DSI_DCS_WRITE_LONG_PACKET;
	else
		msg.type = DSI_DCS_WRITE_SHORT_PACKET_1_PARAM;
		...
		unsigned char force_update)
{
	...
	/*wait and clear EOF
		 ...
		 * because fps change need stop & re-start vdo mode
		 */
	cmdq_pkt_wfe(handle, mtk_crtc->gce_obj.event[EVENT_VDO_EOF]);
	...
	DDPINFO("%s: read_data0 byte0~3=0x%x~0x%x~0x%x~0x%x\n",
		__func__, read_data[0].byte0, read_data[0].byte1
		, read_data[0].byte2, read_data[0].byte3);
	DDPINFO("%s: read_data1 byte0~3=0x%x~0x%x~0x%x~0x%x\n",
		__func__, read_data[1].byte0, read_data[1].byte1
		, read_data[1].byte2, read_data[1].byte3);
	DDPINFO("%s: read_data2 byte0~3=0x%x~0x%x~0x%x~0x%x\n",
		__func__, read_data[2].byte0, read_data[2].byte1
		, read_data[2].byte2, read_data[2].byte3);
	DDPINFO("%s: read_data3 byte0~3=0x%x~0x%x~0x%x~0x%x\n",
		__func__, read_data[3].byte0, read_data[3].byte1
		, read_data[3].byte2, read_data[3].byte3);
	...
	for (i = 0; i < msg.rx_len; i++) {
		DDPINFO("rx_len[%d]=%d\n", i, (int)msg.rx_len);
		for (j = 0; j < msg.rx_len; j++) {
			DDPINFO("rx_buf[%d]--byte:%d,val:0x%x\n",
				i, j, *(char *)(msg.rx_buf + j));
		}
	}
