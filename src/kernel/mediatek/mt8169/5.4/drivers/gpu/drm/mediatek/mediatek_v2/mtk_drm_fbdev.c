...
int pan_display_test(int frame_num, int bpp)
{
	...
	DDPINFO("%s: frame_num=%d,bpp=%d, w=%d,h=%d,fb_h=%d,fb_size=%d\n",
		__func__, frame_num, bpp, w, h, fb_h, fb_size);
			    ...
			    struct fb_info *info)
{
	...
	DDPINFO("request %dx%d-%d (virtual %dx%d) > %dx%d-%d\n",
			   ...
			   struct drm_fb_helper_surface_size *sizes)
{
	...
	DDPMSG("%s+\n", __func__);
	bytes_per_pixel = DIV_ROUND_UP_DDP(sizes->surface_bpp, 8);
