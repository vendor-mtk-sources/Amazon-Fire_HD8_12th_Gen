		...
		if (!(src_total_roi[i].width && src_total_roi[i].height &&
			...
			for (disp_idx = 0; disp_idx < HRT_DISP_TYPE_NUM; disp_idx++)
				if ((state->lye_state.scn[disp_idx] == ONE_SCALING) ||
					...
					DDPINFO("layer roi is zero!!! RSZ force disconnected\n");
				}
		}
	}
}
...
#if defined(CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT)
	for_each_new_crtc_in_state(new_state, crtc, new_crtc_state, i) {
		struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
		...
		if (state->prop_val[CRTC_PROP_OUTPUT_ENABLE]) {
			...
			fb = drm_framebuffer_lookup(dev, NULL, state->prop_val[CRTC_PROP_OUTPUT_FB_ID]);
			if (fb) {
				...
				pr_err("%s cannot find fb fb_id:%u\n", __func__,
					 ...
					 struct drm_crtc *crtc)
{
	...
	if (panel_funcs && panel_funcs->doze_get_mode_flags) {
		...
		if (mtk_crtc_is_frame_trigger_mode(crtc))
			...
			cmdq_pkt_wait_no_clear(handle,
				mtk_crtc->gce_obj.event[EVENT_VDO_EOF]);//EVENT_VDO_EOF
...
static bool mtk_drm_is_enable_from_lk(struct drm_crtc *crtc)
{
	...
	if (drm_crtc_index(crtc) == 0 && (!is_charging_mode()))
#else
					 ...
					 struct drm_atomic_state *state)
{
	...
	for_each_old_crtc_in_state(state, crtc, old_crtc_state, i) {
		...
		if ((mtk_crtc->enabled == false) && (drm_crtc_index(crtc) == 2)) {
			DDPMSG("%s crtc%d is disable, skip flush\n", __func__, drm_crtc_index(crtc));
			return true;
		}
...
static const enum mtk_ddp_comp_id mt8169_mtk_ddp_main[] = {
	DDP_COMPONENT_OVL0, DDP_COMPONENT_RDMA0,
#ifndef DRM_BYPASS_PQ
	DDP_COMPONENT_COLOR0,   DDP_COMPONENT_CCORR0,
	DDP_COMPONENT_AAL0,      DDP_COMPONENT_GAMMA0,
...
#endif
	DDP_COMPONENT_DSI0,
	/* DDP_COMPONENT_PWM0, */ /* PWM default enable */
};

static const enum mtk_ddp_comp_id mt8169_mtk_ddp_ext[] = {
	DDP_COMPONENT_OVL0_2L, DDP_COMPONENT_RDMA1,
	DDP_COMPONENT_DPI0,
};

static const enum mtk_ddp_comp_id mt8169_mtk_ddp_third[] = {
	DDP_COMPONENT_OVL0_2L, DDP_COMPONENT_WDMA0,
};

#ifndef CONFIG_FPGA_EARLY_PORTING
static const enum mtk_ddp_comp_id mt8169_mtk_ddp_main_minor[] = {
	DDP_COMPONENT_OVL0_2L,       DDP_COMPONENT_OVL0,
	DDP_COMPONENT_WDMA0,
};
#endif

static const enum mtk_ddp_comp_id mt8169_mtk_ddp_main_minor_sub[] = {
	DDP_COMPONENT_RDMA0,
	DDP_COMPONENT_COLOR0,   DDP_COMPONENT_CCORR0,
	DDP_COMPONENT_AAL0,      DDP_COMPONENT_GAMMA0,
	DDP_COMPONENT_POSTMASK0, DDP_COMPONENT_DITHER0,
...
#ifndef CONFIG_FPGA_EARLY_PORTING
static const enum mtk_ddp_comp_id mt8169_mtk_ddp_main_wb_path[] = {
	DDP_COMPONENT_OVL0, DDP_COMPONENT_WDMA0,
};
#endif

static const struct mtk_addon_scenario_data mt8169_addon_main[ADDON_SCN_NR] = {
		[NONE] = {
				.module_num = 0,
				.hrt_type = HRT_TB_TYPE_GENERAL1,
			},
...
static const struct mtk_addon_scenario_data mt8169_addon_ext[ADDON_SCN_NR] = {
	[NONE] = {
		.module_num = 0,
		.hrt_type = HRT_TB_TYPE_GENERAL0,
	},
...
static const struct mtk_crtc_path_data mt8169_mtk_main_path_data = {
	.path[DDP_MAJOR][0] = mt8169_mtk_ddp_main,
	.path_len[DDP_MAJOR][0] = ARRAY_SIZE(mt8169_mtk_ddp_main),
	.path_req_hrt[DDP_MAJOR][0] = true,
	.wb_path[DDP_MAJOR] = NULL,
	.wb_path_len[DDP_MAJOR] = 0,
	.path[DDP_MINOR][0] = NULL,
	.path_len[DDP_MINOR][0] = 0,
	.path_req_hrt[DDP_MINOR][0] = false,
	.path[DDP_MINOR][1] = NULL,/* mt8169_mtk_ddp_main_minor_sub*/
	.path_len[DDP_MINOR][1] = 0,/* ARRAY_SIZE(mt8169_mtk_ddp_main_minor_sub) */
	.path_req_hrt[DDP_MINOR][1] = true,
	.addon_data = mt8169_addon_main
};

static const struct mtk_crtc_path_data mt8169_mtk_ext_path_data = {
	.path[DDP_MAJOR][0] = mt8169_mtk_ddp_ext,
	.path_len[DDP_MAJOR][0] = ARRAY_SIZE(mt8169_mtk_ddp_ext),
	.path_req_hrt[DDP_MAJOR][0] = true,
	.addon_data = mt8169_addon_ext,
};

static const struct mtk_crtc_path_data mt8169_mtk_third_path_data = {
	.path[DDP_MAJOR][0] = mt8169_mtk_ddp_third,
	.path_len[DDP_MAJOR][0] = ARRAY_SIZE(mt8169_mtk_ddp_third),
	.addon_data = mt8169_addon_ext,
};
...
const struct mtk_session_mode_tb mt8169_mode_tb[MTK_DRM_SESSION_NUM] = {
		[MTK_DRM_SESSION_DL] = {
...
static const struct mtk_fake_eng_reg mt8169_fake_eng_reg[] = {
		{.CG_idx = 0, .CG_bit = 20, .share_port = true},
		{.CG_idx = 0, .CG_bit = 21, .share_port = true},
};

static const struct mtk_fake_eng_data mt8169_fake_eng_data = {
	.fake_eng_num =  ARRAY_SIZE(mt8169_fake_eng_reg),
	.fake_eng_reg = mt8169_fake_eng_reg,
};
...
static const struct mtk_mmsys_driver_data mt8169_mmsys_driver_data = {
	.main_path_data = &mt8169_mtk_main_path_data,
	/* There is no dpi related code in mt8169, so use third_path make crtc1 create ok,
         * mt8169 no HDMI/DPI using scenario, so no risk for this change.
         */
	.ext_path_data = &mt8169_mtk_third_path_data,//&mt8169_mtk_ext_path_data,
	.third_path_data = &mt8169_mtk_third_path_data,
	.fake_eng_data = &mt8169_fake_eng_data,
	.mmsys_id = MMSYS_MT8169,
	.mode_tb = mt8169_mode_tb,
	.sodi_config = mt8169_mtk_sodi_config,
	.has_smi_limitation = true,
...
static void mtk_drm_get_top_clk(struct mtk_drm_private *priv)
{
	...
	for (i = 0; i < priv->top_clk_num; i++) {
		 ...
		 */
		/*
		 * When boot mode is KERNEL_POWER_OFF_CHARGING_BOOT(8), do not enable top clk,
		 * make sure display clk is disable to enter deep idle
		 */
#if IS_ENABLED(CONFIG_MTK_BOOT)
		if (is_charging_mode()) {
			DDPMSG("%s boot mode is CHARGING_MODE\n", __func__);
			priv->top_clk_num = -1;

			if (!larb_dev) {
				/* Execute only once to balance the smi larb's init-power-on
				 * function.
				 */
				larb_dev = mtk_drm_get_larb(priv->mmsys_dev);
				if (larb_dev &&
				    of_property_read_bool(larb_dev->of_node, "init-power-on")) {
...
#else
	duration_avg = div_s64(duration_sum, (lcm_fps_ctx[index].num - 2));
#endif
	do_div_DDP(fps, duration_avg);
...
static const struct of_device_id mtk_ddp_comp_dt_ids[] = {
	...
	{.compatible = "mediatek,mt6885-disp-ovl",
	 .data = (void *)MTK_DISP_OVL},
	{.compatible = "mediatek,mt8169-disp-ovl",
	 .data = (void *)MTK_DISP_OVL},
	...
	{.compatible = "mediatek,mt6885-disp-rdma",
	 .data = (void *)MTK_DISP_RDMA},
	{.compatible = "mediatek,mt8169-disp-rdma",
	 .data = (void *)MTK_DISP_RDMA},
	...
	{.compatible = "mediatek,mt6853-disp-wdma",
	 .data = (void *)MTK_DISP_WDMA},
	{.compatible = "mediatek,mt8169-disp-wdma",
	 .data = (void *)MTK_DISP_WDMA},
	...
	{.compatible = "mediatek,mt6853-disp-ccorr",
	 .data = (void *)MTK_DISP_CCORR},
	{.compatible = "mediatek,mt8169-disp-ccorr",
	 .data = (void *)MTK_DISP_CCORR},
	...
	{.compatible = "mediatek,mt6885-disp-color",
	 .data = (void *)MTK_DISP_COLOR},
	{.compatible = "mediatek,mt8169-disp-color",
	 .data = (void *)MTK_DISP_COLOR},
	...
	{.compatible = "mediatek,mt6885-disp-aal",
	 .data = (void *)MTK_DISP_AAL},
	{.compatible = "mediatek,mt8169-disp-aal",
	 .data = (void *)MTK_DISP_AAL},
	...
	{.compatible = "mediatek,mt6853-disp-gamma",
	 .data = (void *)MTK_DISP_GAMMA},
	{.compatible = "mediatek,mt8169-disp-gamma",
	 .data = (void *)MTK_DISP_GAMMA},
	...
	{.compatible = "mediatek,mt6853-disp-dither",
	 .data = (void *)MTK_DISP_DITHER},
	{.compatible = "mediatek,mt8169-disp-dither",
	 .data = (void *)MTK_DISP_DITHER},
	...
	{.compatible = "mediatek,mt6853-dsi",
	 .data = (void *)MTK_DSI},
	{.compatible = "mediatek,mt8169-dsi",
	 .data = (void *)MTK_DSI},
	...
	{.compatible = "mediatek,mt8173-dpi",
	 .data = (void *)MTK_DPI},
	{.compatible = "mediatek,mt8169-dpi",
	 .data = (void *)MTK_DPI},
	...
	{.compatible = "mediatek,mt6853-disp-mutex",
	 .data = (void *)MTK_DISP_MUTEX},
	{.compatible = "mediatek,mt8169-disp-mutex",
	 .data = (void *)MTK_DISP_MUTEX},
	...
	{.compatible = "mediatek,mt6853-disp-pwm",
	 .data = (void *)MTK_DISP_PWM},
	{.compatible = "mediatek,mt8169-disp-pwm",
	 .data = (void *)MTK_DISP_PWM},
	...
	{.compatible = "mediatek,mt6853-disp-rsz",
	 .data = (void *)MTK_DISP_RSZ},
	{.compatible = "mediatek,mt8169-disp-rsz",
	 .data = (void *)MTK_DISP_RSZ},
	...
	{.compatible = "mediatek,mt6853-disp-postmask",
	 .data = (void *)MTK_DISP_POSTMASK},
	{.compatible = "mediatek,mt8169-disp-postmask",
	 .data = (void *)MTK_DISP_POSTMASK},
...
#ifdef CONFIG_FPGA_EARLY_PORTING
static void mtk_dispsys_clk_on_for_ep(void)
{
	void *mmsys_reg_base = ioremap(0x14000000, 0x4);

	writel(0, mmsys_reg_base + 0x100); /* clk on dispsys module */
	writel(0, mmsys_reg_base + 0x1A0); /* clk on dsi */

	DDPINFO("%s mmsys clock gating 0x100 = 0x%x\n", readl(mmsys_reg_base + 0x100));
	DDPINFO("%s mmsys clock gating 0x1A0 = 0x%x\n", readl(mmsys_reg_base + 0x1A0));
}
#endif
...
static int mtk_drm_probe(struct platform_device *pdev)
{
	...
	if (private->data->bypass_infra_ddr_control) {
		...
		infra_node = of_find_compatible_node(NULL, NULL, "mediatek,mt8169-infracfg_ao");
		if (infra_node == NULL) {
...
static const struct of_device_id mtk_drm_of_ids[] = {
	...
	{.compatible = "mediatek,mt8169-dispsys",
	 .data = &mt8169_mmsys_driver_data},
	{} };
