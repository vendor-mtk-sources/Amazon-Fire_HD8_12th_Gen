...
#define OVL_CON_MTX_BT709_TO_RGB_FULL_RANGE (5UL << 16)
#endif
...
static irqreturn_t mtk_disp_ovl_irq_handler(int irq, void *dev_id)
{
	...
	if (val & (1 << 1)) {
		...
		if (mtk_crtc && mtk_crtc->crc_enabled)
			mtk_crtc_handle_ovl_crc(&mtk_crtc->base);
#endif
#endif
	}
	if (val & (1 << 2)) {
...
#if IS_ENABLED(CONFIG_DRM_MEDIATEK_YOCTO)
	bgcolor = comp->mtk_crtc->base.state->bgcolor;
	alpha = DRM_ARGB_ALPHA(bgcolor, 8);
	red = DRM_ARGB_RED(bgcolor, 8);
	green = DRM_ARGB_GREEN(bgcolor, 8);
	blue = DRM_ARGB_BLUE(bgcolor, 8);
	bgc = (((alpha & 0xff) << 24) | ((red & 0xff) << 16)
		| ((green & 0xff) << 8) | (blue & 0xff));
	cmdq_pkt_write(handle, comp->cmdq_base,
			comp->regs_pa + DISP_REG_OVL_ROI_BGCLR, bgc, ~0);
#ifdef CONFIG_DEBUG_FS
	cmdq_pkt_write(handle, comp->cmdq_base,
			comp->regs_pa + 0x010, 1 << 8, 1 << 8);
#endif
#else
	cmdq_pkt_write(handle, comp->cmdq_base,
...
{
	enum mtk_ovl_transfer xfr = OVL_GAMMA2_2;
...
#if IS_ENABLED(CONFIG_DRM_MEDIATEK_YOCTO)
static int mtk_ovl_yuv_matrix_convert(enum drm_color_encoding encoding,
				      enum drm_color_range range)
{
	int ret = OVL_CON_MTX_BT601_TO_RGB;
	...
	if (encoding == DRM_COLOR_YCBCR_BT601)
		...
		if (range == DRM_COLOR_YCBCR_LIMITED_RANGE)
			...
			ret = OVL_CON_MTX_BT709_TO_RGB_FULL_RANGE;
	}
	return ret;
}
#else
				 ...
				 struct cmdq_pkt *handle)
{
	...
	if (fmt == DRM_FORMAT_UYVY || fmt == DRM_FORMAT_YUYV) {
		#if IS_ENABLED(CONFIG_DRM_MEDIATEK_YOCTO)
		con |= mtk_ovl_yuv_matrix_convert(state->base.color_encoding,
						  state->base.color_range);
		#else
	...
	if (pending->enable) {
		...
		do_div_DDP(temp_bw, 1000);
		temp_bw *= ratio_tmp;
		do_div_DDP(temp_bw, 100);
				...
				struct mtk_ddp_comp *comp, unsigned int *gs)
{
	...
	/* gs[GS_OVL_RDMA_ULTRA_TH] = 0x80; */
	/* gs[GS_OVL_RDMA_PRE_ULTRA_TH] = 0xc0; */
...
static const struct compress_info compr_info_mt8169  = {
	.name = "AFBC_V1_2_MTK_1",
	.l_config = &compr_l_config_AFBC_V1_2,
};
...
static const struct mtk_disp_ovl_data mt8169_ovl_driver_data = {
	.addr = DISP_REG_OVL_ADDR_BASE,
	.fmt_rgb565_is_0 = true,
	.fmt_uyvy = 4U << 12,
	.fmt_yuyv = 5U << 12,
