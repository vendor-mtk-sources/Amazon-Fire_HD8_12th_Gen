...
static int mtk_wdma_is_busy(struct mtk_ddp_comp *comp)
{
	...
	value = tmp & FLOW_CTRL_DBG_FLD_WDMA_STA_FLOW_CTRL;
	ret = (value != 0x1) ? 1 : 0;
					 ...
					 unsigned int *gs)
{
	...
	/* WDMA_BUF_CON5 */
	tmp = DIV_ROUND_UP_DDP(consume_rate * Bpp * preultra_low_us, FP);
	gs[GS_WDMA_PRE_ULTRA_LOW_Y] = (fifo_size > tmp) ? (fifo_size - tmp) : 1;

	tmp = DIV_ROUND_UP_DDP(consume_rate * Bpp * ultra_low_us, FP);
	gs[GS_WDMA_ULTRA_LOW_Y] = (fifo_size > tmp) ? (fifo_size - tmp) : 1;

	/* WDMA_BUF_CON6 */
	tmp = DIV_ROUND_UP_DDP(consume_rate * Bpp * preultra_high_us, FP);
	gs[GS_WDMA_PRE_ULTRA_HIGH_Y] =
		(fifo_size > tmp) ? (fifo_size - tmp) : 1;

	tmp = DIV_ROUND_UP_DDP(consume_rate * Bpp * ultra_high_us, FP);
	gs[GS_WDMA_ULTRA_HIGH_Y] = (fifo_size > tmp) ? (fifo_size - tmp) : 1;

	/* WDMA_BUF_CON7 */
	tmp = DIV_ROUND_UP_DDP(consume_rate * preultra_low_us, FP * factor1);
	gs[GS_WDMA_PRE_ULTRA_LOW_U] = (fifo > tmp) ? (fifo - tmp) : 1;

	tmp = DIV_ROUND_UP_DDP(consume_rate * ultra_low_us, FP * factor1);
	gs[GS_WDMA_ULTRA_LOW_U] = (fifo > tmp) ? (fifo - tmp) : 1;

	/* WDMA_BUF_CON8 */
	tmp = DIV_ROUND_UP_DDP(consume_rate * preultra_high_us, FP * factor1);
	gs[GS_WDMA_PRE_ULTRA_HIGH_U] = (fifo > tmp) ? (fifo - tmp) : 1;

	tmp = DIV_ROUND_UP_DDP(consume_rate * ultra_high_us, FP * factor1);
	gs[GS_WDMA_ULTRA_HIGH_U] = (fifo > tmp) ? (fifo - tmp) : 1;

	/* WDMA_BUF_CON9 */
	tmp = DIV_ROUND_UP_DDP(consume_rate * preultra_low_us, FP * factor2);
	gs[GS_WDMA_PRE_ULTRA_LOW_V] = (fifo > tmp) ? (fifo - tmp) : 1;

	tmp = DIV_ROUND_UP_DDP(consume_rate * ultra_low_us, FP * factor2);
	gs[GS_WDMA_ULTRA_LOW_V] = (fifo > tmp) ? (fifo - tmp) : 1;

	/* WDMA_BUF_CON10 */
	tmp = DIV_ROUND_UP_DDP(consume_rate * preultra_high_us, FP * factor2);
	gs[GS_WDMA_PRE_ULTRA_HIGH_V] = (fifo > tmp) ? (fifo - tmp) : 1;

	tmp = DIV_ROUND_UP_DDP(consume_rate * ultra_high_us, FP * factor2);
	gs[GS_WDMA_ULTRA_HIGH_V] = (fifo > tmp) ? (fifo - tmp) : 1;

	/* WDMA_BUF_CON11 */
	tmp = DIV_ROUND_UP_DDP(consume_rate * Bpp * (preultra_low_us + dvfs_offset),
			   FP);
	gs[GS_WDMA_PRE_ULTRA_LOW_Y_DVFS] =
		(fifo_size > tmp) ? (fifo_size - tmp) : 1;
	tmp = DIV_ROUND_UP_DDP(consume_rate * Bpp * (ultra_low_us + dvfs_offset),
			   FP);
	gs[GS_WDMA_ULTRA_LOW_Y_DVFS] =
		(fifo_size > tmp) ? (fifo_size - tmp) : 1;

	/* WDMA_BUF_CON12 */
	tmp = DIV_ROUND_UP_DDP(
		consume_rate * Bpp * (preultra_high_us + dvfs_offset), FP);
	gs[GS_WDMA_PRE_ULTRA_HIGH_Y_DVFS] =
		(fifo_size > tmp) ? (fifo_size - tmp) : 1;
	tmp = DIV_ROUND_UP_DDP(consume_rate * Bpp * (ultra_high_us + dvfs_offset),
			   FP);
	gs[GS_WDMA_ULTRA_HIGH_Y_DVFS] =
		(fifo_size > tmp) ? (fifo_size - tmp) : 1;

	/* WDMA_BUF_CON13 */
	tmp = DIV_ROUND_UP_DDP(consume_rate * (preultra_low_us + dvfs_offset),
			   FP * factor1);
	gs[GS_WDMA_PRE_ULTRA_LOW_U_DVFS] = (fifo > tmp) ? (fifo - tmp) : 1;

	tmp = DIV_ROUND_UP_DDP(consume_rate * (ultra_low_us + dvfs_offset),
			   FP * factor1);
	gs[GS_WDMA_ULTRA_LOW_U_DVFS] = (fifo > tmp) ? (fifo - tmp) : 1;

	/* WDMA_BUF_CON14 */
	tmp = DIV_ROUND_UP_DDP(consume_rate * (preultra_high_us + dvfs_offset),
			   FP * factor1);
	gs[GS_WDMA_PRE_ULTRA_HIGH_U_DVFS] = (fifo > tmp) ? (fifo - tmp) : 1;

	tmp = DIV_ROUND_UP_DDP(consume_rate * (ultra_high_us + dvfs_offset),
			   FP * factor1);
	...
	tmp = DIV_ROUND_UP_DDP(consume_rate * Bpp * urgent_low_offset, FP);
	gs[GS_WDMA_URGENT_LOW_Y] = (fifo_size > tmp) ? (fifo_size - tmp) : 1;

	tmp = DIV_ROUND_UP_DDP(consume_rate * Bpp * urgent_high_offset, FP);
	gs[GS_WDMA_URGENT_HIGH_Y] = (fifo_size > tmp) ? (fifo_size - tmp) : 1;

	/* WDMA URGENT CONTROL 1 */
	tmp = DIV_ROUND_UP_DDP(consume_rate * urgent_low_offset, FP * factor1);
	gs[GS_WDMA_URGENT_LOW_U] = (fifo > tmp) ? (fifo - tmp) : 1;

	tmp = DIV_ROUND_UP_DDP(consume_rate * urgent_high_offset, FP * factor1);
	gs[GS_WDMA_URGENT_HIGH_U] = (fifo > tmp) ? (fifo - tmp) : 1;

	/* WDMA URGENT CONTROL 2 */
	tmp = DIV_ROUND_UP_DDP(consume_rate * urgent_low_offset, FP * factor2);
	gs[GS_WDMA_URGENT_LOW_V] = (fifo > tmp) ? (fifo - tmp) : 1;

	tmp = DIV_ROUND_UP_DDP(consume_rate * urgent_high_offset, FP * factor2);
...
static int wdma_config_yuv420(struct mtk_ddp_comp *comp,
			      ...
			      unsigned long offset, void *handle, bool sec)
#else
static int wdma_config_yuv420(struct mtk_ddp_comp *comp,
			      uint32_t fmt, unsigned int dstPitch,
			      unsigned int Height, unsigned long dstAddress,
...
#ifdef MTK_DRM_SEC_WFD_SUPPORT
	if (sec) {
		cmdq_sec_pkt_write_reg(handle, comp->regs_pa + DISP_REG_WDMA_DST_ADDR1,
			dstAddress, CMDQ_IWC_H_2_MVA, offset + u_off, u_size, 0);
		if (has_v) {
			cmdq_sec_pkt_write_reg(handle, comp->regs_pa + DISP_REG_WDMA_DST_ADDR2,
				dstAddress, CMDQ_IWC_H_2_MVA, offset + v_off, u_size, 0);
		}
	} else {
			    ...
			    struct mtk_ddp_config *cfg, struct cmdq_pkt *handle)
{
...
#ifdef MTK_DRM_SEC_WFD_SUPPORT
	y_size = clip_h * comp->fb->pitches[0];
#endif
	...
	if (is_yuv(comp->fb->format->format)) {
		...
		wdma_config_yuv420(comp, comp->fb->format->format,
			comp->fb->pitches[0], cfg->h, addr, offset, handle, sec);
#else
		wdma_config_yuv420(comp, comp->fb->format->format,
...
#ifdef MTK_DRM_SEC_WFD_SUPPORT
	if (sec)
		cmdq_sec_pkt_write_reg(handle, comp->regs_pa + DISP_REG_WDMA_DST_ADDR0,
			sec_hande, CMDQ_IWC_H_2_MVA, offset, y_size, 0);
	else
...
static const struct mtk_disp_wdma_data mt8169_wdma_driver_data = {
	.fifo_size_1plane = 578,
	.fifo_size_uv_1plane = 29,
	.fifo_size_2plane = 402,
	.fifo_size_uv_2plane = 201,
