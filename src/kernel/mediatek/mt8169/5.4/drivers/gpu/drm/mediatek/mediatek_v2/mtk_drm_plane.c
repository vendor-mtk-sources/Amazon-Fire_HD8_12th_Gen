...
static const uint64_t mtk_plane_modifiers[] = {
...
#if IS_ENABLED(CONFIG_DRM_MEDIATEK_YOCTO)
static bool mtk_drm_plane_format_mod_supported(struct drm_plane *plane,
						uint32_t format,
						uint64_t modifier)
{
...
#if IS_ENABLED(CONFIG_DRM_MEDIATEK_YOCTO)
	.format_mod_supported = mtk_drm_plane_format_mod_supported,
#endif
};
				    ...
				    struct drm_plane_state *old_state)
{
	...
	for (i = 0; i < PLANE_PROP_MAX; i++) {
		if (IS_ENABLED(CONFIG_DRM_MEDIATEK_YOCTO) &&
			i == PLANE_PROP_PLANE_ALPHA)
			state->pending.prop_val[i] = plane->state->alpha;
		else if (IS_ENABLED(CONFIG_DRM_MEDIATEK_YOCTO) &&
				i == PLANE_PROP_DATASPACE)
			state->pending.prop_val[i] =
	...
	for (i = 0; i < PLANE_PROP_MAX; i++) {
		if (IS_ENABLED(CONFIG_DRM_MEDIATEK_YOCTO) &&
			...
			DDPINFO("prop_val[%d]:%d ", i,
				(unsigned int)plane->state->alpha);
		else if (IS_ENABLED(CONFIG_DRM_MEDIATEK_YOCTO) &&
				i == PLANE_PROP_DATASPACE)
			DDPINFO("prop_val[%d]:%d ", i,
				(unsigned int)plane->state->color_encoding);
...
static void mtk_plane_attach_property(struct mtk_drm_plane *plane)
{
	...
	if (num == 0) {
		for (i = 0; i < PLANE_PROP_MAX; i++) {
			...
			#if IS_ENABLED(CONFIG_DRM_MEDIATEK_YOCTO)
				if (!strncmp(mtk_plane_property[i].name,
					"PLANE_PROP_PLANE_ALPHA", 22))
					continue;
				if (!strncmp(mtk_plane_property[i].name,
					"DATASPACE", 9))
					continue;
		   ...
		   enum drm_plane_type type)
{
	...
	err = drm_universal_plane_init(dev, &plane->base, possible_crtcs,
				       ...
				       ARRAY_SIZE(formats), mtk_plane_modifiers,
				       type, NULL);
#else
	err = drm_universal_plane_init(dev, &plane->base, possible_crtcs,
				       &mtk_plane_funcs, formats,
	...
	drm_plane_helper_add(&plane->base, &mtk_plane_helper_funcs);
		...
		if (err) {
			DRM_ERROR("%s:%d failed to create alpha property\n", __func__,
					__LINE__);
			return err;
		}

		err = drm_plane_create_color_properties(&plane->base,
		...
		if (err) {
			DRM_ERROR("%s:%d failed to create color property\n", __func__,
					__LINE__);
			return err;
		}
