...
/* for debug rdma underflow using */
enum DUMP_MODULE_REGS {
	MMSYS_REG_BASE = 0,
	MUTEX_REG_BASE,
	RDMA0_REG_BASE,
	OVL0_REG_BASE,
...
struct mtk_disp_rdma {
	...
	/* for debug rdma underflow using */
	void __iomem *regs[DUMP_REGS_NUM_MAX];
};
...
static void mtk_disp_dump_info_for_underflow(struct mtk_disp_rdma *priv)
{
	DDPMSG("mmsys valid0 0xE9C=0x%x,valid1 0xEA0=0x%x,ready0 0xEA8=0x%x,ready0 0xEAC=0x%x\n",
		readl(priv->regs[MMSYS_REG_BASE] + 0xE9C),
		readl(priv->regs[MMSYS_REG_BASE] + 0xEA0),
		readl(priv->regs[MMSYS_REG_BASE] + 0xEA8),
		readl(priv->regs[MMSYS_REG_BASE] + 0xEAC));
	DDPMSG("mmsys mout&sout info 0xF18=0x%x,0xF28=0x%x,0xF0C=0x%x,0xF20=0x%x,0xF30=0x%x\n",
		readl(priv->regs[MMSYS_REG_BASE] + 0xF18),
		readl(priv->regs[MMSYS_REG_BASE] + 0xF28),
		readl(priv->regs[MMSYS_REG_BASE] + 0xF0C),
		readl(priv->regs[MMSYS_REG_BASE] + 0xF20),
		readl(priv->regs[MMSYS_REG_BASE] + 0xF30));
	DDPMSG("mutex info 0x0=0x%x,0x004=0x%x,0x020=0x%x,0x02C=0x%x,0x030=0x%x\n",
		readl(priv->regs[MUTEX_REG_BASE]),
		readl(priv->regs[MUTEX_REG_BASE] + 0x004),
		readl(priv->regs[MUTEX_REG_BASE] + 0x020),
		readl(priv->regs[MUTEX_REG_BASE] + 0x02C),
		readl(priv->regs[MUTEX_REG_BASE] + 0x030));
	DDPMSG("OVL0 intsta: 0x008=0x%x,RDMA0 intsta: 0x004=0x%x,DSI0 intsta: 0x00C=0x%x\n",
		readl(priv->regs[OVL0_REG_BASE] + 0x008),
		readl(priv->regs[RDMA0_REG_BASE] + 0x004),
		readl(priv->regs[DSI0_REG_BASE] + 0x00C));
	DDPMSG("SMI info dump larb0 0x208=0x%x, 0x288=0x%x\n",
		readl(priv->regs[LARB0_REG_BASE] + 0x208),
		readl(priv->regs[LARB0_REG_BASE] + 0x288));
	DDPMSG("SMI info dump larb0 0x288=0x%x\n", readl(priv->regs[LARB0_REG_BASE] + 0x288));
	DDPMSG("SMI info dump larb0 0x288=0x%x\n", readl(priv->regs[LARB0_REG_BASE] + 0x288));
	DDPMSG("SMI info dump larb1 0x208=0x%x, 0x288=0x%x\n",
		readl(priv->regs[LARB1_REG_BASE] + 0x208),
		readl(priv->regs[LARB1_REG_BASE] + 0x288));
	DDPMSG("SMI info dump larb1 0x288=0x%x\n", readl(priv->regs[LARB1_REG_BASE] + 0x288));
	DDPMSG("SMI info dump larb1 0x288=0x%x\n", readl(priv->regs[LARB1_REG_BASE] + 0x288));
	DDPMSG("ovl0 info dump 0x020=0x%x,0x02C=0x%x,0x038=0x%x,0x058=0x%x,0x078=0x%x,0x098=0x%x,0x244=0x%x\n",
		readl(priv->regs[OVL0_REG_BASE] + 0x020),
		readl(priv->regs[OVL0_REG_BASE] + 0x02C),
		readl(priv->regs[OVL0_REG_BASE] + 0x038),
		readl(priv->regs[OVL0_REG_BASE] + 0x058),
		readl(priv->regs[OVL0_REG_BASE] + 0x078),
		readl(priv->regs[OVL0_REG_BASE] + 0x098),
		readl(priv->regs[OVL0_REG_BASE] + 0x244));
}
...
static irqreturn_t mtk_disp_rdma_irq_handler(int irq, void *dev_id)
{
	...
	if (val & (1 << 4)) {
		if (((priv->underflow_cnt % 100) == 0) ||
			((priv->underflow_cnt % 100) == 1)) {
			...
			mtk_disp_dump_info_for_underflow(priv);
		}
	...
	struct mtk_ddp_config *cfg, unsigned int *gs)
{
	...
	do_div_DDP(consume_rate, 1000);
	consume_rate *= 125;
	do_div_DDP(consume_rate, 16 * 1000);

	/* RDMA golden setting calculation */
	/* DISP_RDMA_MEM_GMC_SETTING_0 */
	gs[GS_RDMA_PRE_ULTRA_TH_LOW] =
		DIV_ROUND_UP_DDP(consume_rate * (pre_ultra_low_us), FP);
	gs[GS_RDMA_PRE_ULTRA_TH_HIGH] =
		DIV_ROUND_UP_DDP(consume_rate * (pre_ultra_high_us), FP);
	if (gsc->is_vdo_mode) {
	...
	gs[GS_RDMA_ULTRA_TH_LOW] =
		DIV_ROUND_UP_DDP(consume_rate * (ultra_low_us), FP);
	...
	gs[GS_RDMA_TH_LOW_FOR_SODI] =
		DIV_ROUND_UP_DDP(consume_rate * (ultra_low_us + 50), FP);
	gs[GS_RDMA_TH_HIGH_FOR_SODI] = DIV_ROUND_UP_DDP(
	...
	gs[GS_RDMA_DVFS_PRE_ULTRA_TH_LOW] =
		DIV_ROUND_UP_DDP(consume_rate * (pre_ultra_low_us + 40), FP);
	gs[GS_RDMA_DVFS_PRE_ULTRA_TH_HIGH] =
		DIV_ROUND_UP_DDP(consume_rate * (pre_ultra_high_us + 40), FP);

	/* DISP_RDMA_DVFS_SETTING_ULTRA */
	gs[GS_RDMA_DVFS_ULTRA_TH_LOW] =
		DIV_ROUND_UP_DDP(consume_rate * (ultra_low_us + 40), FP);
	gs[GS_RDMA_DVFS_ULTRA_TH_HIGH] = gs[GS_RDMA_DVFS_PRE_ULTRA_TH_LOW];

	/* DISP_RDMA_LEAVE_DRS_SETTING */
	gs[GS_RDMA_IS_DRS_STATUS_TH_LOW] =
		DIV_ROUND_UP_DDP(consume_rate * (pre_ultra_low_us + 20), FP);
	gs[GS_RDMA_IS_DRS_STATUS_TH_HIGH] =
		DIV_ROUND_UP_DDP(consume_rate * (pre_ultra_low_us + 20), FP);

	/* DISP_RDMA_ENTER_DRS_SETTING */
	gs[GS_RDMA_NOT_DRS_STATUS_TH_LOW] =
		DIV_ROUND_UP_DDP(consume_rate * (ultra_high_us + 40), FP);
	gs[GS_RDMA_NOT_DRS_STATUS_TH_HIGH] =
		DIV_ROUND_UP_DDP(consume_rate * (ultra_high_us + 40), FP);

	/* DISP_RDMA_MEM_GMC_SETTING_3 */
	gs[GS_RDMA_URGENT_TH_LOW] = DIV_ROUND_UP_DDP(consume_rate *
		urgent_low_us, FP);
	gs[GS_RDMA_URGENT_TH_HIGH] = DIV_ROUND_UP_DDP(consume_rate *
		urgent_high_us, FP);
...
static void mtk_disp_rdma_get_dump_module_regs(struct mtk_disp_rdma *priv)
{
	DDPINFO("%s+\n", __func__);

	priv->regs[MMSYS_REG_BASE] = ioremap(0x14000000, 0x4);
	priv->regs[MUTEX_REG_BASE] = ioremap(0x14001000, 0x4);
	priv->regs[RDMA0_REG_BASE] = ioremap(0x14007000, 0x4);
	priv->regs[OVL0_REG_BASE] = ioremap(0x14005000, 0x4);
	priv->regs[LARB0_REG_BASE] = ioremap(0x14003000, 0x4);
	priv->regs[LARB1_REG_BASE] = ioremap(0x14004000, 0x4);
	priv->regs[DSI0_REG_BASE] = ioremap(0x14013000, 0x4);
}
...
static int mtk_disp_rdma_probe(struct platform_device *pdev)
{
	...
	mtk_disp_rdma_get_dump_module_regs(priv);
...
static const struct mtk_disp_rdma_data mt8169_rdma_driver_data = {
	.fifo_size = SZ_1K * 3 + SZ_32K,
	.pre_ultra_low_us = 250,
	.pre_ultra_high_us = 260,
	.ultra_low_us = 230,
