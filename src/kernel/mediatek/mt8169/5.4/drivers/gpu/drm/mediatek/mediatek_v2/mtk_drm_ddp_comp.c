...
#define MT8169_INFRA_DISP_DDR_CTL 0xB8
#define MT8169_INFRA_DISP_DDR_MASK 0xC02
...
void mt8169_mtk_sodi_config(struct drm_device *drm, enum mtk_ddp_comp_id id,
			    struct cmdq_pkt *handle, void *data)
{
	struct mtk_drm_private *priv = drm->dev_private;
	unsigned int sodi_req_val = 0, sodi_req_mask = 0;
	...
	if (handle == NULL) {
		...
		if (priv->data->bypass_infra_ddr_control) {
			if (!IS_ERR(priv->infra_regs)) {
				v = (readl(priv->infra_regs + MT8169_INFRA_DISP_DDR_CTL)
					| MT8169_INFRA_DISP_DDR_MASK);
				writel_relaxed(v, priv->infra_regs + MT8169_INFRA_DISP_DDR_CTL);
			} else
				DDPPR_ERR("%s: failed to disable infra ddr control\n", __func__);
		}
	} else {
		cmdq_pkt_write(handle, NULL, priv->config_regs_pa +
			MMSYS_SODI_REQ_MASK, sodi_req_val, sodi_req_mask);
