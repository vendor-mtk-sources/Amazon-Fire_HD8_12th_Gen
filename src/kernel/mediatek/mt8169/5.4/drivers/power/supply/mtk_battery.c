...
bool get_battery_id_status(void)
{
	...
	gm = get_mtk_battery();
	if (gm == NULL || gm->g_bat_id_iio_channel == NULL) {
		bm_err("%s: get_mtk_battery failed\n", __func__);
		return false;
	}
	...
	struct mtk_battery *gm)
{
	...
	gm->g_bat_id_iio_channel = iio_channel_get(&dev->dev, "pmic_battery_id");
	...
	struct mtk_battery *gm, unsigned int flow_state, int cmd, int para1)
{
	...
	if (is_algo_active(gm) == true)
		...
		ret = wakeup_fg_daemon(flow_state, cmd, para1);
	...
	const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_THERMAL_INPUT_POWER_LIMIT:
		if (info) {
			info->chg_data[0].thermal_input_power_limit = val->intval;
			pr_notice("%s:thermal_input_power_limit:%d\n",
				__func__, val->intval);
			charger_dev_notify(info->chg1_dev, CHARGER_DEV_NOTIFY_UPDATE);
			if (val->intval == 0)
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_THERMAL_INPUT_POWER_LIMIT:
		if (info)
			val->intval = info->chg_data[0].thermal_input_power_limit;
		else
			val->intval = -1;
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT:
	...
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT:
		if (info)
			val->intval = info->chg_data[0].thermal_charging_current_limit;
		else
			val->intval = -1;
		break;
	default:
...
static int is_bat_eoc_protect(void)
{
	...
	if (chg_psy == NULL || IS_ERR(chg_psy)) {
		pr_notice("%s Couldn't get mtk-master-charger psy\n", __func__);
		ret = -1;
	} else {
		ret = power_supply_get_property(chg_psy,
			POWER_SUPPLY_PROP_EOC_PROTECT, &prop);
	}

	pr_notice("%s bat_eoc_protect:%d\n", __func__, prop.intval);
	return prop.intval;
}
	...
	struct mtk_battery *gm)
{
	...
	fg_read_dts_val(np, "NAFG_RESISTANCE", &(fg_cust_data->nafg_resistance),
		1);
	ret = fg_read_dts_val(np, "MIN_BATTERY_TEMP",
		&min_battery_temp, UNIT_TRANS_10);
	if (ret == -1)
		bm_err("min_battery_temp cannot find,use fault:%d\n", gm->min_battery_temp);
	else
		gm->min_battery_temp = min_battery_temp;
...
void fg_drv_update_hw_status(struct mtk_battery *gm)
{
	...
	bm_err("%s: car[%d,%ld,%ld,%ld,%ld] tmp:%d soc:%d uisoc:%d vbat:%d ibat:%d algo:%d gm3:%d %d %d %d,boot:%d\n",
		__func__,
	...
	if (gm->tbat_precise <= gm->min_battery_temp)
...
int battery_update_routine(void *arg)
{
	...
	battery_update_psd(gm);
	while (1) {
		bm_err("[%s] vbat:%d, vsys:%d\n", __func__,
			gauge_get_int_property(GAUGE_PROP_BATTERY_VOLTAGE),
			gauge_get_int_property(GAUGE_PROP_VSYS_VOLTAGE));
		ret = wait_event_interruptible(gm->wait_que, (gm->fg_update_flag > 0));
		if (ret < 0) {
			bm_err("%s: wait event been interrupted(%d)\n", __func__, ret);
			continue;
		}
