...
static int vsys_vol_get(struct mtk_gauge *gauge,
	struct mtk_gauge_sysfs_field_info *attr, int *val)
{
	int ret;

	if (!IS_ERR(gauge->chan_vsys_voltage)) {
		ret = iio_read_channel_processed(gauge->chan_vsys_voltage, val);
		if (ret < 0)
...
static struct mtk_gauge_sysfs_field_info mt6357_sysfs_field_tbl[] = {
	...
	GAUGE_SYSFS_FIELD_RO(vsys_vol_get,
		GAUGE_PROP_VSYS_VOLTAGE),
...
static void mt6357_charger_auto_on(struct device *dev, bool bEn)
{
	struct mtk_gauge *gauge;
	...
	/* enable envtem */
	if (bEn)
		regmap_update_bits(gauge->regmap, 0xb9a, 0x1, 0);
	else
		regmap_update_bits(gauge->regmap, 0xb9a, 0x1, 1);

	/* enable envtem write */
	regmap_update_bits(gauge->regmap, 0xb9a, 0x2, 0x2);
}
...
static void mt6357_gauge_shutdown(struct platform_device *pdev)
{
	...
	union power_supply_propval dcap;
	int ret, i;
	unsigned int val;
...
static int mt6357_get_bat_id(struct device *dev)
{
	struct iio_channel *chan_bat_id;
	static int bat_id = 0xff;
	int ret, val;

	if (bat_id != 0xff)
		return bat_id;

	chan_bat_id = devm_iio_channel_get(dev, "pmic_battery_id");
	if (IS_ERR(chan_bat_id)) {
		bm_err("%s get bat_id channel fail\n", __func__);
		goto out;
	}

	ret = iio_read_channel_processed(chan_bat_id, &val);
	if (ret < 0) {
		bm_err("[%s]read bat_id channel fail,ret=%d\n", __func__, ret);
		goto out;
	}

	bat_id = val ? 0 : 1;
	bm_debug("[%s]batid %d, val %d mv\n", __func__, bat_id, val);
out:
	if (bat_id == 0xff)
		bat_id = 0;
...
static int mt6357_gauge_probe(struct platform_device *pdev)
{
	...
	gauge->chan_vsys_voltage = devm_iio_channel_get(
		&pdev->dev, "pmic_vsys_voltage");
	if (IS_ERR(gauge->chan_vsys_voltage)) {
		ret = PTR_ERR(gauge->chan_vsys_voltage);
		bm_err("chan_vsys_voltage auxadc get fail, ret=%d\n", ret);
	}
	...
	bm_err("%s: done, id %d\n", __func__, mt6357_get_bat_id(&pdev->dev));
