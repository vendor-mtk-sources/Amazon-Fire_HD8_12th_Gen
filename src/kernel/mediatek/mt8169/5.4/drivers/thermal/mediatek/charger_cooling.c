...
static const int master_charger_state_to_current_limit[CHARGER_STATE_NUM] = {
	-1, 2000000, 1700000, 1500000, 1200000, 1000000, 700000, 500000, 0
};
...
static int charger_cooling_probe(struct platform_device *pdev)
{
	...
	if (!np->name)
		np->name = "chg_thermal";
	strncpy(charger_cdev->name, np->name, sizeof(charger_cdev->name));
	charger_cdev->name[sizeof(charger_cdev->name)-1]= '\0';
