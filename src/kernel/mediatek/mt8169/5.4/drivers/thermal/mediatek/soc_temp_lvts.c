...
static unsigned int lvts_temp_to_raw(struct formula_coeff *co, int temp)
{
	...
	long long golden_temp = (long long) co->golden_temp;

	msr_raw = ((long long)(golden_temp * 500 + co->b - temp) << 14)
		/ (-1 * co->a);
...
static void lvts_reset(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_domain; i++) {
		...
		if (lvts_data->domain[i].reset)
			if (reset_control_deassert(lvts_data->domain[i].reset))
				dev_err(dev, "cannot deassert core reset %d\n", i);
	}
}
...
#define MT8169_NUM_LVTS (ARRAY_SIZE(mt8169_tc_settings))
#define MT8169_TSBG_DEM_CKSEL_X_TSBG_CHOP_EN (DEVICE_WRITE | RG_TSV2F_CTRL_4 << 8 | 0xFC)
#define MT8169_TSV2F_CHOP_CKSEL_AND_TSV2F_EN (DEVICE_WRITE | RG_TSV2F_CTRL_2 << 8 | 0xAC)
#define MT8169_SET_TS_CHOP (DEVICE_WRITE | RG_TSV2F_CTRL_0 << 8 | 0xF7)
#define MT8169_SET_TSV2F_RSV (DEVICE_WRITE | RG_TSV2F_CTRL_3 << 8 | 0x04)

enum mt8169_lvts_domain {
	MT8169_AP_DOMAIN,
	MT8169_NUM_DOMAIN
};

enum mt8169_lvts_sensor_enum {
	MT8169_TS1_0,
	MT8169_TS1_1,
	MT8169_TS1_2,
	MT8169_TS1_3,
 ...
 *==================================================
 */
#define TSBG_DEM_CKSEL_X_TSBG_CHOP_EN_8169 (DEVICE_WRITE | RG_TSV2F_CTRL_4 << 8 | 0xFC)
#define TSV2F_CHOP_CKSEL_AND_TSV2F_EN_8169 (DEVICE_WRITE | RG_TSV2F_CTRL_2 << 8 | 0xAC)
#define SET_TS_CHOP_8169 (DEVICE_WRITE | RG_TSV2F_CTRL_0 << 8 | 0xF7)
#define SET_TSV2F_RSV_8169 (DEVICE_WRITE | RG_TSV2F_CTRL_3 << 8 | 0x04)

void mt8169_device_enable_and_init(struct lvts_data *lvts_data)
{
	unsigned int i;
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		...
		lvts_write_device(lvts_data, SET_COUNTING_WINDOW_20US1_V4, i);
		lvts_write_device(lvts_data, SET_RG_TSFM_LPDLY_V4, i);
		lvts_write_device(lvts_data, TSBG_DEM_CKSEL_X_TSBG_CHOP_EN_8169, i);
		lvts_write_device(lvts_data, TSV2F_CHOP_CKSEL_AND_TSV2F_EN_8169, i);
		lvts_write_device(lvts_data, SET_TS_RSV_V4, i);
		lvts_write_device(lvts_data, SET_TS_CHOP_8169, i);
		lvts_write_device(lvts_data, SET_TSV2F_RSV_8169, i);
	}
...
static void mt8169_efuse_to_cal_data(struct lvts_data *lvts_data)
{
	struct sensor_cal_data *cal_data = &lvts_data->cal_data;

	cal_data->golden_temp = GET_CAL_DATA_BITMASK(0, 31, 24);
	cal_data->count_r[MT8169_TS1_0] = GET_CAL_DATA_BITMASK(1, 31, 8);
	cal_data->count_r[MT8169_TS1_1] = (GET_CAL_DATA_BITMASK(1, 7, 0) << 16) +
						GET_CAL_DATA_BITMASK(2, 31, 16);
	cal_data->count_r[MT8169_TS1_2] = (GET_CAL_DATA_BITMASK(2, 15, 0) << 8) +
						GET_CAL_DATA_BITMASK(3, 31, 24);
	cal_data->count_r[MT8169_TS1_3] = GET_CAL_DATA_BITMASK(3, 23, 0);

	cal_data->count_r[MT8169_TS2_0] = (GET_CAL_DATA_BITMASK(4, 7, 0) << 16) +
						GET_CAL_DATA_BITMASK(5, 31, 16);
	cal_data->count_r[MT8169_TS2_1] = (GET_CAL_DATA_BITMASK(5, 15, 0) << 8) +
						GET_CAL_DATA_BITMASK(6, 31, 24);

	cal_data->count_r[MT8169_TS3_0] = GET_CAL_DATA_BITMASK(7, 31, 8);
	cal_data->count_r[MT8169_TS3_1] = (GET_CAL_DATA_BITMASK(7, 7, 0) << 16) +
						GET_CAL_DATA_BITMASK(8, 31, 16);
	cal_data->count_r[MT8169_TS3_2] = (GET_CAL_DATA_BITMASK(8, 15, 0) << 8) +
						GET_CAL_DATA_BITMASK(9, 31, 24);

	cal_data->count_rc[MT8169_TS1_0] = GET_CAL_DATA_BITMASK(4, 31, 8);
	cal_data->count_rc[MT8169_TS2_0] = GET_CAL_DATA_BITMASK(6, 23, 0);
	cal_data->count_rc[MT8169_TS3_0] = GET_CAL_DATA_BITMASK(9, 23, 0);
}

static struct tc_settings mt8169_tc_settings[] = {
	[0] = {
		.domain_index = MT8169_AP_DOMAIN,
		.addr_offset = 0x0,
		.num_sensor = 4,
		.sensor_map = {MT8169_TS1_0, MT8169_TS1_1, MT8169_TS1_2, MT8169_TS1_3},
		.tc_speed = SET_TC_SPEED_IN_US(118, 118, 118, 118),
		.hw_filter = LVTS_FILTER_2_OF_4,
		.dominator_sensing_point = SENSING_POINT0,
	...
	[1] = {
		...
		.addr_offset = 0x100,
		.num_sensor = 2,
		.sensor_map = {MT8169_TS2_0, MT8169_TS2_1},
		.tc_speed = SET_TC_SPEED_IN_US(118, 118, 118, 118),
		.hw_filter = LVTS_FILTER_2_OF_4,
		.dominator_sensing_point = SENSING_POINT0,
		.hw_reboot_trip_point = 105000,
	...
	[2] = {
		...
		.addr_offset = 0x200,
		.num_sensor = 3,
		.sensor_map = {MT8169_TS3_0, MT8169_TS3_1, MT8169_TS3_2},
		.tc_speed = SET_TC_SPEED_IN_US(118, 118, 118, 118),
		.hw_filter = LVTS_FILTER_2_OF_4,
		.dominator_sensing_point = SENSING_POINT0,
		.hw_reboot_trip_point = 105000,
...
static struct lvts_data mt8169_lvts_data = {
	.num_domain = MT8169_NUM_DOMAIN,
	.num_tc = MT8169_NUM_LVTS,
	.tc = mt8169_tc_settings,
	.num_sensor = MT8169_NUM_TS,
	.ops = {
		.efuse_to_cal_data = mt8169_efuse_to_cal_data,
		.device_enable_and_init = mt8169_device_enable_and_init,
		.device_enable_auto_rck = device_enable_auto_rck_v4,
		.device_read_count_rc_n = device_read_count_rc_n_v4,
		.set_cal_data = set_calibration_data_v4,
		.init_controller = init_controller_v4,
...
static const struct of_device_id lvts_of_match[] = {
	...
	{
		.compatible = "mediatek,mt8169-lvts",
		.data = (void *)&mt8169_lvts_data,
	},
	{
	},
};
