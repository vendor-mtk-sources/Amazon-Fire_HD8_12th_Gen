...
#include "rootfs_check/mtd_verity.h"
#endif
...
void __init prepare_namespace(void)
{
...
#ifdef CONFIG_ROOTFS_CHECK
	{
		...
		int rootfs_mtd_num;
#endif
		pr_notice("[rootfs_check]checking rootfs integrity...\n");
#ifdef CONFIG_MTD_NAND
...
#endif
		{
			...
			while (driver_probe_done() != 0 ||
				(rootfs_dev = name_to_dev_t(saved_root_name))
					== 0) {
				pr_notice("[rootfs_check] wait for %s...\n",
					saved_root_name);
				msleep(100);
			}
...
verity_pass:
		pr_notice("[rootfs_check]rootfs pass integrity check,boot continue...\n");
	}
#endif
