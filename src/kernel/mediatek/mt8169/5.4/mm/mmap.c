...
{
	if (atomic_dec_and_test(&vma_spf_vm_ref_count(vma)))
		__free_vma(vma);
}
#else
...
#ifdef CONFIG_SPECULATIVE_PAGE_FAULT
#define mm_rb_write_lock(mm)	write_lock(mm_mm_rb_lock(mm))
#define mm_rb_write_unlock(mm)	write_unlock(mm_mm_rb_lock(mm))
#else
...
struct vm_area_struct *get_vma(struct mm_struct *mm, unsigned long addr)
{
	...
	read_lock(mm_mm_rb_lock(mm));
	vma = __find_vma(mm, addr);
	if (vma)
		atomic_inc(&vma_spf_vm_ref_count(vma));
	read_unlock(mm_mm_rb_lock(mm));
	...
	bool *need_rmap_locks)
{
	...
	if (new_vma) {
		...
		*need_rmap_locks = (new_vma->vm_pgoff <= vma->vm_pgoff);
		 ...
		 * speculative page fault handler. But we don't want it
