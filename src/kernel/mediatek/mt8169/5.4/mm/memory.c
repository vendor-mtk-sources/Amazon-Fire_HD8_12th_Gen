...
again:
	...
	if (!pmd_same(pmdval, vm_fault_orig_pmd(vmf)))
		goto out;
#endif
		...
		unsigned long address, unsigned int flags)
{
	...
	vm_fault_sequence(&vmf) = raw_read_seqcount(&vma_spf_vm_sequence(vma));
#endif
...
extern const struct vm_operations_struct *spf_ext4_file_vm_ops;
#endif
#ifdef CONFIG_F2FS_FS
extern const struct vm_operations_struct *spf_f2fs_file_vm_ops;
#endif
/* Caller must guarantee vm_ops is not NULL and valid */
static bool filemap_support_spf(const struct vm_operations_struct *vm_ops)
{
	if (vm_ops->fault == filemap_fault)
		return true;
#ifdef CONFIG_EXT4_FS
	if (vm_ops == spf_ext4_file_vm_ops)
		return true;
#endif
#ifdef CONFIG_F2FS_FS
	if (vm_ops == spf_f2fs_file_vm_ops)
		return true;
#endif
	return false;
}
		...
		unsigned long access_vm)
{
	...
	seq = raw_read_seqcount(&vma_spf_vm_sequence(vma));
	if (seq & 1)
		goto out_put;
	...
	if (vma_is_anonymous(vma)) {
		...
		/* Check if we supoort this vm_ops */
		if (!filemap_support_spf(vma->vm_ops))
			goto out_put;
#else
		/*
	...
	vm_fault_orig_pmd(&vmf) = READ_ONCE(*vmf.pmd);
	/*
	 ...
	 */
	if (unlikely(pmd_devmap(vm_fault_orig_pmd(&vmf)) ||
		     pmd_none(vm_fault_orig_pmd(&vmf)) || pmd_trans_huge(vm_fault_orig_pmd(&vmf)) ||
		     is_swap_pmd(vm_fault_orig_pmd(&vmf))))
		goto out_walk;
	 ...
	 */
	if (read_seqcount_retry(&vma_spf_vm_sequence(vma), seq)) {
...
use_comm:
	...
	if (pucBuf != &aucBuf[0]) {
		...
		printk("No cmdline, using comm %s, %d\n", task->comm,
			aucBuf[0]);
